...
/// A fake sdn session used to provide summaries with a REST client to handle room avatars.
...
- (void)shareViewController:(ShareViewController *)shareViewController didRequestShareForRoomIdentifiers:(NSSet<NSString *> *)roomIdentifiers
{
    ...
    MXSession *session = [[MXSession alloc] initWithSDNRestClient:restClient];
    ...
    [session setStore:self.fileStore success:^{
        ...
        for (NSString *roomIdentifier in roomIdentifiers) {
            MXRoom *room = [MXRoom loadRoomFromStore:self.fileStore withRoomId:roomIdentifier sdnSession:session];
            if (room) {
        ...
        MXLogError(@"[ShareManager] Failed preparing sdn session");
    }];
}
...
- (void)checkFakeSession
{
    ...
    fakeSession = [[MXSession alloc] initWithSDNRestClient:mxRestClient];
}
