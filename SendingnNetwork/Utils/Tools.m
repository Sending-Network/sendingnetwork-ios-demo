    ...
    if (user)
    {
        switch (user.presence)
        {
            ...
            case MXPresenceUnknown:
                // Fix https://github.com/Sending-Network/sendingnetwork-ios/issues/6597
...
+ (BOOL)isUniversalLink:(NSURL*)url
{
    ...
    for (NSString *sdnPermalinkHost in BuildSettings.permalinkSupportedHosts)
    {
        if ([url.host isEqualToString:sdnPermalinkHost])
        {
            NSArray<NSString*> *hostPaths = BuildSettings.permalinkSupportedHosts[sdnPermalinkHost];
            if (hostPaths.count)
            {
