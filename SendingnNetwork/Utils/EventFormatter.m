...
NSString *const FunctionalMembersStateEventType = @"io.sendingnetwork.functional_members";
                ...
                || [event.type isEqualToString:kWidgetModularEventTypeString])
        {
            ...
            if (widget)
            {
                ...
                else
                {
                    ...
                    NSMutableArray<MXEvent*> *widgetStateEvents = [NSMutableArray arrayWithArray:[roomState stateEventsWithType:kWidgetSDNEventTypeString]];
                    ...
                    for (MXEvent *widgetStateEvent in widgetStateEvents)
                    {
                        ...
                        {
                            Widget *activeWidget = [[Widget alloc] initWithWidgetEvent:widgetStateEvent inSDNSession:mxSession];
                                            ...
                                            error:(MXKEventFormatterError*)error
{
    ...
    if (events.count)
    {
        ...
        else if (events[0].eventType == MXEventTypeRoomMember)
        {
            ...
            // TODO: Build a complete summary like SendingnNetwork-web
...
{
    // Override this method to ignore the identicons defined by default in sdn kit.
...
/**
 Gets the latest state event of type `io.sendingnetwork.functional_members` from the supplied array of state events.
 Note: This function will be expensive on big rooms, recommended for use only on DMs.
 @return An event of type `io.sendingnetwork.functional_members`, or nil if the event wasn't found.
 */
