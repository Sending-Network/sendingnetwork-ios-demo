...
#import "UIViewController+SendingnNetworkSearch.h"
...
/**
 `UIViewControllerSendingnNetworkSearchInternals` is the internal single point storage for the search feature.
 
 It hosts all required data so that only one associated object can be used in the category.
 */
@interface UIViewControllerSendingnNetworkSearchInternals : NSObject
...
@implementation UIViewControllerSendingnNetworkSearchInternals
@end


#pragma mark - UIViewController+SendingnNetworkSearch
#pragma mark -

@interface UIViewController ()

// The single associated object hosting all data.
@property(nonatomic) UIViewControllerSendingnNetworkSearchInternals *searchInternals;

@end

@implementation UIViewController (SendingnNetworkSearch)
...
- (void)setSearchInternals:(UIViewControllerSendingnNetworkSearchInternals *)searchInternals
{
    objc_setAssociatedObject(self, @selector(searchInternals), searchInternals, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIViewControllerSendingnNetworkSearchInternals *)searchInternals
{
    UIViewControllerSendingnNetworkSearchInternals *searchInternals = objc_getAssociatedObject(self, @selector(searchInternals));
    if (!searchInternals)
    {
        // Initialise internal data at the first call
        searchInternals = [[UIViewControllerSendingnNetworkSearchInternals alloc] init];
