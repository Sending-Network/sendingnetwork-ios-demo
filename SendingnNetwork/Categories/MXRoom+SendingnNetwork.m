...
@implementation MXRoom (SendingnNetwork)
...
- (void)setRoomTag:(NSString*)tag completion:(void (^)(void))completion
{
    ...
    MXLogDebug(@"[MXRoom+SendingnNetwork] Update the room %@ tag from %@ to %@ with tag order %@", self.roomId, oldTag, tag, tagOrder);
    ...
    [self replaceTag:oldTag
           ...
           withOrder:tagOrder
             success: ^{
                 ...
                 MXLogDebug(@"[MXRoom+SendingnNetwork] Failed to update the tag %@ of room (%@)", tag, self.roomId);
...
- (void)mute:(void (^)(void))completion
{
    ...
    else
    {
        ...
        {
            MXLogDebug(@"[MXRoom+SendingnNetwork] Request in progress: ignore push rule update");
            if (completion)
            {
                completion();
            }
...
- (void)encryptionTrustLevelForUserId:(NSString*)userId onComplete:(void (^)(UserEncryptionTrustLevel userEncryptionTrustLevel))onComplete
{
    if (self.mxSession.crypto)
    {
        [self.mxSession.crypto trustLevelSummaryForUserIds:@[userId] forceDownload:NO success:^(MXUsersTrustLevelSummary *usersTrustLevelSummary) {
            ...
            MXLogErrorDetails(@"[MXRoom+SendingnNetwork] Error fetching trust level summary", error);
