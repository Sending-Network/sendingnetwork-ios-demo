...
final class TabBarCoordinator: NSObject, SplitViewMasterCoordinatorProtocol {
    ...
    // TODO: Embed UINavigationController in each tab like recommended by Apple and remove these properties. UITabBarViewController shoud not be embed in a UINavigationController (https://github.com/Sending-Network/SendingnNetwork-ios/issues/3086).
    ...
    private var currentSDNSession: MXSession? {
        return parameters.userSessionsService.mainUserSession?.sdnSession
    }
    ...
    init(parameters: TabBarCoordinatorParameters) {
        ...
        let masterNavigationController = SendingnNetworkNavigationController()
    ...
    func start(with spaceId: String?) {
        ...
        if self.hasStartedOnce == false {
            ...
            // Add existing SDN sessions if any
            for userSession in self.parameters.userSessionsService.userSessions {
                self.addSDNSessionToMasterTabBarController(userSession.sdnSession)
            }
    ...
    private func createUnifiedSearchController() -> UnifiedSearchViewController {
        ...
        for userSession in self.parameters.userSessionsService.userSessions {
            viewController.addSDNSession(userSession.sdnSession)
        }
    ...
    private func updateMasterTabBarController(with spaceId: String?, forceReload: Bool = false) {
        ...
        self.masterTabBarController.filterRooms(withParentId: spaceId, inSDNSession: self.currentSDNSession)
    }
    ...
    private func showRoom(withId roomId: String, eventId: String? = nil) {
        ...
        guard let sdnSession = self.parameters.userSessionsService.mainUserSession?.sdnSession else {
            return
        }
        
        self.showRoom(with: roomId, eventId: eventId, sdnSession: sdnSession)
    }
    ...
    private func showRoom(with roomId: String, eventId: String?, sdnSession: MXSession, completion: (() -> Void)? = nil) {
    ...
    @objc private func userSessionsServiceWillRemoveUserSession(_ notification: Notification) {
        ...
        self.removeSDNSessionFromMasterTabBarController(userSession.sdnSession)
    }
    
    // TODO: Remove SDN session handling from the view controller
    private func addSDNSessionToMasterTabBarController(_ sdnSession: MXSession) {
        MXLog.debug("[TabBarCoordinator] masterTabBarController.addSDNSession")
        self.masterTabBarController.addSDNSession(sdnSession)
    }
    
    // TODO: Remove SDN session handling from the view controller
    private func removeSDNSessionFromMasterTabBarController(_ sdnSession: MXSession) {
        MXLog.debug("[TabBarCoordinator] masterTabBarController.removeSDNSession")
        self.masterTabBarController.removeSDNSession(sdnSession)
    }
    ...
    func showCoachMessageIfNeeded(with session: MXSession) {
        if !SendingnNetworkSettings.shared.slideMenuRoomsCoachMessageHasBeenDisplayed {
        ...
        if isAuthenticated, let spaceService = session.spaceService, masterTabBarController.presentedViewController == nil, navigationRouter.modules.count == 1 {
                if spaceService.isInitialised && !spaceService.rootSpaceSummaries.isEmpty {
                    SendingnNetworkSettings.shared.slideMenuRoomsCoachMessageHasBeenDisplayed = true
...
extension TabBarCoordinator: MasterTabBarControllerDelegate {
    ...
    func masterTabBarController(_ masterTabBarController: MasterTabBarController!, didSelectRoomWithId roomId: String!, andEventId eventId: String!, inSDNSession sdnSession: MXSession!, completion: (() -> Void)!) {
        self.showRoom(with: roomId, eventId: eventId, sdnSession: sdnSession, completion: completion)
    }
