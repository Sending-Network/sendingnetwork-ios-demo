...
- (void)viewDidLoad
{
    ...
    // Note: UITabBarViewController should not be embed in a UINavigationController (https://github.com/Sending-Network/SendingnNetwork-ios/issues/3086)
...
- (void)initializeDataSources
{
    ...
    if (mainSession)
    {
        ...
        recentsDataSource = [[RecentsDataSource alloc] initWithSDNSession:mainSession
        ...
        if (mxSessions.count > 1)
        {
            for (MXSession *mxSession in mxSessions)
            {
                if (mxSession != mainSession)
                {
                    ...
                    [recentsDataSource addSDNSession:mxSession];
                }
            }
        }
    }
...
- (void)addSDNSession:(MXSession *)mxSession
{
    ...
    if (self.homeViewController)
    {
        ...
        if (!recentsDataSource)
        {
            ...
            // Add sdn sessions observer on first added session
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onSDNSessionStateDidChange:) name:kMXSessionStateDidChangeNotification object:nil];
            return;
        }
        else
        {
...
- (void)removeSDNSession:(MXSession *)mxSession
{
    ...
    [recentsDataSource removeSDNSession:mxSession];
    
    // Check whether there are others sessions
    if (!recentsDataSource.mxSessions.count)
    {
        // Remove sdn sessions observer
...
- (void)filterRoomsWithParentId:(NSString*)roomParentId
                inSDNSession:(MXSession*)mxSession
{
    if (roomParentId) {
...
{
    if (SendingnNetworkSettings.shared.hideVerifyThisSessionAlert
    ...
    if (session.vc_nodeConfiguration.encryption.isSecureBackupRequired)
    {
...
- (void)presentVerifyCurrentSessionAlertWithSession:(MXSession*)session
{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:[VectorL10n doNotAskAgain]
                                            ...
                                            handler:^(UIAlertAction * action) {
                                                SendingnNetworkSettings.shared.hideVerifyThisSessionAlert = YES;
                                            }]];
...
- (void)showSettingsSecurityScreenForSession:(MXSession*)session
{
    ...
    SecurityViewController *securityViewController = [SecurityViewController instantiateWithSDNSession:session];
...
- (void)resetReviewSessionsFlags
{
    ...
    SendingnNetworkSettings.shared.hideVerifyThisSessionAlert = NO;
}
