...
///
/// - checkingBackup: Load current backup on the node
/// - checkError: Fail to load current backup data
/// - noBackup: There is no backup on the node
/// - backup: There is a valid backup on the node. All keys have been backed up to it
/// - backupAndRunning: There is a valid backup on the node. Keys are being sent to it
/// - backupButNotVerified: There is a backup on the node but it has not been verified yet
