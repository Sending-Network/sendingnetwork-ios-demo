...
+ (ManageSessionViewController*)instantiateWithSDNSession:(MXSession*)sdnSession andDevice:(MXDevice*)device;
{
...
{
    MXWeakify(self);
    [self.mainSession.sdnRestClient deviceByDeviceId:device.deviceId success:^(MXDevice *device) {
...
- (void)renameDevice
{
    ...
    [currentAlert addAction:[UIAlertAction actionWithTitle:[VectorL10n ok]
                                                   ...
                                                   handler:^(UIAlertAction * action)
                             {
                                 ...
                                 [self.mainSession.sdnRestClient setDeviceName:text forDeviceId:self->device.deviceId success:^{
                                     [self reloadDeviceWithCompletion:^{
...
- (void)removeDevice
{
    ...
    [reauthenticationPresenter presentWith:coordinatorParameters animated:YES success:^(NSDictionary<NSString *,id> *_Nullable authParams) {
        ...
        [self.mainSession.sdnRestClient deleteDeviceByDeviceId:self->device.deviceId authParams:authParams success:^{
            animationCompletion();
