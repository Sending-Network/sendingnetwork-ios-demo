...
+ (SecurityViewController*)instantiateWithSDNSession:(MXSession*)sdnSession
{
    ...
    [viewController addSDNSession:sdnSession];
...
- (void)updateSections
{
    ...
    BOOL isSecureBackupRequired = self.mainSession.vc_nodeConfiguration.encryption.isSecureBackupRequired;
    ...
    if (SendingnNetworkSettings.shared.settingsSecurityScreenShowSessions && !SendingnNetworkSettings.shared.enableNewSessionManager)
    {
    ...
    if (SendingnNetworkSettings.shared.settingsSecurityScreenShowCryptographyInfo)
    {
        [cryptographySection addRowWithTag:CRYPTOGRAPHY_INFO];
    }
    
    if (SendingnNetworkSettings.shared.settingsSecurityScreenShowCryptographyExport && !isSecureBackupRequired)
    {
    ...
    if (SendingnNetworkSettings.shared.settingsSecurityScreenShowAdvancedUnverifiedDevices)
    {
...
- (void)loadDevices
{
    ...
    MXWeakify(self);
    [self.mainSession.sdnRestClient devices:^(NSArray<MXDevice *> *devices) {
...
- (MXKTableViewCell*)deviceCellWithDevice:(MXDevice*)device forTableView:(UITableView*)tableView
{
    ...
    if ([deviceId isEqualToString:self.mainSession.sdnRestClient.credentials.deviceId])
    {
        ...
        if (section == SECTION_CRYPTO_SESSIONS)
        {
            ...
            if (deviceIndex < devicesArray.count)
            {
                ...
                if (self.mainSession.crypto.crossSigning.state == MXCrossSigningStateNotBootstrapped)
                {
                    ...
                    ManageSessionViewController *viewController = [ManageSessionViewController instantiateWithSDNSession:self.mainSession andDevice:device];
                ...
                {
                    if ([device.deviceId isEqualToString:self.mainSession.sdnRestClient.credentials.deviceId])
                    {
...
- (void)exportEncryptionKeys:(UITapGestureRecognizer *)recognizer
{
    ...
    exportView = [[MXKEncryptionKeysExportView alloc] initWithSDNSession:self.mainSession];
