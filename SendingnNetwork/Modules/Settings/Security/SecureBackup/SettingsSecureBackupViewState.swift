...
enum SettingsSecureBackupViewState {
    ...
    ///
    /// - noKeyBackup: There is no backup on the node
    /// - keyBackup: There is a valid running backup on the node. Keys are being sent to it
    /// - keyBackupNotTrusted: There is a backup on the node but it is not trusted
    enum KeyBackupState {
        case noKeyBackup
