...
/**
 ...
 As a `MXKDataSource` child class, the class has a state where values have the following meanings:
     - MXKDataSourceStatePreparing: the data source is not yet ready or it is fetching data from the node.
     - MXKDataSourceStateReady: the data source data is ready.
     - MXKDataSourceStateFailed: the data source failed to fetch data.
 
 There is no way in SDN to be notified when there is a change in the public room directory.
...
/**
 The node to list public rooms from.
 Default is nil. In this case, the user's node is used.
 */
@property (nonatomic) NSString *node;

/**
 Flag to indicate to list all public rooms from all networks of `node`.
 NO will list only pure SDN rooms.
 */
...
/**
 The flag indicating that all rooms has been retrieved from the node.
 */
...
/**
 ...
 Setting a new value may trigger a request to the node. So, the data source state
 may change to MXKDataSourceStatePreparing.
 */
@property (nonatomic) NSString *searchPattern;

/**
 Paginate more public rooms matching `from the node.
...
/**
 ...
 @param mxSession the sdn session in which the room should be available.
 @return indexPath the index of the cell (nil if not found or if the related section is shrinked).
 */
- (NSIndexPath*)cellIndexPathWithRoomId:(NSString*)roomId andSDNSession:(MXSession*)mxSession;
