...
- (NSString *)directoryServerDisplayname
{
    ...
    if (_node)
    {
        directoryServerDisplayname = _node;
    }
    ...
    else
    {
        if (_includeAllNetworks)
        {
            ...
            directoryServerDisplayname = self.mxSession.sdnRestClient.credentials.nodeName;
        }
        else
        {
            // We display only SDN rooms of the user's HS
            directoryServerDisplayname = [VectorL10n sdn];
        }
    }
...
{
    if ([node isEqualToString:self.mxSession.sdnRestClient.credentials.nodeName])
    {
...
- (MXHTTPOperation *)paginate:(void (^)(NSUInteger))complete failure:(void (^)(NSError *))failure
{
    ...
    newPublicRoomsRequest = [self.mxSession.sdnRestClient publicRoomsOnServer:_node limit:_paginationLimit since:nextBatch filter:_searchPattern thirdPartyInstanceId:_thirdpartyProtocolInstance.instanceId includeAllNetworks:_includeAllNetworks success:^(MXPublicRoomsResponse *publicRoomsResponse) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < rooms.count)
    {
        ...
        [publicRoomCell render:rooms[indexPath.row] withSDNSession:self.mxSession];
...
{
    PublicRoomsDirectoryDataSource *source = [[[self class] allocWithZone:zone] initWithSDNSession:self.mxSession];
    
    source.node = [self.node copyWithZone:zone];
