...
#import <ParticleAuthServiceWrap/ParticleAuthServiceWrap.h>
...
{
    /**
     The sdn REST client used to make sdn API requests.
     */
...
@property (nonatomic, copy) NSString *random_sdm;
@property (nonatomic, copy) NSString *random_sdn;
@property (nonatomic, copy) NSString *sdmAddress;

@property (nonatomic, copy) NSString *editAddress;
...
- (void)viewDidLoad
{
    ...
    // Load welcome image from SDNKit asset bundle
    ...
    _NodeTextField.text = _defaultNodeUrl;
    ...
    // Create here REST client (if node is defined)
    [self updateRESTClient];
    
    // Localize labels
//    _NodeLabel.text = [VectorL10n loginNodeTitle];
    _NodeTextField.placeholder = [VectorL10n loginServerUrlPlaceholder];
//    _NodeInfoLabel.text = [VectorL10n loginNodeInfo];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleNotification:) name:@"NotificationLogin" object:nil];
...
//    [self loginWithSDNOtherAddress:@""];
    [self particleAction:WrapLoginTypeApple];
}
...
- (void)setDefaultNodeUrl:(NSString *)defaultNodeUrl
{
    _defaultNodeUrl = defaultNodeUrl;
    
    if (!_NodeTextField.text.length)
    {
        [self setNodeTextFieldText:defaultNodeUrl];
    }
}
...
- (void)setNodeTextFieldText:(NSString *)NodeUrl
{
    if (!NodeUrl.length)
    {
    ...
    _NodeTextField.text = NodeUrl;
    
    if (!mxRestClient || ![mxRestClient.node isEqualToString:NodeUrl])
    {
        [self updateRESTClient];
...
- (void)updateIdentityServerURL:(NSString*)url
{
    ...
    if (![self.identityService.identityServer isEqualToString:url])
    {
        if (url.length)
        {
            self.identityService = [[MXIdentityService alloc] initWithIdentityServer:url accessToken:nil andNodeRestClient:mxRestClient];
        }
        else
        {
...
- (void)checkIdentityServer
{
    ...
    NSString *node = mxRestClient.node;

    // First, fetch the IS advertised by the HS
    if (node)
    {
        MXLogDebug(@"[MXKAuthenticationVC] checkIdentityServer for node %@", node);

        autoDiscovery = [[MXAutoDiscovery alloc] initWithUrl:node];
...
- (MXHTTPOperation*)checkIdentityServerRequirementWithCompletion:(void (^)(BOOL identityServerRequired))completion
{
    ...
    {
        operation = [mxRestClient supportedSDNVersions:^(MXSDNVersions *sdnVersions) {

            MXLogDebug(@"[MXKAuthenticationVC] checkIdentityServerRequirement: %@", sdnVersions.doesServerRequireIdentityServerParam ? @"YES": @"NO");
            completion(sdnVersions.doesServerRequireIdentityServerParam);
...
- (void)setUserInteractionEnabled:(BOOL)userInteractionEnabled
{
    ...
    _NodeTextField.enabled = userInteractionEnabled;
...
- (void)setExternalRegistrationParameters:(NSDictionary*)parameters
{
    if (parameters.count)
    {
        ...
        // This is required before updating view's textfields (node url...)
        [self loadViewIfNeeded];
        
        // Force register mode
        self.authType = MXKAuthenticationTypeRegister;
        
        // Apply provided node if any
        id hs_url = parameters[@"hs_url"];
        NSString *nodeURL = nil;
        if (hs_url && [hs_url isKindOfClass:NSString.class])
        {
            nodeURL = hs_url;
        }
        [self setNodeTextFieldText:nodeURL];
...
- (void)setSoftLogoutCredentials:(MXCredentials *)softLogoutCredentials
{
    ...
    if (softLogoutCredentials)
    {
        ...
        [self setNodeTextFieldText:softLogoutCredentials.node];
...
- (void)onFailureDuringAuthRequest:(NSError *)error
{
    ...
    // detect if it is a SDN SDK issue
    if (dict)
    {
                                      ...
                                      retry:(BOOL)retry
{
    ...
    MXWeakify(self);
    [[MXKAccountManager sharedManager].dehydrationService rehydrateDeviceWithSDNRestClient:mxRestClient dehydrationKey:keyData success:^(NSString * deviceId) {
        MXStrongifyAndReturnIfNil(self);
...
{
    NSString *nodeURL = [NodeAddress sanitized:_NodeTextField.text];
    
    if (nodeURL.length)
    {
        // Check change
        if ([nodeURL isEqualToString:mxRestClient.node] == NO)
        {
            mxRestClient = [[MXRestClient alloc] initWithNode:nodeURL andOnUnrecognizedCertificateBlock:^BOOL(NSData *certificate) {
                ...
                NSString *nodeURLStr = [VectorL10n sslNodeUrl:nodeURL];
                NSString *fingerprint = [VectorL10n sslFingerprintHash:@"SHA256"];
                NSString *certFingerprint = [certificate mx_SHA256AsHexString];
                
                NSString *msg = [NSString stringWithFormat:@"%@\n\n%@\n\n%@\n\n%@\n\n%@\n\n%@", [VectorL10n sslCertNotTrust], [VectorL10n sslCertNewAccountExpl], nodeURLStr, fingerprint, certFingerprint, [VectorL10n sslOnlyAccept]];
...
- (void)dismissKeyboard
{
    ...
    [_NodeTextField resignFirstResponder];
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (textField == _NodeTextField)
    {
        [self setNodeTextFieldText:textField.text];
    }
...
- (void)particleAction:(WrapLoginType)type {
    [SDMWalletManager sharedSingleton].otherLogin = @"Other";
    
    [WrapParticleAuthService loginWithType:type account:@"" supportAuthType:@[] loginFormMode:NO successHandler:^(WrapUserInfo * userInfo) {
        MXLogDebug(@"--- userInfo.name:%@ ---", userInfo.name);
        for (int i = 0; i < userInfo.wallets.count; i ++) {
            WrapWalletInfo *wallet =  [userInfo.wallets objectAtIndex:i];
            MXLogDebug(@"wallet.publicAddress:%@ chainName:%@", wallet.publicAddress,wallet.chainName);
            if ([wallet.chainName isEqualToString:@"evm_chain"]) {
                //TODO WG
//                [[WalletService share] addWatchWalletWithEthAddress:wallet.publicAddress];
                SendingnNetworkSettings.shared.particleUserName = userInfo.name;

                MXDIDLoginIdentifier *did = [MXDIDLoginIdentifier modelFromAddress:wallet.publicAddress nonce:@"" timestamp:0 publicKey:wallet.publicAddress  token:userInfo.token type:@"" medium:@"did" salt:@""];
                MXLoginDIDIdentityProvider *provider = [MXLoginDIDIdentityProvider modelFromIdentifier:did];
//                [self loginWithDIDProvider:provider];
//                [self getSDMRandom:[NSDictionary dictionary] address:wallet.publicAddress];
                [self loginWithSDNOtherAddress:wallet.publicAddress];
                break;
            }
        }
        
    } failureHandler:^(NSError * error) {
        MXLogDebug(@"error:%@",error);
    }];
}

- (void)loginWithDIDProvider:(MXLoginDIDIdentityProvider *)provider {
    NSDictionary *parameters = provider.JSONDictionary;
    MXLogDebug(@"--- loginWithDIDProvider:%@ address:%@---",parameters,provider.identifier.address);
//    [self loginWithParameters:parameters];
}
...
- (void)loginWithParameters:(NSDictionary*)parameters
{
    ...
    NSMutableDictionary *theParameters = [NSMutableDictionary dictionaryWithDictionary:parameters];
    theParameters[@"initial_device_display_name"] = self.deviceDisplayName;
    self.currentParameter = [NSDictionary dictionaryWithDictionary:theParameters];
    [self loginWithSDNParameters:theParameters];*/

//    [self getSDMRandom:parameters];
}

- (void)getSDMRandom:(NSDictionary*)parameters address:(NSString*)address {
//    NSDictionary *identifier = [parameters objectForKey:@"identifier"];
//    NSString *address = [identifier objectForKey:@"publicKey"];
    self.sdmAddress = address;
    [self beginActivityIndicator];
    MXWeakify(self);
    [mxRestClient getSDMLoginRandomAddress:address success:^(NSDictionary *response) {
        MXStrongifyAndReturnIfNil(self);
        [self endActivityIndicator];
        self.random_sdm = [response objectForKey:@"random"];
        
        // Add the device name
        NSMutableDictionary *theParameters = [NSMutableDictionary dictionaryWithDictionary:parameters];
        theParameters[@"initial_device_display_name"] = self.deviceDisplayName;
//        self.currentParameter = [NSDictionary dictionaryWithDictionary:theParameters];
        [self loginWithSDNParameters:theParameters address:address];
        
    } failure:^(NSError *error) {
        MXStrongifyAndReturnIfNil(self);
        [self endActivityIndicator];
        [self onFailureDuringAuthRequest:error];
        [[LegacyAppDelegate theDelegate] logoutWithConfirmation:NO completion:^(BOOL isLoggedOut) {

        }];
    }];
}

- (void)loginWithSDNParameters:(NSDictionary *)parameters address:(NSString *)address{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *walletDict = [defaults objectForKey:@"walletDictKey"];
    NSString *walletTypeKey = [walletDict objectForKey:@"walletTypeKey"];
    
//    NSDictionary *identifier = [parameters objectForKey:@"identifier"];
//    NSString *address = [identifier objectForKey:@"publicKey"];
    
    if ([SDMWalletManager sharedSingleton].otherLogin.length > 0) {
        [self loginWithSDNOtherAddress:address];
    } else {
//        [self loginWithSDNWalletAddress:address];
    }
}


//other login type
- (void)loginWithSDNOtherAddress:(NSString *)address {
    MXLogDebug(@"[MXKAuthenticationVC] loginWithSDNOtherAddress:%@",address);
    [self beginActivityIndicator];
    MXWeakify(self);
    [mxRestClient getDIDList:address success:^(MXDIDListResponse *response) {
        MXStrongifyAndReturnIfNil(self);
        [self endActivityIndicator];
        MXLogDebug(@"[MXKAuthenticationVC] response:%@",response);
        if (response.array.count > 0) {
            NSString *didStr = response.array.firstObject;
            self.did = didStr;
           //TODO WG
//            self.random = [[WalletService share] randomStringWithLength:20];
            [self beginActivityIndicator];
            MXWeakify(self);
//            [self->mxRestClient postPreLoginDID:self.did address:@"" random:self.random success:^(MXPreLoginResponse *response) {
            ...
            [self->mxRestClient postPreLoginDID:self.did address:@"" success:^(MXPreLoginResponse *response) {
                ...
                NSString *updated = response.updated;
                self.did = did;
                self.updated = updated;
                self.message = message;
                self.random_sdn = response.random_server;
                MXLogDebug(@"[MXKAuthenticationVC] postPreLoginDID message:%@",message);
                SDMWalletManager.sharedSingleton.message = message;
                //sign message
                if ([[SDMWalletManager sharedSingleton].otherLogin isEqualToString:@"Other"]) {
                    NSString *hexMessage = [self convertStringToHexStr:message];
                    MXLogDebug(@"hexMessage:%@",hexMessage);
                    [WrapParticleAuthService signMessageWithMessage:hexMessage successHandler:^(NSString *signMessage) {
                        [self->mxRestClient appServiceSign:message success:^(NSDictionary *dic) {
                            [self sdnDidLoginWithSign:signMessage withAddress:address signature:dic[@"signature"]];
                        } failure:^(NSError *error) {
                            
                        }];
                        
                    } failureHandler:^(NSError *error) {
                        MXLogDebug(@"[MXKAuthenticationVC] signMessageWithMessage:%@",error);
                    }];
                } else {
//                    MXLoginDIDIdentityProvider *provider = WalletService.share.signHexMessageDIDIndentityProvider;
//                    [self sdnDidLoginWithSign:provider.identifier.token withAddress:address];
                }
                ...
                [self endActivityIndicator];
                [self onFailureDuringAuthRequest:error];
                MXLogDebug(@"[MXKAuthenticationVC] SDN Login process failure");
//                [[UploadLogManager shared] uploadLogWithAddress:address];
            }];
        ...
        [self endActivityIndicator];
        [self onFailureDuringAuthRequest:error];
        MXLogDebug(@"[MXKAuthenticationVC] SDN getDIDList failure");
//        [[UploadLogManager shared] uploadLogWithAddress:address];
    }];
}

- (void)sdnDidLoginWithSign:(NSString *)sign withAddress:(NSString *)address signature:(NSString*)signature {
    NSMutableDictionary *param = [[NSMutableDictionary alloc] init];
    [param setValue:@"m.login.did.identity" forKey:@"type"];
    [param setValue:self.updated forKey:@"updated"];
    [param setValue:@"" forKey:@"device_id"];
    [param setValue:self.random forKey:@"random"];
    [param setValue:self.random_sdn forKey:@"random_server"];
    
    NSMutableDictionary *idDict = [[NSMutableDictionary alloc] init];
    [idDict setValue:self.did forKey:@"did"];
    [idDict setValue:self.editAddress  forKey:@"address"];
    [idDict setValue:sign forKey:@"token"];
    [param setValue:idDict forKey:@"identifier"];
...
//    }
    ...
    MXLogDebug(@"[MXKAuthenticationVC] sdnDidLoginWithSign:%@",param);
    [self beginActivityIndicator];
    MXWeakify(self);
    [mxRestClient postLoginDID:self.did withParameter:param success:^(MXDIDLoginResponse *response) {
        MXStrongifyAndReturnIfNil(self);
        [self endActivityIndicator];
        ...
        loginResponse.deviceId = response.deviceId;
//        loginResponse.guideState = response.guideState;
        MXLogDebug(@"[MXKAuthenticationVC] postLoginDID:accessToken=%@",response.accessToken);
        MXLogDebug(@"[MXKAuthenticationVC] postLoginDID:%@userId=",response.userId);
        MXLogDebug(@"[MXKAuthenticationVC] postLoginDID:%@deviceId=",response.deviceId);

        MXCredentials *credentials = [[MXCredentials alloc] initWithLoginResponse:loginResponse
                                                            andDefaultCredentials:self->mxRestClient.credentials];
        
        NSDictionary *userInfo = @{@"key": @"value"};
        [[NSNotificationCenter defaultCenter] postNotificationName:@"MyNotificationNameLoginSuccess" object:nil userInfo:userInfo];
        ...
        if (!credentials.userId || !credentials.accessToken) {
            ...
            if (credentials.accessToken.length > 0) {
                NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                [defaults setValue:credentials.accessToken forKey:@"sdnAccessToken"];
                [defaults setValue:credentials.userId forKey:@"sdnUserId"];
                [defaults setValue:credentials.deviceId forKey:@"sdnDeviceId"];
                [defaults synchronize];
            }
        }
        credentials.allowedCertificate = self->mxRestClient.allowedCertificate;
        [self createSDNAccountWithCredentials:credentials];
...
- (void)createSDNAccountWithCredentials:(MXCredentials *)credentials {
    MXKAccount *account = [[MXKAccount alloc] initWithCredentials:credentials];
    account.identityServerURL = credentials.identityServer;
    [[MXKAccountManager sharedManager] addAccount:account andOpenSession:YES];
}
...
- (UIActivityIndicatorView *)customActivityIndicator {
    if (_customActivityIndicator == nil) {
        _customActivityIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
        _customActivityIndicator.backgroundColor = [UIColor colorWithRed:0.8 green:0.8 blue:0.8 alpha:1.0];
        _customActivityIndicator.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        _customActivityIndicator.hidesWhenStopped = YES;
        
        CGRect frame = _customActivityIndicator.frame;
        frame.size.width += 30;
        frame.size.height += 30;
        _customActivityIndicator.bounds = frame;
        [_customActivityIndicator.layer setCornerRadius:5];
        
        _customActivityIndicator.center = self.view.center;
...
- (void)beginActivityIndicator
{
    ...
    {
        [self.view bringSubviewToFront:self.customActivityIndicator];
        ...
        [UIView animateWithDuration:0.3 delay:0.3 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            self.customActivityIndicator.alpha = 1;
