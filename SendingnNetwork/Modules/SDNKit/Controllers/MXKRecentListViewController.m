...
- (void)onSDNSessionChange
{
    ...
    {
        if ([MXKRoomDataSourceManager sharedManagerForSDNSession:mxSession].isServerSyncInProgress)
        {
...
- (void)displayList:(MXKRecentsDataSource *)listDataSource
{
    ...
    if (dataSource)
    {
        ...
        // Remove associated sdn sessions
        NSArray *mxSessions = self.mxSessions;
        for (MXSession *mxSession in mxSessions)
        {
    ...
    // Report all sdn sessions at view controller level to update UI according to sessions state
...
- (void)dataSource:(MXKDataSource *)dataSource didAddSDNSession:(MXSession *)mxSession
{
    [self addSDNSession:mxSession];
}

- (void)dataSource:(MXKDataSource *)dataSource didRemoveSDNSession:(MXSession *)mxSession
{
    [self removeSDNSession:mxSession];
}
                ...
                if ([cellData conformsToProtocol:@protocol(MXKRecentCellDataStoring)])
                {
                    ...
                    else
                    {
                        [_delegate recentListViewController:self
                                            ...
                                            inSDNSession:recentCellData.mxSession];
                    }
                }
            }
        }
