...
- (void)setMxAccount:(MXKAccount *)account
{
    ...
    if (account)
    {
        // Report sdn account session
        [self addSDNSession:account.mxSession];
...
- (void)saveUserInfo
{
    ...
    if (isAvatarUpdated)
    {
        if (uploadedPictureURL == nil)
        {
            ...
            // Upload picture
            MXMediaLoader *uploader = [MXMediaManager prepareUploaderWithSDNSession:self.mainSession initialRange:0 andRange:1.0];
...
- (void)updateUserPicture:(NSString *)avatar_url force:(BOOL)force
{
    if (force || currentPictureURL == nil || [currentPictureURL isEqualToString:avatar_url] == NO)
    {
        ...
        currentDownloadId = [MXMediaManager thumbnailDownloadIdForSDNContentURI:currentPictureURL
                                                                          inFolder:kMXMediaManagerAvatarThumbnailFolder
                                                                     toFitViewSize:self.userPictureButton.frame.size
                                                                        withMethod:MXThumbnailingMethodCrop];
        if (!currentDownloadId)
        {
            // Set the placeholder in case of invalid SDN Content URI.
            [self updateUserPictureButton:self.picturePlaceholder];
        }
        else
        {
            ...
            {
                NSString *cacheFilePath = [MXMediaManager thumbnailCachePathForSDNContentURI:currentPictureURL
                                                                                        andType:nil
                                                                                       inFolder:kMXMediaManagerAvatarThumbnailFolder
                                                                                  toFitViewSize:self.userPictureButton.frame.size
                                                                                     withMethod:MXThumbnailingMethodCrop];
                ...
                else
                {
                    ...
                    [[NSNotificationCenter defaultCenter] addObserver:self
                                                             selector:@selector(onMediaLoaderStateChange:)
                                                                 name:kMXMediaLoaderStateDidChangeNotification
                                                               object:nil];
                    imageLoader = [self.mainSession.mediaManager downloadThumbnailFromSDNContentURI:currentPictureURL
                                                                                              withType:nil
                                                                                              inFolder:kMXMediaManagerAvatarThumbnailFolder
                                                                                         toFitViewSize:self.userPictureButton.frame.size
                                                                                            withMethod:MXThumbnailingMethodCrop
...
- (IBAction)onButtonPressed:(id)sender
{
    ...
    else if (sender == emailSubmitButton)
    {
        ...
        [submittedEmail requestValidationTokenWithSDNRestClient:self.mainSession.sdnRestClient isDuringRegistration:NO nextLink:nil success:^{
...
//            textView.text = [NSString stringWithFormat:@"%@\n%@\n%@", [VectorL10n settingsConfigNode:_mxAccount.mxCredentials.Node], [VectorL10n settingsConfigIdentityServer:_mxAccount.identityServerURL], [VectorL10n settingsConfigUserId:_mxAccount.mxCredentials.userId]];
