...
{
    sdnIDs = _contact.sdnIdentifiers;
    return sdnIDs.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (row < sdnIDs.count)
    {
        cell.mxkLabel.text = [sdnIDs objectAtIndex:row];
    }
    else
    {
        // should never happen
...
{
    if (self.delegate && sender.tag < sdnIDs.count)
    {
        sender.enabled = NO;
        
        [self.delegate contactDetailsViewController:self startChatWithSDNId:[sdnIDs objectAtIndex:sender.tag] completion:^{
