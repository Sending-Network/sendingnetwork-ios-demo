...
/**
 ...
 all sdnKit view controllers and table view controllers.
 
 It manages the following points:
 - sdn sessions handling, one or more sessions are supported.
 - stop/start activity indicator according to the state of the associated sdn sessions.
 - update view appearance on sdn session state change.
...
/**
 Enable the change of the navigation bar tint color according to the sdn node reachability status (YES by default).
 Set NO this property to disable navigation tint color change.
 */
@property (nonatomic) BOOL enableBarTintColorStatusChange;

/**
 List of associated sdn sessions (empty by default).
...
/**
 The first associated sdn session is considered as the main session (nil by default).
 */
...
/**
 Add a sdn session in the list of associated sessions (see 'mxSessions' property).
 ...
 @param mxSession a SDN session.
 */
- (void)addSDNSession:(MXSession*)mxSession;

/**
 Remove a sdn session from the list of associated sessions (see 'mxSessions' property).
 
 Remove the session. The 'kMXSessionStateDidChangeNotification' observer is removed if there is no more sdn session.
...
/**
 ...
 A session is automatically removed from the list when its state is 'MXSessionStateClosed'. Else [self onSDNSessionChange] is called.
...
/**
 This method is called on the following sdn session changes:
 ...
 - switches in red the navigation bar tintColor when all sessions are in `MXSessionStateNodeNotReachable` state.
 - switches in orange the navigation bar tintColor when at least one session is in `MXSessionStateNodeNotReachable` state.
