...
/**
 Tells the delegate that the user wants to start chat with the contact by using the selected sdn id.
 
 @param contactDetailsViewController the `MXKContactDetailsViewController` instance.
 @param sdnId the selected sdn id of the contact.
 @param completion the block to execute at the end of the operation (independently if it succeeded or not).
 */
- (void)contactDetailsViewController:(MXKContactDetailsViewController *)contactDetailsViewController startChatWithSDNId:(NSString*)sdnId completion:(void (^)(void))completion;
