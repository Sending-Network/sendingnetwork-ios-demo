...
@interface MXKContactListViewController ()
{
    ...
    BOOL displaySDNUsers;
    
    // screenshot of the local contacts
    NSArray* localContactsArray;
    MXKSectionedContacts* sectionedLocalContacts;
    
    // screenshot of the sdn users
    NSArray* sdnContactsArray;
    MXKSectionedContacts* sectionedSDNContacts;
...
- (void)viewDidLoad
{
    ...
    // global init
    displaySDNUsers = (0 == self.contactsControls.selectedSegmentIndex);
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onContactsRefresh:) name:kMXKContactManagerDidUpdateSDNContactsNotification object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onContactsRefresh:) name:kMXKContactManagerDidUpdateLocalContactSDNIDsNotification object:nil];
...
- (void)updateSectionedSDNContacts:(BOOL)force
{
    [self stopActivityIndicator];
    
    MXKContactManager* sharedManager = [MXKContactManager sharedManager];
    
    if (force || !sdnContactsArray)
    {
        sdnContactsArray = sharedManager.sdnContacts;
        sectionedSDNContacts = [sharedManager getSectionedContacts:sdnContactsArray];
    }
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    else if (displaySDNUsers)
    {
        [self updateSectionedSDNContacts:NO];
        sectionNb = sectionedSDNContacts.sectionedContacts.count;
...
{
    MXKSectionedContacts* sectionedContacts = contactsSearchBar ? sectionedFilteredContacts : (displaySDNUsers ? sectionedSDNContacts : sectionedLocalContacts);
...
- (NSString *)tableView:(UITableView *)aTableView titleForHeaderInSection:(NSInteger)section
{
    ...
    MXKSectionedContacts* sectionedContacts = displaySDNUsers ? sectionedSDNContacts : sectionedLocalContacts;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (contact)
    {
        cell.contactAccessoryViewType = MXKContactTableCellAccessorySDNIcon;
        [cell render:contact];
        cell.delegate = self;
    }
...
{
    if ([notif.name isEqualToString:kMXKContactManagerDidUpdateSDNContactsNotification])
    {
        [self updateSectionedSDNContacts:YES];
    }
    ...
    else //if ([notif.name isEqualToString:kMXKContactManagerDidUpdateLocalContactSDNIDsNotification])
    {
...
- (void)search:(id)sender
{
    ...
    if (!contactsSearchBar)
    {
        ...
        if (sectionedContacts.sectionedContacts.count > 0)
        {
            ...
            filteredContacts = [(displaySDNUsers ? sdnContactsArray : localContactsArray) mutableCopy];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    if ((contactsSearchBar == searchBar) && (![latestSearchedPattern isEqualToString:searchText]))
    {
        ...
        // contacts
        NSArray* contacts = displaySDNUsers ? sdnContactsArray : localContactsArray;
