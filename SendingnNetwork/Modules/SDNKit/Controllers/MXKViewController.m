...
#import "UIViewController+SDNKit.h"
#import "MXSession+SDNKit.h"
...
- (void)setDefaultBarTintColor:(UIColor *)barTintColor
{
    ...
    {
        // Force update by taking into account the sdn session state.
        [self onSDNSessionChange];
    }
    else
    {
...
- (void)onSDNSessionChange
{
    ...
    if (mxSessionArray.count)
    {
        ...
        for (NSUInteger index = 0; index < mxSessionArray.count;)
        {
            ...
            {
                // Call here the dedicated method which may be overridden.
                // This method will call again [onSDNSessionChange] when session is removed.
                [self removeSDNSession:mxSession];
                return;
            }
            else
            {
                if (mxSession.state == MXSessionStateNodeNotReachable)
                {
                    barTintColor = [UIColor orangeColor];
                }
                else
                {
                    allNodeNotReachable = NO;
        ...
        // Check whether the navigation bar color depends on node reachability.
        if (enableBarTintColorStatusChange)
        {
            // The navigation bar tintColor reflects the sdn node reachability status.
            if (allNodeNotReachable)
            {
