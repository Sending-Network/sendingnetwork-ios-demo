...
@interface MXKRoomCreationView ()
{
    ...
    // Array of node suffix (NSString instance)
    NSMutableArray *NodeSuffixArray;
}
...
- (void)setMxSessions:(NSArray *)mxSessions
{
    ...
    if (mxSessions.count)
    {
        NodeSuffixArray = [NSMutableArray array];
        
        for (MXSession *mxSession in mxSessions)
        {
            NSString *nodeSuffix = mxSession.sdnRestClient.nodeSuffix;
            if (nodeSuffix && [NodeSuffixArray indexOfObject:nodeSuffix] == NSNotFound)
            {
                [NodeSuffixArray addObject:nodeSuffix];
            }
        }
    }
    else
    ...
    if (NodeSuffixArray.count == 1)
    {
        _roomAliasTextField.placeholder = [VectorL10n roomCreationAliasPlaceholderWithNode:NodeSuffixArray.firstObject];
    }
    else
    {
        _roomAliasTextField.placeholder = [VectorL10n roomCreationAliasPlaceholder];
...
- (NSString*)alias
{
    ...
    if (alias.length)
    {
        ...
        for (NSString *NodeSuffix in NodeSuffixArray)
        {
            // Remove node suffix
            NSRange range = [alias rangeOfString:NodeSuffix];
...
- (void)selectSDNSession:(void (^)(MXSession *selectedSession))onSelection
{
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (textField == _roomAliasTextField)
    {
        if (NodeSuffixArray.count == 1)
        {
            ...
            if (range.location == NSNotFound)
            {
                textField.text = [textField.text stringByAppendingString:NodeSuffixArray.firstObject];
            }
        }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    else if (textField == _roomAliasTextField)
    {
        ...
        if (!textField.text.length || textField.text.length == range.length)
        {
            if ([string hasPrefix:@"#"] == NO)
            {
                if ([string isEqualToString:@":"] && NodeSuffixArray.count == 1)
                {
                    textField.text = [NSString stringWithFormat:@"#%@",NodeSuffixArray.firstObject];
                }
                else
                {
                    textField.text = [NSString stringWithFormat:@"#%@",string];
        ...
        else if (NodeSuffixArray.count == 1)
        {
...
- (IBAction)onButtonPressed:(id)sender
{
    ...
    [self selectSDNSession:^(MXSession *selectedSession)
    {
        if (sender == self->_createRoomBtn)
        {
            ...
            [selectedSession createRoomWithParameters:roomCreationParameters success:^(MXRoom *room) {
                                    ...
                                    {
                                        // Open created room
                                        [self.delegate roomCreationView:self showRoom:room.roomId withSDNSession:selectedSession];
                                    }
