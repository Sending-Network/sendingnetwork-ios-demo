...
NSString *kMXKJavascriptOnRegistered = @"window.sdnRegistration.onRegistered = function(nodeUrl, userId, accessToken) {   \
sendObjectMessage({  \
'action': 'onRegistered',           \
'Node': nodeUrl,        \
'userId': userId,                   \
...
NSString *kMXKJavascriptOnLogin = @"window.sdnLogin.onLogin = function(response) {   \
sendObjectMessage({  \
'action': 'onLogin',           \
'response': response        \
});                                     \
    ...
    if ([urlString hasPrefix:@"js:"])
    {
        ...
        else
        {
            if ([@"onRegistered" isEqualToString:parameters[@"action"]])
            {
                ...
                // We cannot use [MXLoginResponse modelFromJSON:] because of https://github.com/sdn-org/synapse/issues/4756
                // Because of this issue, we cannot get the device_id allocated by the node
                // TODO: Fix it once the node issue is fixed (filed at https://github.com/Sending-Network/SendingnNetwork-meta/issues/273).
                MXLoginResponse *loginResponse = [MXLoginResponse new];
                loginResponse.node = parameters[@"Node"];
                loginResponse.userId = parameters[@"userId"];
                loginResponse.accessToken = parameters[@"accessToken"];
                
                MXLogDebug(@"[MXKAuthenticationFallbackWebView] Registered on node: %@", loginResponse.node);

                // Sanity check
                if (loginResponse.node.length && loginResponse.userId.length && loginResponse.accessToken.length)
                {
            ...
            else if ([@"onLogin" isEqualToString:parameters[@"action"]])
            {
                ...
                MXLogDebug(@"[MXKAuthenticationFallbackWebView] Logged in on node: %@", loginResponse.node);
