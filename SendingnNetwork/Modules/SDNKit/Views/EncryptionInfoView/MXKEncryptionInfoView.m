...
- (instancetype)initWithEvent:(MXEvent*)event andSDNSession:(MXSession*)session
{
...
- (instancetype)initWithDeviceInfo:(MXDeviceInfo*)deviceInfo andSDNSession:(MXSession*)session
{
    self = [[[self class] nib] instantiateWithOwner:nil options:nil].firstObject;
    if (self)
    {
...
- (void)updateTextViewText
{
    ...
    if (_mxDeviceInfo)
    {
        ...
        _verifyButton.hidden = _blockButton.hidden = [_mxDeviceInfo.deviceId isEqualToString:_mxSession.sdnRestClient.credentials.deviceId];
