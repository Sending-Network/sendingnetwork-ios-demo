...
// The regex used to find sdn ids.
...
+ (void)initialize
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        userIdRegex = [NSRegularExpression regularExpressionWithPattern:kMXToolsRegexStringForSDNUserIdentifier options:NSRegularExpressionCaseInsensitive error:nil];
        roomIdRegex = [NSRegularExpression regularExpressionWithPattern:kMXToolsRegexStringForSDNRoomIdentifier options:NSRegularExpressionCaseInsensitive error:nil];
        roomAliasRegex = [NSRegularExpression regularExpressionWithPattern:kMXToolsRegexStringForSDNRoomAlias options:NSRegularExpressionCaseInsensitive error:nil];
        eventIdRegex = [NSRegularExpression regularExpressionWithPattern:kMXToolsRegexStringForSDNEventIdentifier options:NSRegularExpressionCaseInsensitive error:nil];
        ...
        // if we have a custom clientPermalinkBaseUrl, we also need to support sdn.to permalinks
        NSString *permalinkPattern = [NSString stringWithFormat:@"(?:%@|%@)%@", BuildSettings.clientPermalinkBaseUrl, kMXSDNDotToUrl, kMXKToolsRegexStringForPermalink];
...
+ (void)createLinksInMutableAttributedString:(NSMutableAttributedString*)mutableAttributedString forEnabledSDNIds:(NSInteger)enabledSDNIdsBitMask
{
    ...
    if (enabledSDNIdsBitMask & MXKTOOLS_USER_IDENTIFIER_BITWISE)
    {
    ...
    if (enabledSDNIdsBitMask & MXKTOOLS_ROOM_IDENTIFIER_BITWISE)
    {
    ...
    if (enabledSDNIdsBitMask & MXKTOOLS_ROOM_ALIAS_BITWISE)
    {
    ...
    if (enabledSDNIdsBitMask & MXKTOOLS_EVENT_IDENTIFIER_BITWISE)
    {
    ...
    // This allows to check for normal url based links (like https://sendingnetwork.io)
    ...
    [regex enumerateMatchesInString:mutableAttributedString.string
        ...
        if (!hasAlreadyLink)
        {
            if (!linkMatches)
            {
                ...
                // manage URLs with 2 hashes like "https://sdn.to/#/#sdn:sdn.org"
