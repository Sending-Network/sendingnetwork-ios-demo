...
/**
 `MXKEventFormatter` is an utility class for formating SDN events into strings which
...
@protected
    /**
     The sdn session. Used to get contextual data.
     */
    MXSession *mxSession;
...
/**
 Flags indicating if the formatter must create clickable links for SDN user ids,
 room ids, room aliases or event ids.
 Default is NO.
 */
@property (nonatomic) BOOL treatSDNUserIdAsLink;
@property (nonatomic) BOOL treatSDNRoomIdAsLink;
@property (nonatomic) BOOL treatSDNRoomAliasAsLink;
@property (nonatomic) BOOL treatSDNEventIdAsLink;

/**
 Initialise the event formatter.

 @param mxSession the SDN to retrieve contextual data.
