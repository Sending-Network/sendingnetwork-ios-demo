...
- (instancetype)initWithSDNSession:(MXSession *)sdnSession
{
    self = [super init];
    if (self)
    {
        ...
        // Use the same list as sdn-react-sdk ( https://github.com/sdn-org/sdn-react-sdk/blob/24223ae2b69debb33fa22fcda5aeba6fa93c93eb/src/HtmlUtils.js#L25 )
        _allowedHTMLTags = @[
                             @"font", // custom to sdn for IRC-style font coloring
                             @"del", // for markdown
        ...
        defaultRoomSummaryUpdater = [MXRoomSummaryUpdater roomSummaryUpdaterForSession:sdnSession];
                                            ...
                                            error:(MXKEventFormatterError *)error
{
    ...
    *error = MXKEventFormatterErrorNone;
    ...
    if (isRedacted)
    {
        ...
        if ((SendingnNetworkSettings.shared.enableThreads && [mxSession.threadingService isEventThreadRoot:event])
    ...
    switch (event.eventType)
    {
        ...
        case MXEventTypeRoomMessage:
        {
            ...
            else
            {
                ...
                else if (event.isReplyEvent || (eventThreadId && !SendingnNetworkSettings.shared.enableThreads))
                {
                ...
                if (body)
                {
                    ...
                    if ([msgtype isEqualToString:kMXMessageTypeEmote])
                    {
                        ...
                        if (event.relatesTo.inReplyTo || (!SendingnNetworkSettings.shared.enableThreads && event.isInThread))
                        {
                     ...
                     andLatestRoomState:(MXRoomState*)latestRoomState
{
    ...
    if (roomState && (event.isReplyEvent || (!SendingnNetworkSettings.shared.enableThreads && event.isInThread)))
    {
...
- (NSString*)renderReplyTo:(NSString*)htmlString withRoomState:(MXRoomState*)roomState
{
    ...
    // Replace <a href=\"https://sdn.to/#/mxid\">mxid</a>
    // By <a href=\"https://sdn.to/#/mxid\">Display name</a>
    // To replace the user SDN ID by his display name when available.
...
- (void)postRenderAttributedString:(NSMutableAttributedString*)mutableAttributedString
{
    ...
    NSInteger enabledSDNIdsBitMask= 0;

    // If enabled, make user id clickable
    if (_treatSDNUserIdAsLink)
    {
        enabledSDNIdsBitMask |= MXKTOOLS_USER_IDENTIFIER_BITWISE;
    }

    // If enabled, make room id clickable
    if (_treatSDNRoomIdAsLink)
    {
        enabledSDNIdsBitMask |= MXKTOOLS_ROOM_IDENTIFIER_BITWISE;
    }

    // If enabled, make room alias clickable
    if (_treatSDNRoomAliasAsLink)
    {
        enabledSDNIdsBitMask |= MXKTOOLS_ROOM_ALIAS_BITWISE;
    }

    // If enabled, make event id clickable
    if (_treatSDNEventIdAsLink)
    {
        enabledSDNIdsBitMask |= MXKTOOLS_EVENT_IDENTIFIER_BITWISE;
    }

    [MXKTools createLinksInMutableAttributedString:mutableAttributedString forEnabledSDNIds:enabledSDNIdsBitMask];
}
