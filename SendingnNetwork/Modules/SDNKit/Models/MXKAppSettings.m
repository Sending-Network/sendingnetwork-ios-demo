...
static NSString *const kMXAppGroupID = @"group.org.sdn";
...
+ (NSString *)cacheFolder
{
    ...
    // Use a dedicated cache folder for SDNKit
    cacheFolder = [cacheFolder stringByAppendingPathComponent:@"SDNKit"];
    ...
    if (cacheFolder && ![[NSFileManager defaultManager] fileExistsAtPath:cacheFolder])
    {
        ...
        if (error)
        {
            MXLogDebug(@"[MXKAppSettings] cacheFolder: Error: Cannot create SDNKit folder at %@. Error: %@", cacheFolder, error);
        }
    }
...
-(instancetype)init
{
    if (self = [super init])
    {
        ...
        firstURLDetectionIgnoredHosts = @[[NSURL URLWithString:kMXSDNDotToUrl].host];
...
- (NSArray<NSString *> *)firstURLDetectionIgnoredHosts
{
    if (self == [MXKAppSettings standardAppSettings])
    {
        return [NSUserDefaults.standardUserDefaults objectForKey:@"firstURLDetectionIgnoredHosts"] ?: @[[NSURL URLWithString:kMXSDNDotToUrl].host];
    }
    else
    {
