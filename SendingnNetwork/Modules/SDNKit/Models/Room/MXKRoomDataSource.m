...
#import "MXKTools.h"
#import "MXAggregatedReactions+SDNKit.h"
...
+ (void)loadRoomDataSourceWithRoomId:(NSString*)roomId threadId:(NSString*)threadId andSDNSession:(MXSession*)mxSession onComplete:(void (^)(id roomDataSource))onComplete
{
    MXKRoomDataSource *roomDataSource = [[self alloc] initWithRoomId:roomId andSDNSession:mxSession threadId:threadId];
    [self ensureSessionStateForDataSource:roomDataSource initialEventId:nil andSDNSession:mxSession onComplete:onComplete];
}

+ (void)loadRoomDataSourceWithRoomId:(NSString*)roomId initialEventId:(NSString*)initialEventId threadId:(NSString*)threadId andSDNSession:(MXSession*)mxSession onComplete:(void (^)(id roomDataSource))onComplete
{
    MXKRoomDataSource *roomDataSource = [[self alloc] initWithRoomId:roomId initialEventId:initialEventId threadId:threadId andSDNSession:mxSession];
    [self ensureSessionStateForDataSource:roomDataSource initialEventId:initialEventId andSDNSession:mxSession onComplete:onComplete];
}
...
+ (void)ensureSessionStateForDataSource:(MXKRoomDataSource*)roomDataSource initialEventId:(NSString*)initialEventId andSDNSession:(MXSession*)mxSession onComplete:(void (^)(id roomDataSource))onComplete
{
...
- (instancetype)initWithRoomId:(NSString *)roomId andSDNSession:(MXSession *)sdnSession threadId:(NSString *)threadId
{
    self = [super initWithSDNSession:sdnSession];
    if (self)
    {
        ...
        NSString *virtualRoomId = [sdnSession virtualRoomOf:_roomId];
        if (virtualRoomId)
        {
        ...
        self.eventFormatter = [[MXKEventFormatter alloc] initWithSDNSession:self.mxSession];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(virtualRoomsDidChange:) name:kMXSessionVirtualRoomsDidChangeNotification object:sdnSession];
    }
    return self;
}

- (instancetype)initWithRoomId:(NSString*)roomId initialEventId:(NSString*)initialEventId2 threadId:(NSString*)threadId andSDNSession:(MXSession*)mxSession
{
    self = [self initWithRoomId:roomId andSDNSession:mxSession threadId:threadId];
    if (self)
    {
        if (initialEventId2)
        {
...
{
    self = [self initWithRoomId:peekingRoom2.roomId initialEventId:theInitialEventId threadId:nil andSDNSession:peekingRoom2.mxSession];
    if (self)
    {
        ...
        if (_room)
        {
            ...
            if (_isLive)
            {
                ...
                [_room liveTimeline:^(id<MXEventTimeline> liveTimeline) {
                    ...
                    // This assumption is satisfied by SDNKit. Only MXRoomDataSource does it.
            ...
            if (MXTimelineDirectionForwards == direction)
            {
                ...
                if (self.room.outgoingMessages.count && [event.sender isEqualToString:self.mxSession.myUser.userId])
                {
                    ...
                    if (localEcho)
                    {
                        ...
                        {
                            // Replace the local echo by the true event sent by the node
...
- (void)paginateToFillRect:(CGRect)rect direction:(MXTimelineDirection)direction withMinRequestMessagesCount:(NSUInteger)minRequestMessagesCount success:(void (^)(void))success failure:(void (^)(NSError *error))failure
{
    ...
    @synchronized(bubbles)
    {
        ...
        else if (minRequestMessagesCount && [self canPaginate:direction])
        {
            ...
            // Give a chance to load data from the store before doing node requests
...
- (void)resendEventWithEventId:(NSString *)eventId success:(void (^)(NSString *))success failure:(void (^)(NSError *))failure
{
    ...
    else if ([event.type isEqualToString:kMXEventTypeStringRoomMessage])
    {
        ...
        {
            // Resend the SDN event by reusing the existing echo
        ...
        else if ([msgType isEqualToString:kMXMessageTypeImage])
        {
            ...
            if (contentURL && [contentURL hasPrefix:kMXMediaUploadIdPrefix])
            {
                ...
                NSString *localImagePath = [MXMediaManager cachePathForSDNContentURI:contentURL andType:mimetype inFolder:_roomId];
        ...
        else if ([msgType isEqualToString:kMXMessageTypeAudio])
        {
            ...
            NSString *localFilePath = [MXMediaManager cachePathForSDNContentURI:contentURL andType:mimetype inFolder:_roomId];
