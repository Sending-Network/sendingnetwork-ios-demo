...
- (instancetype)initWithEvent:(MXEvent*)event andMediaManager:(MXMediaManager*)mediaManager
{
    ...
    if (self)
    {
        ...
        _cacheFilePath = [MXMediaManager cachePathForSDNContentURI:_contentURL andType:mimetype inFolder:_eventRoomId];
        _downloadId = [MXMediaManager downloadIdForSDNContentURI:_contentURL inFolder:_eventRoomId];
...
- (NSString*)getThumbnailCachePath
{
    if (_mxcThumbnailURI)
    {
        return [MXMediaManager cachePathForSDNContentURI:_mxcThumbnailURI andType:_thumbnailMimeType inFolder:_eventRoomId];
    }
    ...
    {
        return [MXMediaManager thumbnailCachePathForSDNContentURI:_contentURL
                                                             andType:@"image/jpeg"
...
- (NSString *)getThumbnailDownloadId
{
    if (_mxcThumbnailURI)
    {
        return [MXMediaManager downloadIdForSDNContentURI:_mxcThumbnailURI inFolder:_eventRoomId];
    }
    // In case of an unencrypted image, consider the thumbnail URI deduced from the content URL, except if
    // the attachment is currently uploading.
    // Note: When the uploading is in progress, the upload id is stored in the content url (nasty trick).
    ...
    {
        return [MXMediaManager thumbnailDownloadIdForSDNContentURI:_contentURL
                                                             inFolder:_eventRoomId
                                                        toFitViewSize:CGSizeMake(kThumbnailWidth, kThumbnailHeight)
                                                           withMethod:MXThumbnailingMethodScale];
    }
...
- (void)getThumbnail:(void (^)(MXKAttachment *, UIImage *))onSuccess failure:(void (^)(MXKAttachment *, NSError *error))onFailure
{
    ...
    if (thumbnailFile)
    {
        ...
        {
            [_mediaManager downloadEncryptedMediaFromSDNContentFile:thumbnailFile
    ...
    else
    {
        ...
        {
            [_mediaManager downloadMediaFromSDNContentURI:_mxcThumbnailURI
        ...
        else
        {
            ...
            [_mediaManager downloadThumbnailFromSDNContentURI:_contentURL
                                                         withType:@"image/jpeg"
                                                        inFolder:_eventRoomId
                                                   toFitViewSize:CGSizeMake(kThumbnailWidth, kThumbnailHeight)
                                                      withMethod:MXThumbnailingMethodScale
...
- (void)prepare:(void (^)(void))onAttachmentReady failure:(void (^)(NSError *error))onFailure
{
    ...
    else
    {
        ...
        if (!loader)
        {
            if (_isEncrypted)
            {
                loader = [_mediaManager downloadEncryptedMediaFromSDNContentFile:contentFile
                                                                           mimeType:mimetype
                                                                           inFolder:_eventRoomId];
            }
            else
            {
                loader = [_mediaManager downloadMediaFromSDNContentURI:_contentURL
                                                                 withType:mimetype
                                                                 inFolder:_eventRoomId];
            }
        }
