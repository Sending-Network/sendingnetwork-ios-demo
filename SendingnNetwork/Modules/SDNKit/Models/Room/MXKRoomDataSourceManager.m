...
+ (MXKRoomDataSourceManager *)sharedManagerForSDNSession:(MXSession *)mxSession
{
    // Manage a pool of managers: one per SDN session
    ...
    @synchronized(_roomDataSourceManagers)
    {
        ...
        // If not available yet, create the `MXKRoomDataSourceManager` for this SDN session
        roomDataSourceManager = _roomDataSourceManagers[mxSessionId];
        if (!roomDataSourceManager)
        {
            roomDataSourceManager = [[MXKRoomDataSourceManager alloc]initWithSDNSession:mxSession];
...
+ (void)removeSharedManagerForSDNSession:(MXSession*)mxSession
{
...
{
    // Check first the sdn session state
...
- (void)roomDataSourceForRoom:(NSString *)roomId create:(BOOL)create onComplete:(void (^)(MXKRoomDataSource *roomDataSource))onComplete
{
    ...
    if (!roomDataSource && create && roomId)
    {
        ...
        [_roomDataSourceClass loadRoomDataSourceWithRoomId:roomId threadId:nil andSDNSession:mxSession onComplete:^(id roomDataSource) {
