...
/**
 In case the cell data represents a node, its description.
 */
...
/**
 Define a MXKDirectoryServerCellData that will store a node.

 @param node the node name (ex: "sdn.org).
 @param includeAllNetworks YES to list all public rooms on the node whatever their protocol.
                           NO to list only sdn rooms.
 */
- (id)initWithNode:(NSString*)node includeAllNetworks:(BOOL)includeAllNetworks;
