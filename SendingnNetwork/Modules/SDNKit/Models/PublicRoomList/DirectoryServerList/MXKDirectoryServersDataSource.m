...
- (void)loadData
{
    ...
    // Add user's HS
    NSString *userNode = self.mxSession.sdnRestClient.credentials.nodeName;
    id<MXKDirectoryServerCellDataStoring> cellData = [[class alloc] initWithNode:userNode includeAllNetworks:YES];
    [cellDataArray addObject:cellData];

    // Add user's HS but for SDN public rooms only
    cellData = [[class alloc] initWithNode:userNode includeAllNetworks:NO];
    [cellDataArray addObject:cellData];

    // Add custom directory servers
    for (NSString *node in _roomDirectoryServers)
    {
        if (![node isEqualToString:userNode])
        {
            cellData = [[class alloc] initWithNode:node includeAllNetworks:YES];
            [cellDataArray addObject:cellData];
        }
    }

    MXWeakify(self);
    request = [self.mxSession.sdnRestClient thirdpartyProtocols:^(MXThirdpartyProtocolsResponse *thirdpartyProtocolsResponse) {
