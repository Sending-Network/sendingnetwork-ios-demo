...
#import <SDNSDK/MXBackgroundModeHandler.h>
                        ...
                        failure:(void (^)(NSError *))failure
{
    ...
    if (enable)
    {
        ...
        {
            [self.mxSession.sdnRestClient setPusherWithPushkey:currentPusher.pushkey
...
#pragma mark - SDN user's profile
...
- (void)setUserDisplayName:(NSString*)displayname success:(void (^)(void))success failure:(void (^)(NSError *error))failure
{
    ...
    {
//        failure ([NSError errorWithDomain:kMXKAccountErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: [VectorL10n accountErrorSDNSessionIsNotOpened]}]);
        failure ([NSError errorWithDomain:kMXKAccountErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: @"error"}]);
...
- (void)loadCurrentPusher:(void (^)(void))success failure:(void (^)(NSError *error))failure
{
    ...
    [self.mxSession supportedSDNVersions:^(MXSDNVersions *sdnVersions) {
        if (!sdnVersions.supportsRemotelyTogglingPushNotifications)
        {
        ...
        [self.mxSession.sdnRestClient pushers:^(NSArray<MXPusher *> *pushers) {
            MXPusher *ownPusher;
        ...
        MXLogWarning(@"[MXKAccount] loadPusher: supportedSDNVersions failed due to error %@", error);
...
/**
 Create a sdn session based on the provided store.
...
-(void)openSessionWithStore:(id<MXStore>)store
{
    ...
    {
        MXLogDebug(@"[MXKAccount] SDN session cannot be created without credentials");
        return;
    }
    ...
    mxSession = [[MXSession alloc] initWithSDNRestClient:mxRestClient];
    ...
    MXKEventFormatter *eventFormatter = [[MXKEventFormatter alloc] initWithSDNSession:self.mxSession];
...
- (void)closeSession:(BOOL)clearStore
{
    ...
    if (mxSession)
    {
        ...
        [MXKRoomDataSourceManager removeSharedManagerForSDNSession:mxSession];
...
{
    if (!mxSession || !mxSession.sdnRestClient)
    {
...
- (void)softLogout
{
    ...
    // Stop SDK making requests to the node
    [mxSession close];
}
...
// Enable/Disable the APNS pusher for this account on this device on the node.
...
// Enable/Disable the pusher based on PushKit for this account on this device on the node.
...
- (void)listenToNotifications:(MXOnNotification)onNotification
{
    ...
    notificationCenterListener = [self.mxSession.notificationCenter listenToNotifications:^(MXEvent *event, MXRoomState *roomState, MXPushRule *rule)
    {
        ...
        MXKRoomDataSourceManager *roomDataSourceManager = [MXKRoomDataSourceManager sharedManagerForSDNSession:self->mxSession];
...
- (void)launchInitialServerSync
{
    ...
    // Use /sync filter corresponding to current settings and node capabilities
    MXWeakify(self);
    [self buildSyncFilter:^(MXFilterJSONModel *syncFilter) {
        ...
        [self checkSyncFilterCompatibility:syncFilter completion:^(BOOL compatible) {
            ...
            [self.mxSession startWithSyncFilter:syncFilter onServerSyncDone:^{
                ...
                MXLogDebug(@"[MXKAccount] %@: The session is ready. SDN SDK session has been started in %0.fms.", self.mxCredentials.userId, [[NSDate date] timeIntervalSinceDate:self->openSessionStartDate] * 1000);
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    ...
    [[MXKAccountManager sharedManager].dehydrationService dehydrateDeviceWithSDNRestClient:mxRestClient crossSigning:crossSigning dehydrationKey:keyData success:^(NSString *deviceId) {
...
- (void)onSDNSessionStateChange
{
    ...
    {
        // Remove listener (if any), this action is required to handle correctly sdn sdk handler reload (see clear cache)
        if (userUpdateListener)
        {
            [mxSession.myUser removeListener:userUpdateListener];
            userUpdateListener = nil;
...
- (void)supportLazyLoadOfRoomMembersWithSDNVersion:(MXSDNVersions *)sdnVersions
                                           completion:(void (^)(BOOL supportLazyLoadOfRoomMembers))completion
{
    ...
    [mxSession.store filterIdForFilter:filter success:^(NSString * _Nullable filterId) {
        ...
        {
            // Check the SDN versions supported by the HS
            if (sdnVersions)
            {
                if (sdnVersions.supportLazyLoadMembers)
                {
                    // The HS supports LL
                    completion(YES);
                }
...
- (void)buildSyncFilter:(void (^)(MXFilterJSONModel *syncFilter))completion
{
    ...
    void(^buildSyncFilter)(MXSDNVersions *) = ^(MXSDNVersions *sdnVersions) {
        BOOL supportsNotificationsForThreads = sdnVersions ? sdnVersions.supportsNotificationsForThreads : NO;
        
        if (syncWithLazyLoadOfRoomMembersSetting)
        {
            // Check if the server supports LL sync filter before enabling it
            [self supportLazyLoadOfRoomMembersWithSDNVersion:sdnVersions completion:^(BOOL supportLazyLoadOfRoomMembers) {
    ...
    [mxSession supportedSDNVersions:^(MXSDNVersions *sdnVersions) {
        buildSyncFilter(sdnVersions);
    } failure:^(NSError *error) {
