...
/**
 ...
 The notification object is the sdn user id of the account.
 */
...
/**
 `MXKAccount` object contains the credentials of a logged sdn user. It is used to handle sdn
...
/**
 The sdn session opened with the account (nil by default).
 */
@property (nonatomic, readonly) MXSession *mxSession;

/**
 The account user's display name (nil by default, available if sdn session `mxSession` is opened).
...
/**
 The account user's avatar url (nil by default, available if sdn session `mxSession` is opened).
 The notification `kMXKAccountUserInfoDidChangeNotification` is posted in case of change of this property.
 */
...
/**
 The account user's presence (`MXPresenceUnknown` by default, available if sdn session `mxSession` is opened).
 The notification `kMXKAccountUserInfoDidChangeNotification` is posted in case of change of this property.      
 */
...
/**
 ...
 This method creates or removes a pusher on the node.
...
/**
 ...
 YES when Push is turned on (locally available and enabled node side).
 */
...
/**
 Init `MXKAccount` instance with credentials. No sdn session is opened by default.
...
/**
 Invalidate the access token, close the sdn session and delete all store data.
...
/**
 ...
 @param sendLogoutServerRequest indicate to send logout request to node.
 @param completion the block to execute at the end of the operation (independently if it succeeded or not).
 */
...
/**
 Pause the current sdn session.
 
 @warning: This sdn session is paused without using background task if no background mode handler
...
/**
 Resume the current sdn session.
 */
- (void)resume;

/**
 Close the potential sdn session and open a new one if the account is not disabled.
