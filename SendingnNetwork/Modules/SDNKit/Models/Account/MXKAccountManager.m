...
NSString *const MXKAccountManagerDataType = @"org.sdn.kit.MXKAccountManagerDataType";
...
- (void)prepareSessionForActiveAccounts
{
    ...
    {
        // Check whether the account is enabled. Open a new sdn session if none.
...
- (void)addAccount:(MXKAccount *)account andOpenSession:(BOOL)openSession
{
    ...
    // Check conditions to open a sdn session
    if (openSession && !account.disabled)
    {
        // Open a new sdn session by default
...
{
    MXLogDebug(@"[MXKAccountManager] logout (%@), send logout request to node: %d", theAccount.mxCredentials.userId, sendLogoutRequest);
...
{
    NSString *sdnKitCacheFolder = [MXKAppSettings cacheFolder];
    return [sdnKitCacheFolder stringByAppendingPathComponent:kMXKAccountsKey];
}
