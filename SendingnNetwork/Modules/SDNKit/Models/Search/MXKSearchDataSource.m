...
- (instancetype)initWithSDNSession:(MXSession *)mxSession
{
    ...
    if (self)
    {
        ...
        _eventFormatter = [[MXKEventFormatter alloc] initWithSDNSession:mxSession];
...
- (void)convertNodeResultsIntoCells:(MXSearchRoomEventResults*)roomEventResults onComplete:(dispatch_block_t)onComplete
{
...
- (void)doSearch
{
    ...
    MXWeakify(self);
    searchRequest = [self.mxSession.sdnRestClient searchMessagesWithText:_searchText roomEventFilter:_roomEventFilter beforeLimit:0 afterLimit:0 nextBatch:nextBatch success:^(MXSearchRoomEventResults *roomEventResults) {
        ...
        MXWeakify(self);
        [self convertNodeResultsIntoCells:roomEventResults onComplete:^{
            MXStrongifyAndReturnIfNil(self);
