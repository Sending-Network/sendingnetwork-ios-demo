...
@interface MXKRecentsDataSource ()
{
    ...
    /**
     Array of `MXKSessionRecentsDataSource` instances (one by sdn session).
     */
...
- (instancetype)initWithSDNSession:(MXSession *)sdnSession
{
    ...
    if (self)
    {
        [self addSDNSession:sdnSession];
    }
    return self;
}

- (MXKSessionRecentsDataSource *)addSDNSession:(MXSession *)sdnSession
{
    MXKSessionRecentsDataSource *recentsDataSource = [[MXKSessionRecentsDataSource alloc] initWithSDNSession:sdnSession];
    ...
    if (recentsDataSource)
    {
        ...
        [mxSessionArray addObject:sdnSession];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(dataSource:didAddSDNSession:)])
        {
            [self.delegate dataSource:self didAddSDNSession:sdnSession];
        }
...
- (void)removeSDNSession:(MXSession*)sdnSession
{
    for (NSUInteger index = 0; index < mxSessionArray.count; index++)
    {
        ...
        if (mxSession == sdnSession)
        {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(dataSource:didRemoveSDNSession:)])
            {
                [self.delegate dataSource:self didRemoveSDNSession:sdnSession];
            }
...
- (NSIndexPath*)cellIndexPathWithRoomId:(NSString*)roomId andSDNSession:(MXSession*)sdnSession
{
    ...
    for (NSInteger section = 0; section < displayedRecentsDataSourceArray.count; section++)
    {
        ...
        if (recentsDataSource.mxSession == sdnSession)
        {
