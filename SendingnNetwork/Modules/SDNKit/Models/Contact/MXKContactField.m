...
- (id)initWithContactID:(NSString*)contactID sdnID:(NSString*)sdnID
{
    self = [super init];
...
- (void)loadAvatarWithSize:(CGSize)avatarSize
{
    ...
    if (_sdnID)
    {
        if (shouldCheckAvatarURL)
        {
            ...
            if (mxSessions.count)
            {
                // Check whether a sdn user is already known
                MXUser* user;
                MXSession *mxSession;
                
                for (mxSession in mxSessions)
                {
                    user = [mxSession userWithUserId:_sdnID];
                    if (user)
                    {
                        _sdnAvatarURL = user.avatarUrl;
                        if (_sdnAvatarURL)
                        {
                ...
                {
                    MXWeakify(self);
                    [mxSession.sdnRestClient avatarUrlForUser:_sdnID
                                                         success:^(NSString *mxAvatarUrl) {
                                                             
                                                             MXStrongifyAndReturnIfNil(self);
                                                             self.sdnAvatarURL = mxAvatarUrl;
...
- (void)downloadAvatarImage:(CGSize)avatarSize
{
    ...
    if (_sdnAvatarURL)
    {
        NSString *cacheFilePath = [MXMediaManager thumbnailCachePathForSDNContentURI:_sdnAvatarURL
                                                                                andType:nil
        ...
        {
            NSString *downloadId = [MXMediaManager thumbnailDownloadIdForSDNContentURI:_sdnAvatarURL inFolder:kMXMediaManagerAvatarThumbnailFolder toFitViewSize:avatarSize withMethod:MXThumbnailingMethodCrop];
            ...
            {
                [mediaManager downloadThumbnailFromSDNContentURI:_sdnAvatarURL
                                                                     withType:nil
                                                                     inFolder:kMXMediaManagerAvatarThumbnailFolder
                                                                toFitViewSize:avatarSize
                                                                   withMethod:MXThumbnailingMethodCrop
...
- (id)initWithCoder:(NSCoder *)coder
{
    if (self)
    {
        ...
        _sdnID = [coder decodeObjectForKey:@"sdnID"];
    }
...
- (void)encodeWithCoder:(NSCoder *)coder
{
    ...
    [coder encodeObject:_sdnID forKey:@"sdnID"];
}
