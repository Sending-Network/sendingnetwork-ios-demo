...
/**
 Posted when the sdn contact list is loaded or updated.
 The notification object is:
 - a contact Id when a sdn contact has been added/updated/removed.
 or
 - nil when all sdn contacts are concerned.
 */
extern NSString * _Nonnull const kMXKContactManagerDidUpdateSDNContactsNotification;
...
/**
 Posted when local contact sdn ids is updated.
 The notification object is:
 - a contact Id when a local contact has been added/updated/removed.
 or
 - nil when all local contacts are concerned.
 */
extern NSString * _Nonnull const kMXKContactManagerDidUpdateLocalContactSDNIDsNotification;

/**
 Posted when the presence of a sdn user linked at least to one contact has changed.
 The notification object is the sdn Id. The `userInfo` dictionary contains an `MXPresenceString` object under the `kMXKContactManagerSDNPresenceKey` key, representing the sdn user presence.
 */
extern NSString * _Nonnull const kMXKContactManagerSDNUserPresenceChangeNotification;
extern NSString * _Nonnull const kMXKContactManagerSDNPresenceKey;
...
/**
 ...
 - The sdn contacts retrieved from the sdn one-to-one rooms.
...
/**
 Block called (if any) to discover the SDN users bound to a set of third-party identifiers (email addresses, phone numbers).
...
/**
 Associated sdn sessions (empty by default).
 */
@property (nonatomic, readonly, nonnull) NSArray *mxSessions;

/**
 The current list of the contacts extracted from sdn data. Depends on 'contactManagerMXRoomSource'.
 */
@property (nonatomic, readonly, nullable) NSArray *sdnContacts;
...
/**
 The current list of the local contacts who have contact methods which can be used to invite them or to discover sdn users.
 */
 ...
 */
@property (nonatomic, readonly, nonnull) NSArray *directSDNContacts;

/// Flag to allow local contacts access or not. Default value is YES.
@property (nonatomic, assign) BOOL allowLocalContactsAccess;

/**
 Add/remove sdn session. The sdn contact list is automatically updated (see kMXKContactManagerDidUpdateSDNContactsNotification event).
 */
- (void)addSDNSession:(MXSession* _Nonnull)mxSession;
- (void)removeSDNSession:(MXSession* _Nonnull)mxSession;
...
/**
 Refresh sdn IDs for a specific local contact. See kMXKContactManagerDidUpdateLocalContactSDNIDsNotification
 posted when update is done.
 
 @param contact the local contact to refresh.
 */
- (void)updateSDNIDsForLocalContact:(MXKContact* _Nonnull)contact;

/**
 Refresh sdn IDs for all local contacts. See kMXKContactManagerDidUpdateLocalContactSDNIDsNotification
