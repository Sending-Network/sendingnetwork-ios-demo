...
final class EditHistoryCoordinatorBridgePresenter: NSObject {
    ...
    func present(from viewController: UIViewController, animated: Bool) {
        ...
        let navigationController = SendingnNetworkNavigationController(rootViewController: editHistoryCoordinator.toPresentable())
    ...
    func createEventFormatter(session: MXSession) -> EventFormatter? {
        guard let formatter = EventFormatter(sdnSession: session) else {
        ...
        formatter.treatSDNUserIdAsLink = true
        formatter.treatSDNRoomIdAsLink = true
        formatter.treatSDNRoomAliasAsLink = true
