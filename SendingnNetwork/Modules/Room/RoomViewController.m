...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!SendingnNetworkSettings.shared.threadsNoticeDisplayed && SendingnNetworkSettings.shared.enableThreads)
    {
...
- (void)removeSDNSession:(MXSession *)mxSession
{
    ...
    [super removeSDNSession:mxSession];
}

- (void)onSDNSessionChange
{
    [super onSDNSessionChange];
...
// Will be removed in the future https://github.com/Sending-Network/sendingnetwork-ios/issues/5608
...
- (void)displayRoom:(MXKRoomDataSource *)dataSource
{
    ...
    {
        roomPreviewData = nil;
        [self removeSDNSession:self.mainSession];
    }
...
{
    if (SendingnNetworkSettings.shared.enableWysiwygComposer)
    {
...
- (BOOL)supportCallOption
{
    ...
    BOOL callOptionAllowed = (self.roomDataSource.room.isDirect && SendingnNetworkSettings.shared.roomScreenAllowVoIPForDirectRoom) || (!self.roomDataSource.room.isDirect && SendingnNetworkSettings.shared.roomScreenAllowVoIPForNonDirectRoom);
    ...
    else if (self.roomDataSource)
    {
        ...
        if (self.roomDataSource.isLive)
        {
            ...
            if (self.supportCallOption)
            {
                ...
                {
                    //  voice call button for SDN call
                    ...
                    //  video call button for SDN call
        ...
        if (SendingnNetworkSettings.shared.enableThreads && !_isWaitingForOtherParticipants)
        {
...
- (void)refreshRoomInputToolbar
{
    ...
    if (userPictureView)
    {
        UIImage *preview = [AvatarGenerator generateAvatarForSDNItem:self.mainSession.myUser.userId withDisplayName:self.mainSession.myUser.displayname];
        
        // Suppose the avatar is stored unencrypted on the SDN media repository.
...
- (void)showMemberDetails:(MXRoomMember *)member
{
    ...
    [memberViewController displayRoomMember:member withSDNRoom:self.roomDataSource.room];
...
- (void)setupActions {
    ...
    if (SendingnNetworkSettings.shared.roomScreenAllowMediaLibraryAction)
    {
    ...
    if (SendingnNetworkSettings.shared.roomScreenAllowStickerAction && !self.isNewDirectChat)
    {
    ...
    if (SendingnNetworkSettings.shared.roomScreenAllowFilesAction)
    {
    ...
    if (SendingnNetworkSettings.shared.enableVoiceBroadcast && !self.isNewDirectChat)
    {
    ...
    if (SendingnNetworkSettings.shared.roomScreenAllowCameraAction)
    {
...
- (void)sendVideoAsset:(AVAsset *)videoAsset isPhotoLibraryAsset:(BOOL)isPhotoLibraryAsset
{
    ...
    if (SendingnNetworkSettings.shared.showMediaCompressionPrompt)
    {
...
- (void)showRoomWithId:(NSString*)roomId
{
    ...
    {
        [[AppDelegate theDelegate] showRoom:roomId andEventId:nil withSDNSession:self.roomDataSource.mxSession];
    }
}
...
- (void)startChatWithUserId:(NSString *)userId completion:(void (^)(void))completion
{
    ...
    {
        [[AppDelegate theDelegate] showNewDirectChat:userId withSDNSession:self.mainSession completion:completion];
    }
}
...
- (void)refreshPreviewHeader:(BOOL)isLandscapeOriented
{
    if (previewHeader)
    {
        ...
        else
        {
            ...
            {
                previewHeader.roomAvatarPlaceholder = [AvatarGenerator generateAvatarForSDNItem:roomPreviewData.roomId withDisplayName:roomPreviewData.roomName];
            }
            else
            {
...
- (void)displayRoomPreview:(RoomPreviewData *)previewData
{
    ...
    if (previewData)
    {
        ...
        [self addSDNSession:previewData.mxSession];
...
- (void)dataSource:(MXKDataSource *)dataSource didRecognizeAction:(NSString *)actionIdentifier inCell:(id<MXKCellRendering>)cell userInfo:(NSDictionary *)userInfo
{
    ...
    if (self.customizedRoomDataSource)
    {
        ...
        else if ([actionIdentifier isEqualToString:kMXKRoomBubbleCellSendingnNetworkEditButtonPressed])
        {
        ...
        else if ([actionIdentifier isEqualToString:kMXKRoomBubbleCellTapOnAttachmentView])
        {
            if (((MXKRoomBubbleTableViewCell*)cell).bubbleData.attachment.eventSentState == MXEventSentStateFailed)
            {
                ...
                if (selectedEvent)
                {
                    [self dataSource:dataSource didRecognizeAction:kMXKRoomBubbleCellSendingnNetworkEditButtonPressed inCell:cell userInfo:@{kMXKRoomBubbleCellEventKey:selectedEvent}];
                }
                else
                {
                 ...
                 [actionIdentifier isEqualToString:RoomGroupCallStatusCell.answerAction])
        {
            ...
            Widget *widget = [[Widget alloc] initWithWidgetEvent:widgetEvent
                                                 inSDNSession:self.customizedRoomDataSource.mxSession];
...
- (void)showAdditionalActionsMenuForEvent:(MXEvent*)selectedEvent inCell:(id<MXKCellRendering>)cell animated:(BOOL)animated
{
    ...
    switch (selectedEvent.eventType) {
        case MXEventTypeCustom:
            if ([selectedEvent.type isEqualToString:kWidgetSDNEventTypeString]
                || [selectedEvent.type isEqualToString:kWidgetModularEventTypeString])
            {
                Widget *widget = [[Widget alloc] initWithWidgetEvent:selectedEvent inSDNSession:self.roomDataSource.mxSession];
    ...
    if (selectedEvent.sentState == MXEventSentStateSent)
    {
        ...
        if (BuildSettings.messageDetailsAllowPermalink)
        {
            [self.eventMenuBuilder addItemWithType:EventMenuItemTypePermalink
                ...
                // Create a sdn.to permalink that is common to all sdn clients
        ...
        if (![selectedEvent.sender isEqualToString:self.mainSession.myUserId] && SendingnNetworkSettings.shared.roomContextualMenuShowReportContentOption)
        {
...
- (BOOL)dataSource:(MXKDataSource *)dataSource shouldDoAction:(NSString *)actionIdentifier inCell:(id<MXKCellRendering>)cell userInfo:(NSDictionary *)userInfo defaultValue:(BOOL)defaultValue
{
    ...
    if ([actionIdentifier isEqualToString:kMXKRoomBubbleCellShouldInteractWithURL])
    {
        ...
        else if ([MXTools isSDNUserIdentifier:absoluteURLString])
        {
            shouldDoAction = NO;
            ...
            else
            {
                ...
                if (user)
                {
                    selectedContact = [[MXKContact alloc] initSDNContactWithDisplayName:((user.displayname.length > 0) ? user.displayname : user.userId) andSDNID:user.userId];
                }
                else
                {
                    selectedContact = [[MXKContact alloc] initSDNContactWithDisplayName:userId andSDNID:userId];
                }
                [self performSegueWithIdentifier:@"showContactDetails" sender:self];
            }
        }
        // Open the clicked room
        else if ([MXTools isSDNRoomIdentifier:absoluteURLString] || [MXTools isSDNRoomAlias:absoluteURLString])
        {
            shouldDoAction = NO;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    else if ([[segue identifier] isEqualToString:@"showUnknownDevices"])
    {
        if (unknownDevices)
        {
            ...
            [usersDevicesViewController displayUsersDevices:unknownDevices andSDNSession:self.roomDataSource.mxSession onComplete:nil];
...
- (void)roomInputToolbarViewShowSendMediaActions:(MXKRoomInputToolbarView *)toolbarView
{
    ...
    self.composerCreateActionListBridgePresenter = [[ComposerCreateActionListBridgePresenter alloc] initWithActions:actionItems
                                                                                                     wysiwygEnabled:SendingnNetworkSettings.shared.enableWysiwygComposer
                                                                                              textFormattingEnabled:SendingnNetworkSettings.shared.enableWysiwygTextFormatting];
...
- (void)roomMemberDetailsViewController:(MXKRoomMemberDetailsViewController *)roomMemberDetailsViewController startChatWithMemberId:(NSString *)sdnId completion:(void (^)(void))completion
{
    [self startChatWithUserId:sdnId completion:completion];
}
                ...
                case MXKRoomViewControllerJoinRoomResultSuccess:
                {
                    ...
                    if (eventId)
                    {
                        ...
                        [RoomDataSource loadRoomDataSourceWithRoomId:self.roomDataSource.roomId
                                                    ...
                                                    andSDNSession:self.mainSession
...
//        BOOL needsReload = roomDataSource.currentTypingUsers == nil;
        // Quick fix for https://github.com/Sending-Network/sendingnetwork-ios/issues/4230
...
- (void)listenToServerNotices
{
    if (!serverNotices)
    {
        serverNotices = [[MXServerNotices alloc] initWithSDNSession:self.roomDataSource.mxSession];
        ...
        else if (self.customizedRoomDataSource.roomState.isObsolete)
        {
            ...
            [roomActivitiesView displayRoomReplacementWithRoomLinkTappedHandler:^{
                ...
                else
                {
                    ...
                    NSString *viaSenderServer = [MXTools serverNameInSDNIdentifier:stoneTombEvent.sender];
...
- (void)refreshMissedDiscussionsCount:(BOOL)force
{
    ...
    // Compute the missed notifications count of the current room by considering its notification mode in SendingnNetwork.
...
- (void)showEncryptionInformation:(MXEvent *)event
{
    ...
    EncryptionInfoView *encryptionInfoView = [[EncryptionInfoView alloc] initWithEvent:event andSDNSession:self.roomDataSource.mxSession];
        ...
        if (cell)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Force the read marker position in order to not depend on the read marker animation (https://github.com/Sending-Network/sendingnetwork-ios/issues/7420)
                    self.updateRoomReadMarker = YES;
                }
            }
        }
...
- (void)contactsTableViewController:(ContactsTableViewController *)contactsTableViewController didSelectContact:(MXKContact*)contact
{
    ...
    [invitePrompt addAction:[UIAlertAction actionWithTitle:[VectorL10n invite]
        ...
        else
        {
            ...
            // Is it an email or a SDN user ID?
            ...
            else //if ([MXTools isSDNUserIdentifier:participantId])
            {
                [room inviteUser:participantId success:^{
...
- (NSArray<RoomContextualMenuItem*>*)contextualMenuItemsForEvent:(MXEvent*)event andCell:(id<MXKCellRendering>)cell
{
    ...
    BOOL showMoreOption = (event.isState && SendingnNetworkSettings.shared.roomContextualMenuShowMoreOptionForStates)
        || (!event.isState && SendingnNetworkSettings.shared.roomContextualMenuShowMoreOptionForMessages);
    ...
    if (result)
    {
        switch (event.eventType) {
            ...
            case MXEventTypeBeaconInfo:
                ...
                break;
            case MXEventTypeCustom:
                if ([event.type isEqualToString:kWidgetSDNEventTypeString]
                    || [event.type isEqualToString:kWidgetModularEventTypeString])
                {
                    Widget *widget = [[Widget alloc] initWithWidgetEvent:event inSDNSession:self.roomDataSource.mxSession];
                    if ([widget.type isEqualToString:kWidgetTypeJitsiV1] ||
                        [widget.type isEqualToString:kWidgetTypeJitsiV2])
                    {
                        result = NO;
...
- (BOOL)showThreadOptionForEvent:(MXEvent*)event
{
    return !self.roomDataSource.threadId
        && !event.threadId
        && (SendingnNetworkSettings.shared.enableThreads || self.mainSession.store.supportedSDNVersions.supportsThreads);
}
...
- (void)showThreadsNotice
{
    ...
    threadsNoticeVC.didTapDoneButton = ^{
        ...
        [self.threadsNoticeModalPresenter dismissWithAnimated:YES completion:^{
            SendingnNetworkSettings.shared.threadsNoticeDisplayed = YES;
        }];
    };
...
- (void)highlightAndDisplayEvent:(NSString *)eventId completion:(void (^)(void))completion
{
    ...
    if (row == NSNotFound)
    {
        ...
        [RoomDataSource loadRoomDataSourceWithRoomId:self.roomDataSource.roomId
                                      initialEventId:eventId
                                            threadId:nil
                                    andSDNSession:self.roomDataSource.mxSession
...
- (void)removeTriggerTextFromComposer:(NSString *)textTrigger
{
    ...
    if (roomInputToolbarViewClass == WysiwygInputToolbarView.class && SendingnNetworkSettings.shared.enableWysiwygTextFormatting)
    {
        return;
    }
