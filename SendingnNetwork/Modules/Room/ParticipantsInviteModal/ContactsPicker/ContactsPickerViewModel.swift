...
class ContactsPickerViewModel: NSObject, ContactsPickerViewModelProtocol {
            ...
            for member in members {
                if member.userId == myUserId {
                    if member.membership == .join || member.membership == .invite {
                        ...
                        if let participant = Contact(sdnContactWithDisplayName: displayName, andSDNID: myUserId) {
    ...
    func prepare(contactsViewController: RoomInviteViewController, currentSearchText: String?) -> Bool {
        ...
        guard let contactsDataSource = ContactsDataSource(sdnSession: room.mxSession) else {
        ...
        contactsDataSource.forceSDNIdInDisplayName = true
        ...
        // List all the participants sdn user id to ignore them during the contacts search.
        for contact in actualParticipants ?? [] {
            if let userId = contact.mxMember.userId {
                contactsDataSource.ignoredContactsBySDNId[userId] = contact
            }
        }
        ...
        if let userParticipantId = self.userParticipant?.mxMember.userId {
            contactsDataSource.ignoredContactsBySDNId[userParticipantId] = userParticipant
        }
    ...
    private func handle(roomMember: MXRoomMember, session: MXSession, members: RoomMembers) {
        ...
        if displayName.isEmptyOrNil {
            // Look for the corresponding MXUser in sdn session
        ...
        if let contact = Contact(sdnContactWithDisplayName: displayName, andSDNID: roomMember.userId) {
    ...
    private func add(thirdPartyParticipant invite: MXRoomThirdPartyInvite, roomState: MXRoomState, members: RoomMembers) {
        // If the node has converted the 3pid invite into a room member, do no show it
        ...
        if let contact = Contact(sdnContactWithDisplayName: displayName, andSDNID: nil) {
...
extension ContactsPickerViewModel: ContactsTableViewControllerDelegate {
    ...
    func contactsTableViewController(_ contactsTableViewController: ContactsTableViewController!, didSelect contact: MXKContact?) {
        ...
        // Check for user
        if MXTools.isSDNUserIdentifier(contact.displayName) {
            let user = MXUser(userId: contact.displayName)
            coordinatorDelegate?.contactsPickerViewModelDidStartValidatingUser(self)
            user?.update(fromNodeOfSDNSession: self.room.mxSession, success: { [weak self] in
    ...
    private func invite(contact: MXKContact) {
        if let identifiers = contact.sdnIdentifiers as? [String], let participantId = identifiers.first {
