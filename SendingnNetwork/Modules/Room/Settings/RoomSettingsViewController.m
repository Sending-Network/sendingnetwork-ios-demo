...
#import "MXRoomSummary+SendingnNetwork.h"
...
- (void)updateSections
{
    ...
    if (aliases)
    {
        for (NSString *alias in aliases)
        {
            ...
            {
                // Add it
                if ([alias hasSuffix:self.mainSession.sdnRestClient.nodeSuffix])
                {
    ...
    if (SendingnNetworkSettings.shared.roomSettingsScreenShowDirectChatOption)
    {
    ...
    if (SendingnNetworkSettings.shared.roomSettingsScreenAllowChangingAccessSettings)
    {
    ...
    if (SendingnNetworkSettings.shared.roomSettingsScreenAllowChangingHistorySettings)
    {
    ...
    if (SendingnNetworkSettings.shared.roomSettingsScreenShowAddressSettings)
    {
    ...
    if (SendingnNetworkSettings.shared.roomSettingsScreenShowAdvancedSettings)
    {
        ...
        if (mxRoom.mxSession.crypto)
        {
            ...
            {
                if (SendingnNetworkSettings.shared.roomSettingsScreenAdvancedShowEncryptToVerifiedOption)
                {
    ...
    if (roomAliasLabel)
    {
        ...
        [currentAlert addAction:[UIAlertAction actionWithTitle:[VectorL10n roomDetailsCopyRoomUrl]
            ...
            if (weakSelf)
            {
                ...
                // Create a sdn.to permalink to the room
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    else if (textField == addAddressTextField)
    {
        ...
        {
            // Check whether node suffix should be added
            NSRange range = [textField.text rangeOfString:@":"];
            if (range.location == NSNotFound)
            {
                textField.text = [textField.text stringByAppendingString:self.mainSession.sdnRestClient.nodeSuffix];
            }
        }
    }
}
    ...
    if (textField == addAddressTextField)
    {
        ...
        if (!textField.text.length || textField.text.length == range.length)
        {
            if ([string hasPrefix:@"#"] == NO)
            {
                ...
                {
                    textField.text = [NSString stringWithFormat:@"#%@",self.mainSession.sdnRestClient.nodeSuffix];
                }
                else
                {
        ...
        else
        {
            ...
            // Add node automatically when user adds ':' at the end
    ...
    if (updatedItemsDict.count)
    {
        ...
        if (mxRoomState)
        {
            if (updatedItemsDict[kRoomSettingsAvatarKey])
            {
                ...
                // Upload picture
                uploader = [MXMediaManager prepareUploaderWithSDNSession:mxRoom.mxSession initialRange:0 andRange:1.0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == SECTION_TAG_MAIN)
    {
        ...
        {
            if (SendingnNetworkSettings.shared.roomSettingsScreenShowLowPriorityOption)
            {
    ...
    else if (section == SECTION_TAG_ADDRESSES)
    {
        if (row == ROOM_SETTINGS_ROOM_ADDRESS_NEW_ALIAS)
        {
            ...
            addAddressTextField.placeholder = [VectorL10n roomDetailsNewAddressPlaceholder:self.mainSession.sdnRestClient.nodeSuffix];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.tableView == tableView)
    {
        ...
        else if (section == SECTION_TAG_BANNED_USERS)
        {
            ...
            [roomMemberDetailsViewController displayRoomMember:bannedMembers[row] withSDNRoom:mxRoom];
...
{
    [[AppDelegate theDelegate] showNewDirectChat:sdnId withSDNSession:mxRoom.mxSession completion:completion];
}
...
- (BOOL)addRoomAlias:(NSString*)roomAlias
{
    ...
    if ([MXTools isSDNRoomAlias:roomAlias])
    {
