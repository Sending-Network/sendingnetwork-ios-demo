...
final class RoomInfoCoordinator: NSObject, RoomInfoCoordinatorType {
    ...
    private lazy var segmentedViewController: SegmentedViewController = {
        ...
        MXKRoomDataSource.load(withRoomId: self.room.roomId, threadId: nil, andSDNSession: self.session) { (dataSource) in
        ...
        controller.addSDNSession(self.session)
    ...
    private func showRoomDetails(with target: RoomInfoListTarget, animated: Bool) {
        ...
        case .search:
            MXKRoomDataSourceManager.sharedManager(forSDNSession: session)?.roomDataSource(forRoom: self.room.roomId, create: false, onComplete: { (roomDataSource) in
