...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onSDNSessionChange) name:kMXKRoomDataSourceSyncStatusChanged object:nil];
...
- (void)displayRoom:(MXKRoomDataSource *)dataSource
{
    ...
    if (dataSource)
    {
        ...
        // Report the sdn session at view controller level to update UI according to session state
        [self addSDNSession:roomDataSource.mxSession];
...
- (BOOL)sendAsIRCStyleCommandIfPossible:(NSString*)string
{
    ...
    else if ([string hasPrefix:kMXKSlashCmdChangeDisplayName])
    {
        ...
        {
            [roomDataSource.mxSession.sdnRestClient setDisplayName:displayName success:^{
    ...
    else if ([string hasPrefix:kMXKSlashCmdPartRoom])
    {
        ...
        {
            // Leave another room
            if ([MXTools isSDNRoomAlias:roomIdOrAlias])
            {
            ...
            else if ([MXTools isSDNRoomIdentifier:roomIdOrAlias])
            {
...
- (void)triggerPagination:(NSUInteger)limit direction:(MXTimelineDirection)direction
{
    ...
    [roomDataSource paginate:limit direction:direction onlyFromStore:NO success:^(NSUInteger addedCellNumber) {
        ...
        // Else there is a scroll jump on incoming message (see https://github.com/Sending-Network/vector-ios/issues/79)
        if (direction == MXTimelineDirectionBackwards)
        {
            [self updateCurrentEventIdAtTableBottom:NO];
        }
...
- (void)showEventDetails:(MXEvent *)event
{
    ...
    {
        eventDetailsView = [[customEventDetailsViewClass alloc] initWithEvent:event andSDNSession:roomDataSource.mxSession];
    }
    else
    {
        eventDetailsView = [[MXKEventDetailsView alloc] initWithEvent:event andSDNSession:roomDataSource.mxSession];
    }    
        ...
        while (index--)
        {
            ...
            if (cell && (cell.frame.origin.y < contentBottomOffsetY))
            {
                ...
                if (currentEventIdAtTableBottom)
                {
                    ...
                    {
                        // Indicate to the node that the user has read this event.
                        ...
                        [keysPrompt addAction:[UIAlertAction actionWithTitle:[VectorL10n attachmentE2eKeysImport]
                                                                     ...
                                                                     handler:^(UIAlertAction * action) {
                                                                           ...
                                                                           if (weakSelf)
                                                                           {
                                                                               ...
                                                                               self->importView = [[MXKEncryptionKeysImportView alloc] initWithSDNSession:self->roomDataSource.mxSession];
