...
- (UIImage*)picturePlaceholder
{
    if (self.mxRoomMember)
    {
        ...
        return [AvatarGenerator generateAvatarForSDNItem:self.mxRoomMember.userId withDisplayName:self.mxRoomMember.displayname];
    }
...
- (void)showRoomWithId:(NSString*)roomId
{
    ...
    [[AppDelegate theDelegate] showRoom:roomId andEventId:nil withSDNSession:self.mainSession];
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    else if (self.mxRoomMember)
    {
        ...
        if (SendingnNetworkSettings.shared.roomMemberScreenShowIgnore)
        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.section == devicesIndex)
    {
        ...
        if (indexPath.row < devicesArray.count)
        {
            ...
            deviceCell.verifyButton.hidden = deviceCell.blockButton.hidden = [deviceInfo.deviceId isEqualToString:self.mxRoom.mxSession.sdnRestClient.credentials.deviceId];
        }
        cell = deviceCell;
    }
    else
