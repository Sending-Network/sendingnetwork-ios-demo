...
- (void)setMxRoom:(MXRoom *)mxRoom
{
    ...
    [mxRoom.mxSession preloadRoomsData:_mxRoom ? @[_mxRoom.roomId, mxRoom.roomId] : @[mxRoom.roomId]
        ...
        if (self.mxRoom)
        {
            ...
            [self removeSDNSession:self.mxRoom.mxSession];
        }
        ...
        if (self.mxRoom)
        {
            ...
            // Update the current sdn session.
            [self addSDNSession:self.mxRoom.mxSession];
            ...
            for (MXRoomMember *mxMember in members)
            {
                ...
                if ([mxMember.userId isEqualToString:userId])
                {
                    if (mxMember.membership == MXMembershipJoin || mxMember.membership == MXMembershipInvite)
                    {
                        ...
                        self->userParticipant = [[Contact alloc] initSDNContactWithDisplayName:displayName andSDNID:userId];
...
- (void)handleRoomMember:(MXRoomMember*)mxMember
{
    ...
    if (mxMember.membership == MXMembershipJoin || mxMember.membership == MXMembershipInvite)
    {
        ...
        Contact *contact = [[Contact alloc] initSDNContactWithDisplayName:displayName andSDNID:mxMember.userId];
...
- (void)addRoomThirdPartyInviteToParticipants:(MXRoomThirdPartyInvite*)roomThirdPartyInvite roomState:(MXRoomState*)roomState
{
    ...
    {
        Contact *contact = [[Contact alloc] initSDNContactWithDisplayName:roomThirdPartyInvite.displayname andSDNID:nil];
...
- (void)showDetailFor:(MXRoomMember* _Nonnull)member from:(UIView* _Nullable)sourceView {
    ...
    [memberDetailsViewController displayRoomMember:member withSDNRoom:self.mxRoom];
...
{
    [[AppDelegate theDelegate] showNewDirectChat:sdnId withSDNSession:self.mxRoom.mxSession completion:completion];
}
