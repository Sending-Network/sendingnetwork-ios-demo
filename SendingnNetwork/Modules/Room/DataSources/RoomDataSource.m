...
{
    self = [super initWithRoomId:roomId andSDNSession:sdnSession threadId:threadId];
    if (self)
    {
        ...
        [sdnSession.threadingService addDelegate:self];
...
- (void)updateEventFormatter
{
    ...
    self.eventFormatter = [[EventFormatter alloc] initWithSDNSession:self.mxSession];
    self.eventFormatter.treatSDNUserIdAsLink = YES;
    self.eventFormatter.treatSDNRoomIdAsLink = YES;
    self.eventFormatter.treatSDNRoomAliasAsLink = YES;
...
{
    if (SendingnNetworkSettings.shared.enableLiveLocationSharing)
    {
        [self updateCurrentUserLocationSharingStatus];
    }
...
- (BOOL)shouldQueueEventForProcessing:(MXEvent *)event roomState:(MXRoomState *)roomState direction:(MXTimelineDirection)direction
{
    ...
    else if (SendingnNetworkSettings.shared.enableThreads)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Use the SendingnNetwork style placeholder
    if (!roomBubbleCellData.senderAvatarPlaceholder)
    {
        roomBubbleCellData.senderAvatarPlaceholder = [AvatarGenerator generateAvatarForSDNItem:roomBubbleCellData.senderId withDisplayName:roomBubbleCellData.senderDisplayName];
    }
        ...
        if ((((self.showBubbleReceipts && cellData.readReceipts.count) || cellData.reactions.count || cellData.hasLink || cellData.hasThreadRoot) && !isCollapsableCellCollapsed) || self.showReadMarker)
        {
            ...
            for (MXKRoomBubbleComponent *component in bubbleComponents)
            {
                ...
                if (component.event.sentState != MXEventSentStateFailed)
                {
                    ...
                    if (SendingnNetworkSettings.shared.enableThreads && component.thread && !self.threadId)
                    {
                        ...
                        if (receipts.count)
                        {
                            ...
                            for (MXReceiptData* data in receipts)
                            {
                                ...
                                if (roomMember)
                                {
                                    ...
                                    [placeholders addObject:[AvatarGenerator generateAvatarForSDNItem:roomMember.userId withDisplayName:roomMember.displayname]];
                                }
                            }
                        }
