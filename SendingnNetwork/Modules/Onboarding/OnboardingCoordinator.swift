...
struct OnboardingCoordinatorParameters {
    ...
    init(router: NavigationRouterType? = nil) {
        self.router = router ?? NavigationRouter(navigationController: SendingnNetworkNavigationController(isLockedToPortraitOnPhone: true))
    }
}
...
//                completeIfReady()
    ...
    private func authenticationCoordinator(_ coordinator: AuthenticationCoordinatorProtocol,
        ...
        if authenticationFlow == .register,
           ...
           let userSession = UserSessionsService.shared.userSession(withUserId: userId) {
            ...
            // If personalisation is to be shown, check that the node supports it otherwise show the congratulations screen
            if shouldShowPersonalization {
                checkNodeCapabilities(for: userSession)
                return
            } else {
    ...
    /// Checks the capabilities of the user's node in order to determine
    ...
    private func checkNodeCapabilities(for userSession: UserSession) {
        userSession.sdnSession.sdnRestClient.capabilities { [weak self] capabilities in
            ...
            MXLog.warning("[OnboardingCoordinator] Node capabilities not returned. Skipping personalisation")
            self?.beginPostAuthentication(for: userSession)
        }
    }
    ...
    /// Show the congratulations screen for new users. The screen will be configured based on the node's capabilities.
    ...
    private func congratulationsCoordinator(_ coordinator: OnboardingCongratulationsCoordinator, didCompleteWith result: OnboardingCongratulationsCoordinatorResult) {
        ...
        case .personalizeProfile(let userSession):
            if shouldShowDisplayNameScreen {
                ...
                showAnalyticsPrompt(for: userSession.sdnSession)
                return
            }
    ...
    private func completeIfReady() {
        ...
        // Reset the authentication service back to using sdn.org
