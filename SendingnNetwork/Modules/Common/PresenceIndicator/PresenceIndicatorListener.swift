...
class PresenceIndicatorListener {
    ...
    init(userId: String, presence: MXPresence, onUpdate: @escaping (MXPresence) -> Void) {
        ...
        NotificationCenter.default.addObserver(forName: .mxkContactManagerSDNUserPresenceChange,
                                               object: nil,
                                               queue: .main) { [weak self] notif in
            guard let self = self,
                  self.userId == notif.object as? String,
                  let presenceString = notif.userInfo?[kMXKContactManagerSDNPresenceKey] as? String else {
                return
            }
