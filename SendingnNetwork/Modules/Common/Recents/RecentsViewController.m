...
- (void)refreshCurrentSelectedCell:(BOOL)forceVisible
{
    ...
    if (masterTabBarController.selectedRoomId)
    {
        ...
        currentSelectedCellIndexPath = [self.dataSource cellIndexPathWithRoomId:masterTabBarController.selectedRoomId andSDNSession:masterTabBarController.selectedRoomSession];
    }
...
- (void)presentRoomJoinFailedAlertForError:(NSError*)error completion:(void(^)(void))completion
{
    ...
    {
        // minging kludge until https://sdn.org/jira/browse/SYN-678 is fixed
...
- (void)showRoomWithRoomId:(NSString*)roomId inSDNSession:(MXSession*)sdnSession
{
    [self showRoomWithRoomId:roomId andAutoJoinInvitedRoom:false inSDNSession:sdnSession];
}

- (void)showRoomWithRoomId:(NSString*)roomId andAutoJoinInvitedRoom:(BOOL)autoJoinInvitedRoom inSDNSession:(MXSession*)sdnSession
{
    MXRoom *room = [sdnSession roomWithRoomId:roomId];
...
{
    // Handle here user actions on recents for SendingnNetwork app
    if ([actionIdentifier isEqualToString:kInviteRecentTableViewCellPreviewButtonPressed])
    {
        ...
        [self showRoomWithRoomId:invitedRoom.roomId inSDNSession:invitedRoom.mxSession];
    }
    else if ([actionIdentifier isEqualToString:kInviteRecentTableViewCellAcceptButtonPressed])
    {
        ...
        Analytics.shared.joinedRoomTrigger = AnalyticsJoinedRoomTriggerInvite;
        [self showRoomWithRoomId:invitedRoom.roomId andAutoJoinInvitedRoom:true inSDNSession:invitedRoom.mxSession];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[InviteRecentTableViewCell class]])
    {
        ...
        {
            // Display the room preview
            [self showRoomWithRoomId:cellData.roomIdentifier inSDNSession:cellData.mxSession];
        }
        else
        {
...
- (void)showRoomDirectory
{
    ...
    else if (SendingnNetworkSettings.shared.roomsAllowToJoinPublicRooms)
    {
...
- (void)openPublicRoom:(MXPublicRoom *)publicRoom
{
    ...
    if ([self.recentsDataSource.publicRoomsDirectoryDataSource.mxSession isJoinedOnRoom:publicRoom.roomId])
    {
        ...
        [self showRoomWithRoomId:publicRoom.roomId
                 inSDNSession:self.recentsDataSource.publicRoomsDirectoryDataSource.mxSession];
    }
    else
    {
...
- (void)recentListViewController:(MXKRecentListViewController *)recentListViewController didSelectRoom:(NSString *)roomId inSDNSession:(MXSession *)sdnSession
{
    Analytics.shared.viewRoomTrigger = AnalyticsViewRoomTriggerRoomList;
    [self showRoomWithRoomId:roomId inSDNSession:sdnSession];
}
...
- (void)createRoomCoordinatorBridgePresenterDelegate:(CreateRoomCoordinatorBridgePresenter *)coordinatorBridgePresenter didCreateNewRoom:(MXRoom *)room
{
    [coordinatorBridgePresenter dismissWithAnimated:YES completion:^{
        ...
        [self showRoomWithRoomId:room.roomId inSDNSession:self.mainSession];
    }];
...
{
    [self showRoomWithRoomId:roomId inSDNSession:self.mainSession];
...
{
    [self showRoomWithRoomId:service.roomId inSDNSession:service.session];
}
