...
{
    if (self = [super initWithSDNSession:mxSession])
    {
...
{
    // Check node update in background
...
- (MXKSessionRecentsDataSource *)addSDNSession:(MXSession *)mxSession
{
    MXKSessionRecentsDataSource *recentsDataSource = [super addSDNSession:mxSession];

    // Initialise the public room directory data source
    // Note that it is single sdn session only for now
    if (!_publicRoomsDirectoryDataSource)
    {
        _publicRoomsDirectoryDataSource = [[PublicRoomsDirectoryDataSource alloc] initWithSDNSession:mxSession];
        _publicRoomsDirectoryDataSource.showNSFWRooms = SendingnNetworkSettings.shared.showNSFWPublicRooms;
...
- (void)removeSDNSession:(MXSession*)sdnSession
{
    [super removeSDNSession:sdnSession];
    
    // sanity check
    if (sdnSession.myUser && sdnSession.myUser.userId)
    {
        id roomTagListener = roomTagsListenerByUserId[sdnSession.myUser.userId];
        
        if (roomTagListener)
        {
            [sdnSession removeListener:roomTagListener];
            [roomTagsListenerByUserId removeObjectForKey:sdnSession.myUser.userId];
        }
    }
    
    if (_publicRoomsDirectoryDataSource.mxSession == sdnSession)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        static NSString* cellIdentifier = @"SendingnNetworkRecentsMovingCell";
        
        UITableViewCell* cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SendingnNetworkRecentsMovingCell"];
...
- (NSInteger)cellIndexPosWithRoomId:(NSString*)roomId andSDNSession:(MXSession*)sdnSession within:(NSArray<id<MXRoomSummaryProtocol>> *)summaries
{
    if (!roomId || !sdnSession || !summaries.count || self.mxSession != sdnSession)
    {
        return NSNotFound;
    }
...
- (NSIndexPath*)cellIndexPathWithRoomId:(NSString*)roomId andSDNSession:(MXSession*)sdnSession
{
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.invitesCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.favoriteCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.peopleCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.conversationCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.lowPriorityCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.serverNoticeCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.suggestedRoomCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.breadcrumbsRoomCellDataArray];
    ...
    {
        index = [self cellIndexPosWithRoomId:roomId andSDNSession:sdnSession within:self.allChatsRoomCellDataArray];
