...
final class SideMenuCoordinator: NSObject, SideMenuCoordinatorType {
    ...
    private func addSpaceListIfNeeded() {
        ...
        guard let mainSDNSession = self.parameters.userSessionsService.mainUserSession?.sdnSession else {
            return
        }
        
        self.addSpaceList(with: mainSDNSession)
    }

    private func addSpaceList(with sdnSession: MXSession) {
    ...
    private func showMenu(forSpaceWithId spaceId: String, from sourceView: UIView?) {
        guard let session = self.parameters.userSessionsService.mainUserSession?.sdnSession else {
            return
        }
