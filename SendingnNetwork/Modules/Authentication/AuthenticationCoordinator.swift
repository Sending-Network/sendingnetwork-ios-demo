...
final class AuthenticationCoordinator: NSObject, AuthenticationCoordinatorProtocol {
    ...
    @MainActor private func startAuthenticationFlow() async {
        if let softLogoutCredentials = authenticationService.softLogoutCredentials,
           let nodeAddress = softLogoutCredentials.node {
            do {
                try await authenticationService.startFlow(.login, for: nodeAddress)
            } catch {
        ...
        if BuildSettings.forceNodeSelection, authenticationService.provisioningLink?.nodeUrl == nil {
            showServerSelectionScreen(for: flow)
            return
        }
        
        do {
            // Start the flow (if nodeAddress is nil, the default server will be used).
        ...
        case .registration:
            if authenticationService.state.node.needsRegistrationFallback {
                showFallback(for: flow)
            } else {
                showRegistrationScreen()
            }
        case .login:
            if authenticationService.state.node.needsLoginFallback {
                showFallback(for: flow)
            } else {
//                showLoginScreen()
            }
...
//                showLoginScreen()
    ...
    @MainActor private func displayUnrecognizedCertificateAlert(for certificate: Data) async -> Bool {
        await withCheckedContinuation { continuation in
            ...
            let nodeURLString = VectorL10n.sslNodeUrl(authenticationService.state.node.displayableAddress)
    ...
    @MainActor private func showRegistrationScreen() {
        ...
        let parameters = AuthenticationRegistrationCoordinatorParameters(navigationRouter: navigationRouter,
                                                                         authenticationService: authenticationService,
                                                                         registrationFlow: node.registrationFlow,
                                                                         loginMode: node.preferredLoginMode)
    ...
    @MainActor private func showVerifyEmailScreen(registrationWizard: RegistrationWizard) {
        ...
        let parameters = AuthenticationVerifyEmailCoordinatorParameters(registrationWizard: registrationWizard,
                                                                        node: authenticationService.state.node)
    ...
    @MainActor private func showReCaptchaScreen(siteKey: String, registrationWizard: RegistrationWizard) {
        ...
        guard let nodeURL = URL(string: authenticationService.state.node.address) else {
            MXLog.failure("[AuthenticationCoordinator] showReCaptchaScreen: The node address is no longer a valid URL.")
            displayError(message: VectorL10n.errorCommonMessage)
            return
        }
    ...
    @MainActor private func handleRegistrationResult(_ result: RegistrationResult) {
        ...
        case .flowResponse(let flowResult):
            ...
            let node = authenticationService.state.node
            guard let nextStage = node.isSDNDotOrg ? flowResult.nextUncompletedStageOrdered : flowResult.nextUncompletedStage else {
    ...
    private func showFallback(for flow: AuthenticationFlow, deviceId: String? = nil) {
        ...
        let navController = SendingnNetworkNavigationController(rootViewController: fallbackVC)
    ...
    private func dismissFallback() {
        ...
        guard let fallbackNavigationVC = navigationRouter.toPresentable().presentedViewController as? SendingnNetworkNavigationController else {
            return
        }
    ...
    private func presentCompleteSecurity() {
        ...
        let cancellable = !session.vc_nodeConfiguration().encryption.isSecureBackupRequired
...
extension AuthenticationCoordinator: SSOAuthenticationPresenterDelegate {
    ...
    @MainActor private func presentSSOAuthentication(for identityProvider: SSOIdentityProvider) {
        let service = SSOAuthenticationService(nodeStringURL: authenticationService.state.node.address)
...
extension AuthenticationCoordinator: AuthFallBackViewControllerDelegate {
    func authFallBackViewController(_ authFallBackViewController: AuthFallBackViewController,
        ...
        guard let session = MXSession(sdnRestClient: client) else {
