                ...
                if (nbPhoneNumber && ![self isFlowCompleted:kMXLoginFlowTypeMSISDN])
                {
                    ...
                    if (restClient)
                    {
                        ...
                        [self checkIdentityServerRequirement:restClient success:^(BOOL identityServerRequired) {
                            ...
                            [self->submittedMSISDN requestValidationTokenWithSDNRestClient:restClient
                ...
                else if (!self.emailContainer.isHidden && self.emailTextField.text.length && ![self isFlowCompleted:kMXLoginFlowTypeEmailIdentity])
                {
                    ...
                    if (restClient)
                    {
                        ...
                        [self checkIdentityServerRequirement:restClient success:^(BOOL identityServerRequired) {
                            ...
                            [self->submittedEmail requestValidationTokenWithSDNRestClient:restClient
                                                                  isDuringRegistration:YES
                                                                              nextLink:nil
                                                                               success:^
                             {
...
- (BOOL)setExternalRegistrationParameters:(NSDictionary *)registrationParameters
{
    ...
    if (restClient)
    {
        ...
        if (hs_url && [hs_url isKindOfClass:NSString.class])
        {
            ...
            if ([nodeURL isEqualToString:restClient.node] == NO)
            {
                MXLogDebug(@"[AuthInputsView] setExternalRegistrationParameters failed: wrong node URL");
                return NO;
            }
        }
    ...
    if (!nodeURL.length || !clientSecret.length || !sid.length || !sessionId.length)
    {
...
{
    // Take some shortcuts and make some assumptions (SendingnNetwork uses MXFileStore and MXRealmCryptoStore) to
...
- (void)displaySoftLogoutMessageWithUserDisplayname:(NSString*)userDisplayname andKeyBackupNeeded:(BOOL)keyBackupNeeded
{
    ...
    NSString *string = [VectorL10n authSoftlogoutReason:softLogoutCredentials.nodeName :userDisplayname :softLogoutCredentials.userId];
...
- (IBAction)selectPhoneNumberCountry:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(authInputsView:presentViewController:animated:)])
    {
        ...
        phoneNumberPickerNavigationController = [[SendingnNetworkNavigationController alloc] init];
        
        // Set SendingnNetwork navigation bar colors
...
- (void)textFieldDidEndEditing:(UITextField*)textField
{
    ...
    {
        if ([MXTools isSDNUserIdentifier:self.userLoginTextField.text])
        {
...
- (BOOL)displayRecaptchaForm:(void (^)(NSString *response))callback
{
    ...
    if (siteKey.length && restClient && callback)
    {
        ...
        [reCaptchaWebView openRecaptchaWidgetWithSiteKey:siteKey fromNode:restClient.node callback:callback];
...
{
    [mxRestClient supportedSDNVersions:^(MXSDNVersions *sdnVersions) {

        MXLogDebug(@"[AuthInputsView] checkIdentityServerRequirement: %@", sdnVersions.doesServerRequireIdentityServerParam ? @"YES": @"NO");
        success(sdnVersions.doesServerRequireIdentityServerParam);
