...
- (void)viewDidLoad
{
    ...
    if (BuildSettings.forceNodeSelection)
    {
        self.defaultNodeUrl = nil;
    }
    else
    {
        self.defaultNodeUrl = SendingnNetworkSettings.shared.nodeUrlString;
    }
    
    self.defaultIdentityServerUrl = SendingnNetworkSettings.shared.identityServerUrlString;
...
//    self.NodeTextField.placeholder = [VectorL10n authNodePlaceholder];
...
- (void)userInterfaceThemeDidChange
{
    ...
    if (self.NodeTextField.placeholder)
    {
        self.NodeTextField.attributedPlaceholder = [[NSAttributedString alloc]
                                                          initWithString:self.NodeTextField.placeholder
    ...
    self.NodeSeparator.backgroundColor = ThemeService.shared.theme.lineBreakColor;
    self.NodeTextField.textColor = ThemeService.shared.theme.textPrimaryColor;
    self.NodeLabel.textColor = ThemeService.shared.theme.textSecondaryColor;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    // This bug rarely happens (https://github.com/Sending-Network/SendingnNetwork-ios/issues/1643)
...
{
    // Skip SDNKit and use a VC instead
...
- (void)showAuthenticationFallBackViewController:(NSString*)fallbackPage
{
    ...
    UINavigationController *navigationController = [[SendingnNetworkNavigationController alloc] initWithRootViewController:authFallBackViewController];
...
- (void)setCustomServerFieldsVisible:(BOOL)isVisible
{
    ...
    if (!isVisible)
    {
        ...
        if (BuildSettings.forceNodeSelection)
        {
            [self setNodeTextFieldText:nil];
        }
        else
        {
            [self setNodeTextFieldText:self.defaultNodeUrl];
        }
    ...
    else
    {
        ...
        NSString *customNodeURL = [[NSUserDefaults standardUserDefaults] objectForKey:@"customNodeURL"];
        if (customNodeURL.length)
        {
            [self setNodeTextFieldText:customNodeURL];
        }
        else
        {
...
{
    NSString *NodeURL = self.NodeTextField.text;
    if (NodeURL.length && ![NodeURL isEqualToString:self.defaultNodeUrl])
    {
        [[NSUserDefaults standardUserDefaults] setObject:NodeURL forKey:@"customNodeURL"];
    }
    else
    {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"customNodeURL"];
    }
...
- (void)authenticationViewController:(MXKAuthenticationViewController *)authenticationViewController didLogWithUserId:(NSString *)userId
{
    ...
    // Create DM with SendingnNetwork-bot on new account creation.
    if (self.authType == MXKAuthenticationTypeRegister && botCreationEnabled)
    {
        MXRoomCreationParameters *roomCreationParameters = [MXRoomCreationParameters parametersForDirectRoomWithUser:@"@SendingnNetwork-bot:sdn.org"];
        [session createRoomWithParameters:roomCreationParameters success:nil failure:^(NSError *error) {
            MXLogDebug(@"[AuthenticationVC] Create chat with SendingnNetwork-bot failed");
        }];
    }
...
- (void)customiseServersWithWellKnown:(MXWellKnown*)wellKnown
{
    if (self.customServersContainer.hidden)
    {
        ...
        if (![self.defaultNodeUrl isEqualToString:wellKnown.node.baseUrl]
            || ![self.defaultIdentityServerUrl isEqualToString:wellKnown.identityServer.baseUrl])
        {
            [self showCustomNode:wellKnown.node.baseUrl andIdentityServer:wellKnown.identityServer.baseUrl];
        }
    }
    else
    {
        ...
        else
        {
            ...
            if (![customNodeURL isEqualToString:wellKnown.node.baseUrl]
...
- (void)showCustomNode:(NSString*)node andIdentityServer:(NSString*)identityServer
{
    // Store the wellknown data into NSUserDefaults before displaying them
    [[NSUserDefaults standardUserDefaults] setObject:node forKey:@"customNodeURL"];
...
{
    NSString *NodeStringURL = self.NodeTextField.text;
    ...
    SSOAuthenticationService *ssoAuthenticationService = [[SSOAuthenticationService alloc] initWithNodeStringURL:NodeStringURL];
