...
final class EnterNewRoomDetailsViewModel: EnterNewRoomDetailsViewModelType {
    ...
    init(session: MXSession, parentSpace: MXSpace?) {
        ...
        roomCreationParameters.isEncrypted = session.vc_nodeConfiguration().encryption.isE2EEByDefaultEnabled &&  SendingnNetworkSettings.shared.roomCreationScreenRoomIsEncrypted
        roomCreationParameters.joinRule = SendingnNetworkSettings.shared.roomCreationScreenRoomIsPublic ? .public : .private
        viewState = .loaded
    }
    ...
    private func fixRoomAlias(alias: String?) -> String? {
        ...
        alias = alias.replacingOccurrences(of: ":sdn.org", with: "")
        if let node = session.credentials.node {
            alias = alias.replacingOccurrences(of: ":" + node, with: "")
        }
    ...
    private func uploadAvatarIfRequired(ofRoom room: MXRoom) {
        ...
        mediaUploader = MXMediaManager.prepareUploader(withSDNSession: session, initialRange: 0, andRange: 1.0)
