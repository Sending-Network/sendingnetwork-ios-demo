...
final class CreateRoomCoordinator: CreateRoomCoordinatorType {
    ...
    private func createRoomSelectorCoordinator(parentSpace: MXSpace) -> SDNItemChooserCoordinator {
        let paramaters = SDNItemChooserCoordinatorParameters(session: self.parameters.session, viewProvider: AddRoomSelectorViewProvider(), itemsProcessor: AddRoomItemsProcessor(parentSpace: parentSpace))
