...
- (void)viewDidLoad
{
    ...
    // Add each sdn session by default.
    ...
    ContactsDataSource *dataSource = [[ContactsDataSource alloc] initWithSDNSession:self.mainSession]; // TO TEST
    dataSource.areSectionsShrinkable = YES;
    dataSource.displaySearchInputInContactsList = YES;
    dataSource.forceSDNIdInDisplayName = YES;
...
- (void)setupInviteFriendsHeaderView
{
    ...
    if (!SendingnNetworkSettings.shared.allowInviteExernalUsers)
    {
...
- (void)addSDNSession:(MXSession *)mxSession
{
    ...
    userContact = [[MXKContact alloc] initSDNContactWithDisplayName:displayName andSDNID:self.mainSession.myUser.userId];
...
- (void)refreshParticipants
{
    ...
    [contactsDataSource.ignoredContactsBySDNId removeAllObjects];
    ...
    for (MXKContact* contact in participants)
    {
        ...
        if (identifiers.count)
        {
            ...
            contactsDataSource.ignoredContactsBySDNId[identifiers.firstObject] = contact;
        }
        else
        {
    ...
    if (userContact)
    {
        ...
        {
            contactsDataSource.ignoredContactsBySDNId[self.mainSession.myUser.userId] = userContact;
        }
    }
...
- (BOOL)participantsAlreadyContainAnEmail
{
    ...
    {
        // if it is not a sdn contact or a local contact with a SDNID
        if (participant.sdnIdentifiers.count == 0 && ![MXTools isSDNUserIdentifier:participant.displayName])
        {
            return YES;
        }
    }
...
- (BOOL)canAddParticipant: (MXKContact*) contact
{
    ...
    if (![self.mainSession vc_nodeConfiguration].encryption.isE2EEByDefaultEnabled)
    {
        return YES;
    }
    ...
    // if it is not a sdn contact, nor a local contact with a SDNID, and if there is already at least one participant, another participant cannot be added.
    if ((contact.sdnIdentifiers.count == 0 && ![MXTools isSDNUserIdentifier:contact.displayName]) && participants.count > 0)
    {
        return NO;
    }
    ...
    else if (indexPath.section == participantsSection)
    {
        ...
        else
        {
            ...
            if (index < participants.count)
            {
                ...
                {
                    participantCell.showSDNIdInDisplayName = [isMultiUseNameByDisplayName[contact.displayName] isEqualToNumber:@(YES)];
                }
            }
        }
    ...
    if (sender == createBarButtonItem)
    {
        ...
        else
        {
            ...
            if (isDirect && first3rdPartyInvitee)
            {
                ...
                {
                    [self stopActivityIndicator];
                    [[AppDelegate theDelegate] showRoom:existingRoom.roomId andEventId:nil withSDNSession:self.mainSession];
                    return;
                }
            }
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if (searchText != nil && searchText.length > 0)
    {
        ...
        if ([MXTools isSDNUserIdentifier:searchText])
        {
            contact = [[MXKContact alloc] initSDNContactWithDisplayName:searchText andSDNID:searchText];
...
- (void)contactsTableViewController:(ContactsTableViewController *)contactsTableViewController didSelectContact:(MXKContact*)contact
{
    ...
    if (!self.mainSession.sdnRestClient.identityServer && contact.sdnIdentifiers.count == 0)
    {
        NSString *participantId;
