...
final class UserInteractiveAuthenticationService: NSObject {
    ...
    func authenticationSession(with request: AuthenticatedEndpointRequest,
        ...
        return self.session.sdnRestClient.authSessionForRequest(withMethod: request.httpMethod, path: request.path, parameters: request.params, success: { [weak self] (authenticationSession) in
    ...
    /// Build UIA fallback authentication URL for a given stage (https://sdn.org/docs/spec/client_server/latest#fallback)
    /// - Parameters:
    ///   - flowIdentifier: The login type to authenticate with.
    ///   - sessionId:  The the ID of the session given by the node.
    /// - Returns: a `MXHTTPOperation` instance.
    func authenticationFallbackURL(for flowIdentifier: String, sessionId: String) -> URL? {
        guard let nodeStringURL = self.session.sdnRestClient.credentials.node, let nodeURL = URL(string: nodeStringURL) else {
            return nil
        }                
        
        let fallbackPath =  "\(kMXAPIPrefixPathR0)/auth/\(flowIdentifier)/fallback/web?session=\(sessionId)"
        
        return URL(string: fallbackPath, relativeTo: nodeURL)
    }
