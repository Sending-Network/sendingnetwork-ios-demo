...
#include <SDNSDK/MXUIKitBackgroundModeHandler.h>
...
#import <SDNOpenWebRTCWrapper/SDNOpenWebRTCWrapper.h>
#endif

#ifdef MX_CALL_STACK_ENDPOINT
#import <SDNEndpointWrapper/SDNEndpointWrapper.h>
#endif


#if __has_include(<SDNSDK/MXJingleCallStack.h>)
#define CALL_STACK_JINGLE
#endif
#ifdef CALL_STACK_JINGLE
#import <SDNSDK/MXJingleCallStack.h>
...
@interface LegacyAppDelegate () <GDPRConsentViewControllerDelegate, KeyVerificationCoordinatorBridgePresenterDelegate, PushNotificationServiceDelegate, SetPinCoordinatorBridgePresenterDelegate, CallPresenterDelegate, SpaceDetailPresenterDelegate, SecureBackupSetupCoordinatorBridgePresenterDelegate>
{
    ...
    /**
     SDNKit error observer
     */
    id sdnKitErrorObserver;
    
    /**
     sdn session observer used to detect new opened sessions.
     */
    id sdnSessionStateObserver;
    
    /**
     sdn account observers.
     */
    ...
    /**
     Prompt to warn the user about a new backup on the node.
     */
...
/**
 Used to manage on boarding steps, like create DM with SendingnNetwork bot
 */
...
- (BOOL)application:(UIApplication *)application willFinishLaunchingWithOptions:(nullable NSDictionary *)launchOptions
{
    ...
    if (!language)
    {
        // Check whether a langage was only defined at the SendingnNetwork application level.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    MXLogDebug(@"SDNSDK version: %@", SDNSDKVersion);
    MXLogDebug(@"Build: %@\n", build);
    MXLogDebug(@"------------------------------\n");
    ...
    // Set up theme
    ThemeService.shared.themeId = SendingnNetworkSettings.shared.userInterfaceTheme;
    ...
    // Add sdn observers, and initialize sdn sessions if the app is not launched in background.
    [self initSDNSessions];
...
- (void)applicationWillResignActive:(UIApplication *)application
{
    ...
    // Release SDNKit error observer
    if (sdnKitErrorObserver)
    {
        [[NSNotificationCenter defaultCenter] removeObserver:sdnKitErrorObserver];
        sdnKitErrorObserver = nil;
    }
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    // Suspend all running sdn sessions
...
- (void)afterAppUnlockedByPin:(UIApplication *)application
{
    ...
    if (SendingnNetworkSettings.shared.enableAnalytics)
    {
        #if DEBUG
    ...
    if (mainSession.state == MXSessionStateInitialSyncFailed)
    {
        ...
        NSError *error = [NSError errorWithDomain:NSURLErrorDomain
                                         ...
                                         userInfo:@{NSLocalizedDescriptionKey : [VectorL10n nodeConnectionLost]}];
    ...
    // Observe sdnKit error to alert user on error
    sdnKitErrorObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kMXKErrorNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
    ...
    // Resume all existing sdn sessions
    NSArray *mxAccounts = [MXKAccountManager sharedManager].activeAccounts;
    for (MXKAccount *account in mxAccounts)
    {
        [account resume];
    ...
    // SendingnNetwork has its own dark theme. Prevent iOS from applying its one
                         ...
                         sender:(NSString *)userId
{
    void(^sessionReadyBlock)(MXSession*) = ^(MXSession *session){
        ...
        if (threadId)
        {
            if(![[MXKRoomDataSourceManager sharedManagerForSDNSession:session] hasRoomDataSourceForRoom:roomId])
            {
...
- (BOOL)handleUniversalLink:(NSUserActivity*)userActivity
{
    ...
    // Manage email validation links from node for registration (/registration/email/submit_token)
    // and email addition (/add_threepid/email/submit_token)
    // They look like https://sdn.org/_api/client/unstable/registration/email/submit_token?token=vtQjQIZfwdoREDACTEDozrmKYSWlCXsJ&client_secret=53e679ea-oRED-ACTED-92b8-3012c49c6cfa&sid=qlBCREDACTEDEtgxD
             ...
             || [webURL.path isEqualToString:validateEmailSubmitTokenAPIPathV2])
    {
        ...
        MXSession *mainSession = self.mxSessions.firstObject;
        MXRestClient *nodeRestClient;
        
        if (mainSession.sdnRestClient)
        {
            nodeRestClient = mainSession.sdnRestClient;
        }
        else
        {
            nodeRestClient = [[MXRestClient alloc] initWithNode:identityServer andOnUnrecognizedCertificateBlock:nil];
        }
        
        MXIdentityService *identityService = [[MXIdentityService alloc] initWithIdentityServer:identityServer accessToken:nil andNodeRestClient:nodeRestClient];
...
- (BOOL)handleUniversalLinkWithParameters:(UniversalLinkParameters*)universalLinkParameters
{
    ...
    else if (([pathParams[0] hasPrefix:@"#"] || [pathParams[0] hasPrefix:@"!"]) && pathParams.count >= 1)
    {
        ...
        // Such links come from sdn.to permalinks
                ...
                else
                {
                    ...
                    if (eventId)
                    {
                        ...
                        dispatch_group_notify(eventDispatchGroup, dispatch_get_main_queue(), ^{
                            ...
                            if (SendingnNetworkSettings.shared.enableThreads)
                            {
                                if (event.threadId)
                                {
                ...
                void(^findRoom)(void) = ^{
                    if ([self->_masterTabBarController.selectedViewController conformsToProtocol:@protocol(MXKViewControllerActivityHandling)])
                    {
                        ...
                        if ([roomIdOrAlias hasPrefix:@"#"])
                        {
                            ...
                            [account.mxSession.sdnRestClient resolveRoomAlias:roomIdOrAlias success:^(MXRoomAliasResolution *resolution) {
                                ...
                                MXLogDebug(@"[AppDelegate] Universal link: Error: The node failed to resolve the room alias (%@)", roomIdOrAlias);
                        ...
                        else
                        {
                            ...
                            [account.mxSession.sdnRestClient roomSummaryWith:roomIdOrAlias via:roomPreviewData.viaServers success:^(MXPublicRoom *room) {
    ...
    else if (userId)
    {
        ...
        MXKContact *contact = [[MXKContact alloc] initSDNContactWithDisplayName:displayName andSDNID:userId];
...
#pragma mark - SDN sessions handling

// TODO: Move this method content in UserSessionsService
- (void)initSDNSessions
{
    MXLogDebug(@"[AppDelegate] initSDNSessions");

    // Set first RoomDataSource class used in Vector
    [MXKRoomDataSourceManager registerRoomDataSourceClass:RoomDataSource.class];
    
    // Register sdn session state observer in order to handle multi-sessions.
    sdnSessionStateObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kMXSessionStateDidChangeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *notif) {
        ...
        {
            [self removeSDNSession:mxSession];
        }
    ...
    addedAccountObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kMXKAccountManagerDidAddAccountNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *notif) {
        ...
        if (account)
        {
            ...
            EventFormatter *eventFormatter = [[EventFormatter alloc] initWithSDNSession:account.mxSession];
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:kMXKAccountManagerDidSoftlogoutAccountNotification  object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *notif) {
        ...
        {
            [self removeSDNSession:account.mxSession];
        }
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:kMXSessionIgnoredUsersDidChangeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull notif) {
        ...
        [[AppDelegate theDelegate] reloadSDNSessions:YES];
    ...
    // Observers have been defined, we can start a sdn session for each enabled accounts.
    MXLogDebug(@"[AppDelegate] initSDNSessions: prepareSessionForActiveAccounts (app state: %tu)", [[UIApplication sharedApplication] applicationState]);
...
- (void)addSDNSession:(MXSession *)mxSession
{
    if (mxSession)
    {
        ...
        dispatch_after(dispatch_walltime(DISPATCH_TIME_NOW, 0.3 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            [[MXKContactManager sharedManager] addSDNSession:mxSession];
        });

        // Register the session to the widgets manager
        [[WidgetManager sharedManager] addSDNSession:mxSession];
        
        // register the session to the call service
        [_callPresenter addSDNSession:mxSession];
        ...
        [self.delegate legacyAppDelegate:self didAddSDNSession:mxSession];
    }
}

- (void)removeSDNSession:(MXSession*)mxSession
{
    [[MXKContactManager sharedManager] removeSDNSession:mxSession];
    
    // remove session from the call service
    [_callPresenter removeSDNSession:mxSession];
    ...
    [[WidgetManager sharedManager] removeSDNSession:mxSession]; 
    
    // If any, disable the no VoIP support workaround
    [self disableNoVoIPOnSDNSession:mxSession];
    ...
    [self.delegate legacyAppDelegate:self didRemoveSDNSession:mxSession];
}
...
- (void)reloadSDNSessions:(BOOL)clearCache
{
    // Reload all running sdn sessions
    NSArray *mxAccounts = [MXKAccountManager sharedManager].activeAccounts;
    for (MXKAccount *account in mxAccounts)
    {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    {
        // Flush and restore SDN data
        [self reloadSDNSessions:NO];
    }
...
- (void)handleAppState
{
    ...
    if (mainSession)
    {
        ...
        if (mainSession.vc_nodeConfiguration)
        {
            [MXKAppSettings standardAppSettings].outboundGroupSessionKeyPreSharingStrategy = mainSession.vc_nodeConfiguration.encryption.outboundKeysPreSharingMode;
        }
        ...
        if (mainSession.vc_nodeConfiguration.encryption.isSecureBackupRequired
...
- (void)configureCallManagerIfRequiredForSession:(MXSession *)mxSession
{
...
#endif
#ifdef MX_CALL_STACK_ENDPOINT
    callStack = [[MXEndpointCallStack alloc] initWithSDNId:mxSession.myUser.userId];
#endif
#ifdef CALL_STACK_JINGLE
    ...
    else
    {
        ...
        [self enableNoVoIPOnSDNSession:mxSession];
    }
}
...
- (void)enableCallKit:(BOOL)enable forCallManager:(MXCallManager *)callManager
{
    ...
    if (enable)
    {
        // Create adapter for SendingnNetwork
        MXCallKitConfiguration *callKitConfiguration = [[MXCallKitConfiguration alloc] init];
        callKitConfiguration.iconName = @"callkit_icon";
        
        NSData *SendingnNetworkCallKitIconData = UIImagePNGRepresentation([UIImage imageNamed:callKitConfiguration.iconName]);
        [JitsiService.shared configureCallKitProviderWithLocalizedName:callKitConfiguration.name
                                                 ...
                                                 iconTemplateImageData:SendingnNetworkCallKitIconData];
...
- (void)checkDeviceId:(MXSession*)mxSession
{
    ...
    // no device id provided by the node.
    ...
    if (!isErrorNotificationSuspended && ![[NSUserDefaults standardUserDefaults] boolForKey:@"deviceIdAtStartupChecked"])
    {
        ...
        if (!mxSession.sdnRestClient.credentials.deviceId)
        {
...
#pragma mark - SDN Accounts handling

- (void)selectSDNAccount:(void (^)(MXKAccount *selectedAccount))onSelection
{
    NSArray *mxAccounts = [MXKAccountManager sharedManager].activeAccounts;
    ...
    if (roomId.length)
    {
        ...
        if (dedicatedAccount && dedicatedAccount.mxSession)
        {
            ...
            [self showRoom:roomId
                  threadId:threadId
                andEventId:nil
         withSDNSession:dedicatedAccount.mxSession
                    sender:userId];
        }
        else
        {
...
- (void)showRoomWithParameters:(RoomNavigationParameters*)parameters completion:(void (^)(void))completion
{
    ...
    if (roomId && mxSession)
    {
        ...
        {
            MXWeakify(self);
            [mxSession.sdnRestClient roomSummaryWith:roomId via:@[] success:^(MXPublicRoom *room) {
                MXStrongifyAndReturnIfNil(self);
                if ([room.roomTypeString isEqualToString:MXRoomTypeStringSpace])
                {
...
- (void)showRoom:(NSString*)roomId andEventId:(NSString*)eventId withSDNSession:(MXSession*)mxSession
{
    [self showRoom:roomId threadId:nil andEventId:eventId withSDNSession:mxSession sender:nil];
}

- (void)showRoom:(NSString*)roomId threadId:(NSString*)threadId andEventId:(NSString*)eventId withSDNSession:(MXSession*)mxSession sender:(NSString*)userId
{
    ...
    ThreadParameters *threadParameters = nil;
    if (SendingnNetworkSettings.shared.enableThreads && threadId)
    {
...
- (void)showNewDirectChat:(NSString*)userId withSDNSession:(MXSession*)mxSession completion:(void (^)(void))completion
{
    // Ask to restore initial display
    ScreenPresentationParameters *presentationParameters = [[ScreenPresentationParameters alloc] initWithRestoreInitialDisplay:YES];
...
- (void)createDirectChatWithUserId:(NSString*)userId completion:(void (^)(NSString *roomId))completion
{
    ...
    [self selectSDNAccount:^(MXKAccount *selectedAccount) {
    ...
    [self selectSDNAccount:^(MXKAccount *selectedAccount) {
        ...
        if (mxSession)
        {
            ...
            if (directRoom)
            {
                // open it
                Analytics.shared.viewRoomTrigger = AnalyticsViewRoomTriggerCreated;
                [self showRoom:directRoom.roomId andEventId:nil withSDNSession:mxSession];
            ...
            {
                [self showNewDirectChat:userId withSDNSession:mxSession completion:completion];
            }
        }
        else if (completion)
        {
...
- (void)promptForStunServerFallback
{
    ...
    MXSession *mainSession = self.mxSessions.firstObject;
    NSString *NodeName = mainSession.sdnRestClient.credentials.nodeName;

    NSString *message = [NSString stringWithFormat:@"%@\n\n%@",
                         [VectorL10n callNoStunServerErrorMessage1:NodeName],
    ...
    [_errorNotification addAction:[UIAlertAction actionWithTitle:[VectorL10n callNoStunServerErrorUseFallbackButton:stunFallbackHost]
        ...
        SendingnNetworkSettings.shared.allowStunServerFallback = YES;
    ...
    [_errorNotification addAction:[UIAlertAction actionWithTitle:[VectorL10n cancel]
                                                         ...
                                                         handler:^(UIAlertAction * action) {
                                                             ...
                                                             SendingnNetworkSettings.shared.allowStunServerFallback = NO;
...
- (void)checkPermissionForNativeWidget:(Widget*)widget fromUrl:(NSURL*)url completion:(void (^)(BOOL granted))completion
{
    ...
    // Check permission in user SendingnNetwork settings
    __block SendingnNetworkSharedSettings *sharedSettings = [[SendingnNetworkSharedSettings alloc] initWithSession:session];
 ...
 */
- (void)enableNoVoIPOnSDNSession:(MXSession*)mxSession
{
                                     ...
                                     switch (event.eventType)
                                     {
                                         case MXEventTypeCallInvite:
                                         {
                                             ...
                                             [self->noCallSupportAlert addAction:[UIAlertAction actionWithTitle:[VectorL10n rejectCall]
                                                 ...
                                                 [mxSession.sdnRestClient sendEventToRoom:event.roomId threadId:nil eventType:kMXEventTypeStringCallReject content:content txnId:nil success:nil failure:^(NSError *error) {
                                                     MXLogDebug(@"[AppDelegate] enableNoVoIPOnSDNSession: ERROR: Cannot send m.call.reject event.");
                                                 }];
...
- (void)disableNoVoIPOnSDNSession:(MXSession*)mxSession
{
...
- (void)registerUserDidSignInOnNewDeviceNotificationForSession:(MXSession*)session
{
    ...
    self.userDidSignInOnNewDeviceObserver = [NSNotificationCenter.defaultCenter addObserverForName:MXCrossSigningMyUserDidSignInOnNewDeviceNotification
                                                ...
                                                usingBlock:^(NSNotification *notification)
     {
         ...
         [session.sdnRestClient devices:^(NSArray<MXDevice *> *devices) {
...
- (BOOL)presentCompleteSecurityForSession:(MXSession*)mxSession
{
    ...
    {
        keyVerificationCoordinatorBridgePresenter = [[KeyVerificationCoordinatorBridgePresenter alloc] initWithSession:mxSession];
        keyVerificationCoordinatorBridgePresenter.cancellable = !mxSession.vc_nodeConfiguration.encryption.isSecureBackupRequired;
        keyVerificationCoordinatorBridgePresenter.delegate = self;
...
- (void)registerUserConsentNotGivenNotification
{
                                                ...
                                                usingBlock:^(NSNotification *notification)
    {
            ...
            && self.gdprConsentController.presentingViewController == nil)
        {
            ...
            NSString *alertMessage = [VectorL10n gdprConsentNotGivenAlertMessage:NodeName];
...
- (void)presentGDPRConsentFromViewController:(UIViewController*)viewController consentURI:(NSString*)consentURI
{
    ...
    UINavigationController *navigationController = [[SendingnNetworkNavigationController alloc] initWithRootViewController:gdprConsentViewController];
...
- (void)gdprConsentViewControllerDidConsentToGDPRWithSuccess:(GDPRConsentViewController *)gdprConsentViewController
{
    ...
    {
        // And create the room with SendingnNetwork bot in //
        self.onBoardingManager = [[OnBoardingManager alloc] initWithSession:session];
        
        MXWeakify(self);
        void (^createSendingnNetworkBotDMcompletion)(void) = ^() {
            MXStrongifyAndReturnIfNil(self);

            self.onBoardingManager = nil;
        };
        
        [self.onBoardingManager createSendingnNetworkBotDirectMessageIfNeededWithSuccess:^{
            createSendingnNetworkBotDMcompletion();
        } failure:^(NSError * _Nonnull error) {
            createSendingnNetworkBotDMcompletion();
        }];
    }
...
- (void)setupUserDefaults
{
    ...
    if (!SendingnNetworkSettings.shared.isUserDefaultsMigrated)
    {
        [SendingnNetworkSettings.shared migrate];
    }
    
    // Show encrypted message notification content by default.
    if (!SendingnNetworkSettings.shared.isShowDecryptedContentInNotificationsHasBeenSetOnce)
    {
        SendingnNetworkSettings.shared.showDecryptedContentInNotifications = BuildSettings.decryptNotificationsByDefault;
    }
    
    // Need to set `showAllRoomsInHomeSpace` to `true` for the new App Layout
    if (BuildSettings.newAppLayoutEnabled)
    {
        SendingnNetworkSettings.shared.showAllRoomsInHomeSpace = YES;
    }
    
    if (SendingnNetworkSettings.shared.forceThreadsEnabled)
    {
        SendingnNetworkSettings.shared.enableThreads = YES;
        SendingnNetworkSettings.shared.forceThreadsEnabled = NO;
    }
}
