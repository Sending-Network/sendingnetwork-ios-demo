...
final class AppCoordinator: NSObject, AppCoordinatorType {
    ...
    /// Main user SDN session
    private var mainSDNSession: MXSession? {
        return self.userSessionsService.mainUserSession?.sdnSession
    }
    ...
    func start() {
        ...
        // NOTE: When split view is shown there can be no SDN sessions ready. Keep this behavior or use a loading screen before showing the split view.
        self.showSplitView()
    ...
    func open(url: URL, options: [UIApplication.OpenURLOptionsKey: Any] = [:]) -> Bool {
        ...
        // https://developer.apple.com/documentation/xcode/allowing_apps_and_websites_to_link_to_your_content/defining_a_custom_url_scheme_for_your_app
        if ParticleAuthServiceWrap.WrapParticleAuthService.handUrl(url: url) == true {
            return true
        }
    ...
    private func setupLogger() {
        UILog.configure(logger: SDNSDKLogger.self)
    }
    ...
    fileprivate func navigate(to destination: AppNavigatorDestination) {
        ...
        case .space(let spaceId):
            ...
            Analytics.shared.activeSpace = userSessionsService.mainUserSession?.sdnSession.spaceService.getSpace(withId: spaceId)
        }
    }
...
extension AppCoordinator: LegacyAppDelegateDelegate {
    ...
    func legacyAppDelegate(_ legacyAppDelegate: LegacyAppDelegate!, didAddSDNSession session: MXSession!) {
    }
    
    func legacyAppDelegate(_ legacyAppDelegate: LegacyAppDelegate!, didRemoveSDNSession session: MXSession?) {
        ...
        self.userSessionsService.removeUserSession(relatedToSDNSession: session)
    }
