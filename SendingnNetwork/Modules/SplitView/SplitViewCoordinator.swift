...
final class SplitViewCoordinator: NSObject, SplitViewCoordinatorType {
    ...
    init(parameters: SplitViewCoordinatorParameters) {
        ...
        let splitViewController = SendingnNetworkSplitViewController()
    ...
    func start(with spaceId: String?) {
        ...
        if hasStartedOnce == false {
            ...
            let detailNavigationController = SendingnNetworkNavigationController(rootViewController: placeholderDetailViewController)
    ...
    private func releaseRoomDataSourceIfNeeded(for roomCoordinator: RoomCoordinatorProtocol) {
        ...
        let dataSourceManager = MXKRoomDataSourceManager.sharedManager(forSDNSession: session)
