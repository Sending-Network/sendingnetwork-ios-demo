...
/// https://github.com/sdn-org/matrix-analytics-events and integrated via SwiftPM
///
@objcMembers class Analytics: NSObject {
    
    // MARK: - Properties
    
    /// The singleton instance to be used within the SendingnNetwork target.
    ...
    var shouldShowAnalyticsPrompt: Bool {
        ...
        !SendingnNetworkSettings.shared.hasSeenAnalyticsPrompt && BuildSettings.analyticsConfiguration.isEnabled
    }
    ...
    var promptShouldDisplayUpgradeMessage: Bool {
        SendingnNetworkSettings.shared.hasAcceptedMatomoAnalytics
    }
    ...
    func optIn(with session: MXSession?) {
        SendingnNetworkSettings.shared.enableAnalytics = true
        startIfEnabled()
    ...
    func optOut() {
        SendingnNetworkSettings.shared.enableAnalytics = false
    ...
    func startIfEnabled() {
        guard SendingnNetworkSettings.shared.enableAnalytics, !isRunning else { return }
    ...
    func useAnalyticsSettings(from session: MXSession) {
        guard
            SendingnNetworkSettings.shared.enableAnalytics,
            !SendingnNetworkSettings.shared.isIdentifiedForAnalytics
        else { return }
    ...
    func reset() {
        ...
        SendingnNetworkSettings.shared.isIdentifiedForAnalytics = false
    ...
    private func identify(with settings: AnalyticsSettings) {
        ...
        SendingnNetworkSettings.shared.isIdentifiedForAnalytics = true
    }
...
extension Analytics {
    ...
    ///   - errorCode: The error code as returned by the node that triggered the logout.
    ///   - errorReason: The reason for the error as returned by the node that triggered the logout.
