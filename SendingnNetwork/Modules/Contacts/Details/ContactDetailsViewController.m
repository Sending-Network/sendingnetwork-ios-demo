...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // Report sdn session from AppDelegate
...
- (void)setContact:(MXKContact *)contact
{
    ...
    if (!_contact.isSDNContact)
    {
        // Refresh sdn info of the contact
        [[MXKContactManager sharedManager] updateSDNIDsForLocalContact:_contact];
    }
...
- (void)registerOnContactChangeNotifications
{
    ...
    mxPresenceObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kMXKContactManagerSDNUserPresenceChangeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *notif) {
        ...
        NSString* sdnId = self.firstSDNId;
        
        if (sdnId && [sdnId isEqualToString:notif.object])
        {
    ...
    if (_contact.isSDNContact)
    {
        // Observe 'MXKContactManager' notification on SDN contacts to refresh details.
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onContactManagerNotification:)  name:kMXKContactManagerDidUpdateSDNContactsNotification object:nil];
    }
    else
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onContactManagerNotification:)  name:kMXKContactManagerDidUpdateLocalContactSDNIDsNotification object:nil];
    }
}
...
- (NSString*)firstSDNId
{
    ...
    if (_contact.sdnIdentifiers.count > 0)
    {
        sdnId = _contact.sdnIdentifiers.firstObject;
    }
...
- (void)refreshContactThumbnail
{
    ...
    if (!image)
    {
        ...
        if (sdnId)
        {
            image = [AvatarGenerator generateAvatarForSDNItem:sdnId withDisplayName:_contact.displayName];
        }
        else
        {
...
- (void)refreshContactPresence
{
    ...
    if (sdnId)
    {
        ...
        for (MXSession *mxSession in mxSessions)
        {
            user = [mxSession userWithUserId:sdnId];
            if (user)
            {
                break;
            }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    if (sdnId.length)
    {
        ...
        if (![sdnId isEqualToString:self.mainSession.myUser.userId])
        {
            ...
            if (![self.mainSession isUserIgnored:sdnId])
            {
    ...
    // Else check whether the contact has been instantiated with an email or a sdn id
    else if ((!_contact.isSDNContact && _contact.emailAddresses.count) || [MXTools isEmailAddress:_contact.displayName])
    {
        directChatsIndex = sectionCount++;
    }
    else if ([MXTools isSDNUserIdentifier:_contact.displayName])
    {
        sdnId = _contact.displayName;
        directChatsIndex = sectionCount++;
    }
    ...
    if (sdnId.length)
    {
        ...
        NSArray *directRoomIds = self.mainSession.directRooms[sdnId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
    if (indexPath.section == directChatsIndex)
    {
        if (indexPath.row < directChatsArray.count)
        {
            ...
            [[AppDelegate theDelegate] showRoom:directChatsArray[indexPath.row] andEventId:nil withSDNSession:self.mainSession];
        }
        else
        {
    ...
    if ([sender isKindOfClass:[UIButton class]])
    {
        ...
        switch (button.tag)
        {
                ...
                [currentAlert addAction:[UIAlertAction actionWithTitle:[VectorL10n yes]
                                                               ...
                                                               handler:^(UIAlertAction * action) {
                                                                   ...
                                                                   if (weakSelf)
                                                                   {
                                                                       ...
                                                                       [self.mainSession ignoreUsers:@[self.firstSDNId]
                                                                                                   success:^{
                                                                                                       ...
                                                                                                       [self removePendingActionMask];
                                                                                                       MXLogDebug(@"[ContactDetailsViewController] Ignore %@ failed", self.firstSDNId);
            ...
            case ContactDetailsActionUnignore:
            {
                ...
                [self addPendingActionMask];
                __weak __typeof(self) weakSelf = self;
                [self.mainSession unIgnoreUsers:@[self.firstSDNId]
                                        success:^{
                                            ...
                                            __strong __typeof(weakSelf)self = weakSelf;
                                            [self removePendingActionMask];
                                            MXLogDebug(@"[ContactDetailsViewController] Unignore %@ failed", self.firstSDNId);
            ...
            case ContactDetailsActionStartChat:
            {
                ...
                if (_contact.sdnIdentifiers.count)
                {
                    [[AppDelegate theDelegate] showNewDirectChat:self.firstSDNId withSDNSession:self.mainSession completion:^{
                ...
                else
                {
                    ...
                    if ([MXTools isEmailAddress:participantId])
                    {
                        ...
                        if (!self.mainSession.sdnRestClient.identityServer)
                        {
                            [self removePendingActionMask];
                        ...
                        NSString *identityServer = self.mainSession.sdnRestClient.identityServer;
                    ...
                    [self.mainSession vc_canEnableE2EByDefaultInNewRoomWithUsers:inviteArray success:^(BOOL canEnableE2E) {
                        ...
                        self->roomCreationRequest = [self.mainSession createRoomWithParameters:roomCreationParameters success:^(MXRoom *room) {
                            ...
                            [[AppDelegate theDelegate] showRoom:room.roomId andEventId:nil withSDNSession:self.mainSession];
            ...
            case ContactDetailsActionStartVideoCall:
            {
                ...
                MXRoom* directRoom = [self.mainSession directJoinedRoomWithUserId:sdnId];
                ...
                {
                    // Create a new room
                    MXRoomCreationParameters *roomCreationParameters = [MXRoomCreationParameters parametersForDirectRoomWithUser:sdnId];
...
- (void)handleTapGesture:(UITapGestureRecognizer*)tapGestureRecognizer
{
    ...
    if (view == self.contactNameLabelMask && _contact.displayName)
    {
        ...
        {
            // Display contact's sdn id
            NSString *sdnId = self.firstSDNId;
            if (sdnId.length)
            {
                self.contactNameLabel.text = sdnId;
            }
        }
        else
        {
    ...
    else if (view == contactTitleView.memberAvatarMask || view == self.contactAvatarMask)
    {
        ...
        if (self.firstSDNId)
        {
            MXUser *user = [self.mainSession userWithUserId:self.firstSDNId];
            avatarURL = user.avatarUrl;
        }

        // TODO: Display the orignal contact avatar when the contast is not a SDN user
