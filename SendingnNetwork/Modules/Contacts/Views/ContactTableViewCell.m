...
- (void)setShowSDNIdInDisplayName:(BOOL)showSDNIdInDisplayName
{
    _showSDNIdInDisplayName = showSDNIdInDisplayName;
...
// returns the first sdn id of the contact
// nil if there is none
- (NSString*)firstSDNId
{
    NSString* sdnId = nil;
    
    if (contact.sdnIdentifiers.count > 0)
    {
        sdnId = contact.sdnIdentifiers.firstObject;
    }
...
- (void)refreshContactThumbnail
{
    ...
    if (!image)
    {
        ...
        {
            image = [AvatarGenerator generateAvatarForSDNItem:identifiers.firstObject withDisplayName:contact.displayName];
        }
        else if (contact.isThirdPartyInvite)
        {
            image = [AvatarGenerator generateAvatarForText:contact.displayName];
        }
        else if ((!contact.isSDNContact && contact.phoneNumbers.count && !contact.emailAddresses.count))
        {
...
- (void)refreshContactBadgeImage
{
    ...
    if (sdnId)
    {
        [self.mxRoom encryptionTrustLevelForUserId:sdnId onComplete:^(UserEncryptionTrustLevel userEncryptionTrustLevel) {
...
- (void)refreshContactDisplayName
{
    ...
    // Check whether the sdn identifier must be displayed.
    if (_showSDNIdInDisplayName)
    {
        // Append the sdn identifier to the display name.
        NSArray *identifiers = contact.sdnIdentifiers;
        if (identifiers.count)
        {
            NSString *userId = identifiers.firstObject;
            
            // Check whether the display name is not already the sdn id
            if (![contact.displayName isEqualToString:userId])
            {
                // Update the display name by adding the sdn id
