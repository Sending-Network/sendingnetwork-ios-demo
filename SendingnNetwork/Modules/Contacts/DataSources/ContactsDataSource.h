...
/**
 The state of the users search from the node user directory.
 */
typedef enum : NSUInteger
{
    ...
    // The search is based on local known sdn contacts
...
/**
 'ContactsDataSource' is a base class to handle contacts in SendingnNetwork.
 */
...
@protected
    ...
    NSInteger filteredSDNContactsSection;
    
    // Tell whether the non-sdn-enabled contacts must be hidden or not. NO by default.
    BOOL hideNonSDNEnabledContacts;
    
    // Search results
    NSString *currentSearchText;
    NSMutableArray<MXKContact*> *filteredLocalContacts;
    NSMutableArray<MXKContact*> *filteredSDNContacts;
}
...
/**
 Tell whether the sdn id should be added by default in the sdn contact display name (NO by default).
 If NO, the sdn id is added only to disambiguate the contact display names which appear several times.
 */
@property (nonatomic) BOOL forceSDNIdInDisplayName;
...
/**
 The dictionary of the ignored sdn contacts, the keys are their sdn identifier. Empty by default.
 */
@property (nonatomic) NSMutableDictionary<NSString*, MXKContact*> *ignoredContactsBySDNId;

/**
 Filter the contacts list, by keeping only the contacts who have the search pattern
 as prefix in their display name, their sdn identifiers and/or their contact methods (emails, phones).
...
/**
 ...
 a valid email or a SDN user ID.
 */
