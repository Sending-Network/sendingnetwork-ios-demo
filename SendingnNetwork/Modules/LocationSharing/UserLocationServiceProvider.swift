...
class UserLocationServiceProvider {
    ...
    private init() {
        ...
        self.registerSendingnNetworkSettingsNotifications()
    }
    ...
    private func setupUserLocationService(for userSession: UserSession) {
        ...
        let userLocationService = UserLocationService(session: userSession.sdnSession)
    ...
    private func setupUserLocationServiceIfNeeded(for userSession: UserSession) {
        ...
        // Be sure SDN session has is store setup to access beacon info summaries
        guard userSession.sdnSession.state.rawValue >= MXSessionState.storeDataReady.rawValue else {
            return
        }
    ...
    // MARK: - SendingnNetworkSettings
    
    private func registerSendingnNetworkSettingsNotifications() {
        
        NotificationCenter.default.addObserver(self, selector: #selector(SendingnNetworkSettingsDidUpdateLiveLocationSharingActivation(_:)), name: SendingnNetworkSettings.didUpdateLiveLocationSharingActivation, object: nil)
    }
    
    @objc private func SendingnNetworkSettingsDidUpdateLiveLocationSharingActivation(_ notification: Notification) {
