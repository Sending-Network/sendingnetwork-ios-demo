...
/// See https://github.com/sdn-org/prosody-mod-auth-sdn-user-verification
final class JitsiJWTTokenBuilder {
    ...
    func build(jitsiServerDomain: String,
        ...
        // Create Jitsi JWT
        let jitsiJWTPayloadContextSDN = JitsiJWTPayloadContextSDN(token: openIdToken.accessToken,
                                                                        roomId: roomId,
                                                                        serverName: openIdToken.sdnServerName)
        let jitsiJWTPayloadContextUser = JitsiJWTPayloadContextUser(avatar: userAvatarUrl, name: userDisplayName)
        let jitsiJWTPayloadContext = JitsiJWTPayloadContext(sdn: jitsiJWTPayloadContextSDN, user: jitsiJWTPayloadContextUser)
