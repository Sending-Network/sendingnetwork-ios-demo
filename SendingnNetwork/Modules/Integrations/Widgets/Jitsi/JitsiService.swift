...
final class JitsiService: NSObject {
    ...
    private func createJitsiWidgetContent(serverDomain: String,
        ...
        if let authenticationType = authenticationType, authenticationType == .openIDTokenJWT {
            ...
            // More details here:
            // https://github.com/sdn-org/prosody-mod-auth-sdn-user-verification
            conferenceID = Base32Coder.encodedString(roomID, padding: false)
            authenticationTypeString = authenticationType.identifier
        } else {
            let roomIdComponents = RoomIdComponents(sdnID: roomID)
        ...
        // Build widget url
        // SendingnNetwork-iOS does not directly use it but extracts params from it (see `[JitsiViewController openWidget:withVideo:]`)
        ...
        let v1queryStringParts = [
            ...
            "displayName=$sdn_display_name",
            "avatarUrl=$sdn_avatar_url",
            "email=$sdn_user_id"
        ]
