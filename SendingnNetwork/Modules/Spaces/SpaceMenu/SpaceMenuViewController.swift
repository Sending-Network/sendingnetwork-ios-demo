...
class SpaceMenuViewController: UIViewController {
    ...
    class func instantiate(forSpaceWithId spaceId: String, sdnSession: MXSession, viewModel: SpaceMenuViewModelType!) -> SpaceMenuViewController {
        let viewController = StoryboardScene.SpaceMenuViewController.initialScene.instantiate()
        viewController.session = sdnSession
    ...
    private func update(theme: Theme) {
        ...
        if self.spaceId == SpaceListViewModel.Constants.homeSpaceId {
            ...
            let avatarViewData = AvatarViewData(sdnItemId: self.spaceId, displayName: nil, avatarUrl: nil, mediaManager: session.mediaManager, fallbackImage: .image(defaultAsset.image, .center))
    ...
    private func setupViews() {
        ...
        let avatarViewData = AvatarViewData(sdnItemId: summary.roomId, displayName: summary.displayName, avatarUrl: summary.avatar, mediaManager: self.session.mediaManager, fallbackImage: .sdnItem(summary.roomId, summary.displayName))
