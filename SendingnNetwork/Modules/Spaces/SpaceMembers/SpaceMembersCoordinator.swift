...
struct SpaceMembersCoordinatorParameters {
    ...
    init(userSessionsService: UserSessionsService,
         session: MXSession,
         spaceId: String,
         navigationRouter: NavigationRouterType = NavigationRouter(navigationController: SendingnNetworkNavigationController())) {
...
final class SpaceMembersCoordinator: SpaceMembersCoordinatorType {
    ...
    private func navigateTo(roomWith roomId: String) {
        let roomDataSourceManager = MXKRoomDataSourceManager.sharedManager(forSDNSession: self.parameters.session)
