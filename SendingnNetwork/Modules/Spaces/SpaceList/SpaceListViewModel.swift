...
final class SpaceListViewModel: SpaceListViewModelType {
    ...
    func select(spaceWithId spaceId: String) {
        ...
        if let spaceService = self.userSessionsService.mainUserSession?.sdnSession.spaceService,
    ...
    @objc private func loadData() {
        guard let session = self.userSessionsService.mainUserSession?.sdnSession else {
    ...
    private func createHomeViewData(session: MXSession) -> SpaceListItemViewData {
        ...
        let avatarViewData = AvatarViewData(sdnItemId: Constants.homeSpaceId, displayName: nil, avatarUrl: nil, mediaManager: session.mediaManager, fallbackImage: .image(defaultAsset.image, .center))
    ...
    private func createAddSpaceViewData(session: MXSession) -> SpaceListItemViewData {
        ...
        let avatarViewData = AvatarViewData(sdnItemId: Constants.addSpaceId, displayName: nil, avatarUrl: nil, mediaManager: session.mediaManager, fallbackImage: .image(defaultAsset.image, .center))
    ...
    private func getSpacesViewData(session: MXSession) -> (invites: [SpaceListItemViewData], spaces: [SpaceListItemViewData]) {
        ...
        session.spaceService.rootSpaceSummaries.forEach { summary in
            let avatarViewData = AvatarViewData(sdnItemId: summary.roomId, displayName: summary.displayName, avatarUrl: summary.avatar, mediaManager: session.mediaManager, fallbackImage: .sdnItem(summary.roomId, summary.displayName))
