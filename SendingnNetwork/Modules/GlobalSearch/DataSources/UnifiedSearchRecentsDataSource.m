...
- (instancetype)initWithSDNSession:(MXSession *)mxSession recentsListService:(id<RecentsListServiceProtocol>)recentsListService
{
    self = [super initWithSDNSession:mxSession recentsListService:recentsListService];
    if (self)
    {
        _hideRecents = NO;
    }
...
- (RecentsDataSourceSections *)makeDataSourceSections
{
    ...
    if (SendingnNetworkSettings.shared.unifiedSearchScreenShowPublicDirectory)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (sectionType == RecentsDataSourceSectionTypeDirectory)
    {
        ...
        // skip the SDNKit mechanism and return directly the UITableViewCell
...
- (CGFloat)cellHeightAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        // For the cell showing the public rooms directory search result,
        // skip the SDNKit mechanism and return directly the cell height
...
- (void)searchWithPatterns:(NSArray *)patternsList
{
    ...
    if (patternsList.count == 1)
    {
        ...
        if ([MXTools isSDNRoomAlias:pattern] || [MXTools isSDNRoomIdentifier:pattern])
        {
