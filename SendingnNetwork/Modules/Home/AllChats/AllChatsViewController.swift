...
class AllChatsViewController: HomeViewController {
    ...
    override func removeSDNSession(_ mxSession: MXSession!) {
        super.removeSDNSession(mxSession)
        
        guard let dataSource = dataSource else { return }
        dataSource.removeSDNSession(mxSession)
    ...
    private func initDataSource() {
        ...
        let recentsDataSource = RecentsDataSource(sdnSession: mainSession, recentsListService: recentsListService)
    ...
    private func showLeaveSpace() {
        ...
        let selectionHeader = SDNItemChooserSelectionHeader(title: VectorL10n.leaveSpaceSelectionTitle,
                                                               selectAllTitle: VectorL10n.leaveSpaceSelectionAllRooms,
                                                               selectNoneTitle: VectorL10n.leaveSpaceSelectionNoRooms)
        let paramaters = SDNItemChooserCoordinatorParameters(session: session,
        ...
        let coordinator = SDNItemChooserCoordinator(parameters: paramaters)
...
extension AllChatsViewController: UISearchControllerDelegate {
    func willPresentSearchController(_ searchController: UISearchController) {
        // Fix for https://github.com/Sending-Network/sendingnetwork-ios/issues/6680
        self.recentsTableView.scrollToRow(at: IndexPath(row: 0, section: 0), at: .top, animated: true)
    }
}
...
extension AllChatsViewController: SplitViewMasterViewControllerProtocol {
    ...
    ///
    /// - Parameters:
    ///   - session: the sdn session.
    func presentVerifyCurrentSessionAlertIfNeeded(with session: MXSession) {
        guard !SendingnNetworkSettings.shared.hideVerifyThisSessionAlert,
        ...
        guard !session.vc_nodeConfiguration().encryption.isSecureBackupRequired else {
    ...
    /// Open the room with the provided identifier in a specific sdn session.
    ///
    /// - Parameters:
    ...
    private func presentVerifyCurrentSessionAlert(with session: MXSession) {
        ...
        alert.addAction(UIAlertAction(title: VectorL10n.doNotAskAgain,
                                      style: .destructive,
                                      handler: { action in
            SendingnNetworkSettings.shared.hideVerifyThisSessionAlert = true
        }))
    ...
    private func showSettingsSecurityScreen(with session: MXSession) {
        ...
        guard let securityViewController = SecurityViewController.instantiate(withSDNSession: session) else {
    ...
    private func resetReviewSessionsFlags() {
        SendingnNetworkSettings.shared.hideVerifyThisSessionAlert = false
    }
