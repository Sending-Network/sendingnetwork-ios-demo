...
class AllChatsCoordinator: NSObject, SplitViewMasterCoordinatorProtocol {
    ...
    func start(with spaceId: String?) {
        ...
        if self.hasStartedOnce == false {
            ...
            for userSession in self.parameters.userSessionsService.userSessions {
                self.addSDNSessionToAllChatsController(userSession.sdnSession)
            }
    ...
    @objc private func userSessionsServiceWillRemoveUserSession(_ notification: Notification) {
        ...
        self.removeSDNSessionFromAllChatsController(userSession.sdnSession)
    }
    
    // MARK: - SDN Session management
    
    // TODO: Remove SDN session handling from the view controller
    private func addSDNSessionToAllChatsController(_ sdnSession: MXSession) {
        MXLog.debug("[TabBarCoordinator] masterTabBarController.addSDNSession")
        self.allChatsViewController.addSDNSession(sdnSession)
    }
    
    // TODO: Remove SDN session handling from the view controller
    private func removeSDNSessionFromAllChatsController(_ sdnSession: MXSession) {
        MXLog.debug("[TabBarCoordinator] masterTabBarController.removeSDNSession")
        self.allChatsViewController.removeSDNSession(sdnSession)
    }
    ...
    private func updateAvatarButtonItem() {
        ...
        if let avatar = userAvatarViewData(from: currentSDNSession) {
    ...
    private func signOut() {
        guard let session = currentSDNSession else {
