...
NSString *FallBackViewControllerJavascriptOnRegistered = @"window.sdnRegistration.onRegistered = function(nodeUrl, userId, accessToken) {   \
    sendObjectMessage({                     \
        'action': 'onRegistered',           \
        'Node': nodeUrl,        \
        'userId': userId,                   \
...
NSString *FallBackViewControllerJavascriptOnLogin = @"window.sdnLogin.onLogin = function(response) {   \
    sendObjectMessage({             \
        'action': 'onLogin',        \
        'response': response        \
    });                            \
    ...
    if ([urlString hasPrefix:@"js:"])
    {
        ...
        if (!error)
        {
            if ([@"onRegistered" isEqualToString:parameters[@"action"]])
            {
                ...
                // Sanity check
                if (self.delegate
                    && loginResponse.node.length && loginResponse.userId.length && loginResponse.accessToken.length)
                {
