        ...
        [currentAlert addAction:[UIAlertAction actionWithTitle:[VectorL10n unknownDevicesVerify]
                                                       ...
                                                       handler:^(UIAlertAction * action) {
                                                           ...
                                                           if (weakSelf)
                                                           {
                                                               ...
                                                               [usersDevicesViewController displayUsersDevices:unknownDevices andSDNSession:self.mainSession onComplete:^(BOOL doneButtonPressed) {
                                                               ...
                                                               UINavigationController *usersDevicesNavigationController = [[SendingnNetworkNavigationController alloc] init];
...
- (void)checkStunServerFallbackWithCallState:(MXCallState)callState
{
    ...
    // in the app plist if the node does not provide STUN or TURN servers.
    ...
    if (!self.mainSession.callManager.turnServers
        ...
        && !SendingnNetworkSettings.shared.isAllowStunServerFallbackHasBeenSetOnce)
    {
        switch (callState)
        {
...
- (UIImage*)picturePlaceholder
{
    ...
    if (self.peer)
    {
        ...
        return [AvatarGenerator generateAvatarForSDNItem:self.peer.userId
    ...
    {
        return [AvatarGenerator generateAvatarForSDNItem:self.mxCall.room.roomId
...
- (NSURL*)audioURLWithName:(NSString*)soundName
{
    ...
    // Use by default the sdn kit sounds.
    if (!audioUrl)
    {
        ...
        if (self.delegate)
        {
            ...
            [self.delegate dismissCallViewController:self completion:^{
                ...
                if (self.mxCall.room)
                {
                    ...
                    [[AppDelegate theDelegate] showRoom:self.mxCall.room.roomId andEventId:nil withSDNSession:self.mxCall.room.mxSession];
                }
...
- (void)openCallTransfer
{
    ...
    controller.delegate = self;
    UINavigationController *navController = [[SendingnNetworkNavigationController alloc] initWithRootViewController:controller];
...
- (void)callTransferMainViewControllerDidComplete:(CallTransferMainViewController *)viewController consult:(BOOL)consult contact:(MXKContact *)contact phoneNumber:(NSString *)phoneNumber
{
    ...
    void(^continueBlock)(NSString *_Nonnull) = ^(NSString *targetUserId) {
        MXUserModel *targetUser = [[MXUserModel alloc] initWithUserId:targetUserId
                                                          displayname:contact.displayName
                                                            avatarUrl:contact.sdnAvatarURL];
    ...
    if (contact)
    {
        continueBlock(contact.sdnIdentifiers.firstObject);
    }
    else if (phoneNumber)
    {
        MXWeakify(self);
