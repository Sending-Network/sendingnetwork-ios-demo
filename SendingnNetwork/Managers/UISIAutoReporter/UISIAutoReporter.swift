...
@objcMembers class UISIAutoReporter: NSObject, UISIDetectorDelegate {
    ...
    private static let autoRsRequest = "im.vectSending.auto_rs_request"
    private static let reportSpacing = 60
    
    private let bugReporter: MXBugReportRestClient
    private let dispatchQueue = DispatchQueue(label: "io.sendingnetwork.UISIAutoReporter.queue")
    ...
    override init() {
        ...
        self.enabled = SendingnNetworkSettings.shared.enableUISIAutoReporting
        SendingnNetworkSettings.shared.publisher(for: SendingnNetworkSettings.UserDefaultsKeys.enableUISIAutoReporting)
            .sink {  [weak self] _ in
                guard let self = self else { return }
                self.enabled = SendingnNetworkSettings.shared.enableUISIAutoReporting
            }
    ...
    private func sendRageShake(source: UISIDetectedMessage) {
        ...
        self.bugReporter.vc_sendBugReport(
            ...
            success: { reportUrl in
                ...
                session.sdnRestClient.sendDirectToDevice(
                    payload: .init(
