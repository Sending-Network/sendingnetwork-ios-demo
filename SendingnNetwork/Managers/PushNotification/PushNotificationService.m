...
*/
@property (nonatomic, weak) id sdnSessionStateObserver;
...
{
    [session.sdnRestClient pushers:^(NSArray<MXPusher *> *pushers) {
...
- (void)setShouldReceiveVoIPPushes:(BOOL)shouldReceiveVoIPPushes
{
    ...
    if (_shouldReceiveVoIPPushes && _pushNotificationStore.pushKitToken)
    {
        ...
        else
        {
            ...
            self.sdnSessionStateObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kMXSessionStateDidChangeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *notif) {
                ...
                {
                    [self configurePushKit];
                    [notificationCenter removeObserver:self.sdnSessionStateObserver];
                }
            }];
        }
    }
...
- (void)removePusher:(MXPusher*)pusher inSession:(MXSession*)session
{
    ...
    // Shortcut SDNKit and its complex logic and call directly the API
    [session.sdnRestClient setPusherWithPushkey:pusher.pushkey
        ...
        // Brute clean remaining SDNKit data
...
{
    // Launch a background sync for all existing sdn sessions
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions))completionHandler
{
    ...
    if (SendingnNetworkSettings.shared.showInAppNotifications || userInfo[Constants.userInfoKeyPresentNotificationOnForeground])
    {
                                       ...
                                       failure:(void(^)(NSError *error))failure
{
    ...
    dispatch_group_notify(dispatchGroupSession, dispatch_get_main_queue(), ^{
        ...
        else
        {
            ...
            {
                dispatch_group_enter(dispatchGroupDataSource);
                MXKRoomDataSourceManager *manager = [MXKRoomDataSourceManager sharedManagerForSDNSession:mxSession];
...
- (void)pushRegistry:(PKPushRegistry *)registry didReceiveIncomingPushWithPayload:(PKPushPayload *)payload forType:(PKPushType)type withCompletionHandler:(void (^)(void))completion
{
    ...
    if (@available(iOS 13.0, *))
    {
        ...
        if (callInvite)
        {
            ...
            else if ([callInvite.type isEqualToString:kWidgetSDNEventTypeString] ||
