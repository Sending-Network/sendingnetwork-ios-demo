...
/// Store SendingnNetwork specific app settings.
@objcMembers
final class SendingnNetworkSettings: NSObject {
    ...
    static let shared = SendingnNetworkSettings()
    
    /// UserDefaults to be used on reads and writes.
    static var defaults: UserDefaults = {
        guard let userDefaults = UserDefaults(suiteName: BuildSettings.applicationGroupIdentifier) else {
            fatalError("[SendingnNetworkSettings] Fail to load shared UserDefaults")
        }
        return userDefaults
    }()
    ...
    var isUserDefaultsMigrated: Bool {
        return SendingnNetworkSettings.defaults.object(forKey: UserDefaultsKeys.notificationsShowDecryptedContent) != nil
    }
    ...
    func migrate() {
        ...
        for (key, value) in dictionary {
            SendingnNetworkSettings.defaults.set(value, forKey: key)
    ...
    @UserDefault(key: "nodeurl", defaultValue: BuildSettings.serverConfigDefaultNodeUrlString, storage: defaults)
    var nodeUrlString
    ...
    var hasSeenAnalyticsPrompt: Bool {
        SendingnNetworkSettings.defaults.object(forKey: UserDefaultsKeys.enableAnalytics) != nil
    }
    
    /// Whether the user has both seen the Matomo analytics prompt and declined it.
    var hasDeclinedMatomoAnalytics: Bool {
        SendingnNetworkSettings.defaults.object(forKey: UserDefaultsKeys.matomoAnalytics) != nil && !SendingnNetworkSettings.defaults.bool(forKey: UserDefaultsKeys.matomoAnalytics)
    }
    ...
    var hasAcceptedMatomoAnalytics: Bool {
        SendingnNetworkSettings.defaults.bool(forKey: UserDefaultsKeys.matomoAnalytics)
    }
    ...
    var enableLiveLocationSharing {
        didSet {
            NotificationCenter.default.post(name: SendingnNetworkSettings.didUpdateLiveLocationSharingActivation, object: self)
        }
    }
    ...
    /// Flag indicating if we are using rust-based `SDNCryptoSDK` instead of `SDNSDK`'s internal crypto module
    ...
    var isAllowStunServerFallbackHasBeenSetOnce: Bool {
        return SendingnNetworkSettings.defaults.object(forKey: UserDefaultsKeys.allowStunServerFallback) != nil
    }
    ...
    @UserDefault(key: "sdnApps", defaultValue: false, storage: defaults)
    var sdnApps
...
// MARK: - SendingnNetworkSettings notification constants
extension SendingnNetworkSettings {
    public static let didUpdateLiveLocationSharingActivation = Notification.Name("SendingnNetworkSettingsDidUpdateLiveLocationSharingActivation")
}
