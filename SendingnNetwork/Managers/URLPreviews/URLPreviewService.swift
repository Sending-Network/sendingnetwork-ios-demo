...
class URLPreviewService: NSObject {
    ...
    /// first checking the cache, and if necessary making a request to the node.
    ...
    ///   - session: The session to use to contact the node.
    ...
    func preview(for url: URL,
        ...
        // Otherwise make a request to the node to generate a preview
        session.sdnRestClient.preview(for: sanitizedURL, success: { previewResponse in
            MXLog.debug("[URLPreviewService] Cached preview not found. Requesting from node.")
    ...
    private func makePreviewData(from previewResponse: MXURLPreview,
        ...
        if let cachePath = MXMediaManager.cachePath(forSDNContentURI: imageURL, andType: previewResponse.imageType, inFolder: nil),
        ...
        // Otherwise download the image from the node, treating an error as a preview without an image.
        session.mediaManager.downloadMedia(fromSDNContentURI: imageURL, withType: previewResponse.imageType, inFolder: nil) { path in
    ...
    /// has been disabled on the node. If this is true, link detection will be disabled
    ...
    private func checkForDisabledAPI(in error: Error?) {
        // The error we're looking for is a generic 404 and not a sdn error.
        guard
        ...
        if response.statusCode == 404 {
            MXLog.debug("[URLPreviewService] Disabling link detection as node does not support URL previews.")
