...
class UserSessionsService: NSObject {
    ...
    func removeUserSession(relatedToSDNSession sdnSession: MXSession) {
        let foundUserSession = self.userSessions.first { (userSession) -> Bool in
            userSession.sdnSession == sdnSession
        }
    ...
    private func addUserSession(fromAccount account: MXKAccount, postNotification: Bool) -> Bool {
        ...
        guard let sdnSession = account.mxSession else {
            return false
        }
        
        let userSession = UserSession(account: account, sdnSession: sdnSession)
    ...
    private func canAddAccount(_ account: MXKAccount) -> Bool {
        ...
        guard let mxSession = account.mxSession else {
            MXLog.debug("[UserSessionsService] Cannot add a UserSession from a MXKAccount with nil SDN session")
            return false
        }
