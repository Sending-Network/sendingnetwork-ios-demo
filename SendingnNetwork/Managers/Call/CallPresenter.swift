...
class CallPresenter: NSObject {
    ...
    func addSDNSession(_ session: MXSession) {
        sessions.append(session)
    }
    
    func removeSDNSession(_ session: MXSession) {
    ...
    func processWidgetEvent(_ event: MXEvent, inSession session: MXSession) {
        ...
        guard let widget = Widget(widgetEvent: event, inSDNSession: session) else {
        ...
        if widget.isActive {
            ...
            if event.sender == session.myUserId {
                ...
                //  incoming call
                guard SendingnNetworkSettings.shared.enableRingingForGroupCalls else {
    ...
    private func logCallVC(_ callVC: UIViewController, log: String) {
        if let callVC = callVC as? CallViewController {
            MXLog.debug("[CallPresenter] \(log): SDN call: \(String(describing: callVC.mxCall?.callId))")
    ...
    private func addCallObservers() {
        ...
        widgetEventsListener = session.listenToEvents([
            MXEventType(identifier: kWidgetSDNEventTypeString),
    ...
    private func newCall(_ notification: Notification) {
        ...
        if UIApplication.shared.applicationState == .background && call.isIncoming {
            ...
            let callBackgroundTask = handler.startBackgroundTask(withName: "[CallPresenter] addSDNCallObserver", expirationHandler: nil)
    ...
    private func groupCallTileTapped(_ notification: Notification) {
        ...
        guard randomEvent.eventType == .custom,
                (randomEvent.type == kWidgetSDNEventTypeString ||
        ...
        guard let widget = Widget(widgetEvent: randomEvent, inSDNSession: session) else {
            return
        }
