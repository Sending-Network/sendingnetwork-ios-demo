...
class WidgetManagerConfig: NSObject, NSCoding {
    ...
    var baseUrl: NSString? {
        // Same comment as https://github.com/sdn-org/sdn-react-sdk/blob/1b0d8510a2ee93beddcd34c2d5770aa9fc76b1d9/src/ScalarAuthClient.js#L108
        // The terms endpoints are new and so live on standard _sdn prefixes,
        // but IM rest urls are currently configured with paths, so remove the
        // path from the base URL before passing it to the js-sdk

        // We continue to use the full URL for the calls done by
        // SendingnNetwork-iOS, but the standard terms API called
        // by the sdn-ios-sdk lives on the standard _sdn path. This means we
        // don't support running IMs on a non-root path, but it's the only
        // realistic way of transitioning to _sdn paths since configs in
        // the wild contain bits of the API path.

        // Once we've fully transitioned to _sdn URLs, we can give people
