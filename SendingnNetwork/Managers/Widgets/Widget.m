...
- (instancetype)initWithWidgetEvent:(MXEvent *)widgetEvent inSDNSession:(MXSession*)mxSession
{
    // TODO - Room widgets need to be moved to 'm.widget' state events
    // https://docs.google.com/document/d/1uPF7XWY_dXTKVKV7jZQ2KmsI19wn9-kFRgQ1tFQP7wQ/edit?usp=sharing
    if (![widgetEvent.type isEqualToString:kWidgetSDNEventTypeString]
        && ![widgetEvent.type isEqualToString:kWidgetModularEventTypeString])
    {
        // The Widget class works only with modular, aka "m.widget" or "im.vectSending.modular.widgets", widgets
        return nil;
    }
...
- (MXHTTPOperation *)widgetUrl:(void (^)(NSString * _Nonnull))success failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    widgetUrl = [widgetUrl stringByReplacingOccurrencesOfString:@"$sdn_user_id" withString:userId];
    widgetUrl = [widgetUrl stringByReplacingOccurrencesOfString:@"$sdn_display_name" withString:displayName];
    widgetUrl = [widgetUrl stringByReplacingOccurrencesOfString:@"$sdn_avatar_url" withString:avatarUrl];
    widgetUrl = [widgetUrl stringByReplacingOccurrencesOfString:@"$sdn_widget_id" withString:widgetId];
    
    if (self.roomId)
    {
        NSString *roomId = [MXTools encodeURIComponent:self.roomId];
        widgetUrl = [widgetUrl stringByReplacingOccurrencesOfString:@"$sdn_room_id" withString:roomId];
    }
