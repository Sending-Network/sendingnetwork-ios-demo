...
{
    // MXSession kind of hash -> Listener for sdn events for widgets.
    // There is one per sdn session
    ...
    // User id -> MXSession
    NSMutableDictionary<NSString*, MXSession*> *sdnSessions;
}
...
- (instancetype)init
{
    ...
    if (self)
    {
        sdnSessions = [NSMutableDictionary dictionary];
...
- (NSArray<Widget*> *)widgetsOfTypes:(NSArray<NSString*>*)widgetTypes butNotTypesOf:(NSArray<NSString*>*)notWidgetTypes inRoom:(MXRoom*)room withRoomState:(MXRoomState*)roomState;
{
    ...
    NSMutableArray<MXEvent*> *widgetEvents = [NSMutableArray arrayWithArray:[roomState stateEventsWithType:kWidgetSDNEventTypeString]];
    ...
    for (MXEvent *widgetEvent in widgetEvents)
    {
        ...
        {
            Widget *widget = [[Widget alloc] initWithWidgetEvent:widgetEvent inSDNSession:room.mxSession];
            if (widget)
            {
...
- (NSArray<Widget*> *)userWidgets:(MXSession*)mxSession ofTypes:(NSArray<NSString*>*)widgetTypes
{
    ...
    for (NSDictionary *widgetEventContent in [mxSession.accountData accountDataForEventType:kMXAccountDataTypeUserWidgets].allValues)
    {
        ...
        {
            Widget *widget = [[Widget alloc] initWithWidgetEvent:widgetEvent inSDNSession:mxSession];
            if (widget)
            {
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    [self checkWidgetPermissionInRoom:room success:^{
        ...
        // TODO: Move to kWidgetSDNEventTypeString ("m.widget") type but when?
                                     ...
                                     failure:(void (^)(NSError *error))failure
{
    ...
    SendingnNetworkSharedSettings *sharedSettings = [[SendingnNetworkSharedSettings alloc] initWithSession:room.mxSession];
...
- (void)addSDNSession:(MXSession *)mxSession
{
    ...
    sdnSessions[mxSession.sdnRestClient.credentials.userId] = mxSession;

    NSString *hash = [NSString stringWithFormat:@"%p", mxSession];

    id listener = [mxSession listenToEventsOfTypes:@[kWidgetSDNEventTypeString, kWidgetModularEventTypeString] onEvent:^(MXEvent *event, MXTimelineDirection direction, id customObject) {
        ...
        if (self && direction == MXTimelineDirectionForwards)
        {
            ...
            Widget *widget = [[Widget alloc] initWithWidgetEvent:event inSDNSession:mxSession];
            if (widget)
            {
...
- (void)removeSDNSession:(MXSession *)mxSession
{
    ...
    for (NSString *key in [sdnSessions allKeysForObject:mxSession])
    {
        [sdnSessions removeObjectForKey:key];
    }

    // mxSession.myUser.userId and mxSession.sdnRestClient.credentials.userId may be nil here
...
- (MXSession*)sdnSessionForUser:(NSString*)userId
{
    return sdnSessions[userId];
}
...
- (WidgetManagerConfig*)createWidgetManagerConfigForUser:(NSString*)userId
{
    ...
    MXSession *session = [self sdnSessionForUser:userId];

    // Find the integrations settings for the user

    // First, look at sdn account
    // TODO in another user story
    
    // Then, try to the node configuration
    MXWellknownIntegrationsManager *integrationsManager = session.nodeWellknown.integrations.managers.firstObject;
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    operation = [mxSession.sdnRestClient openIdToken:^(MXOpenIdToken *tokenObject) {
        MXStrongifyAndReturnIfNil(self);
...
- (void)loadConfigs
{
    ...
    else
    {
        ...
        if (configsData)
        {
            ...
            [NSKeyedUnarchiver setClass:WidgetManagerConfig.class forClassName:@"SendingnNetwork.WidgetManagerConfig"];
