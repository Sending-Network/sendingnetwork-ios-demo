...
/// `NodeConfigurationBuilder` build `NodeConfiguration` objects according to injected inputs
@objcMembers
final class NodeConfigurationBuilder: NSObject {
    ...
    /// Create an `NodeConfiguration` from an HS Well-Known when possible otherwise it takes hardcoded values from BuildSettings by default.
    func build(from wellKnown: MXWellKnown?) -> NodeConfiguration {
        ...
        let encryptionConfiguration = NodeEncryptionConfiguration(isE2EEByDefaultEnabled: isE2EEByDefaultEnabled,
        ...
        let tileServerConfiguration = NodeTileServerConfiguration(mapStyleURL: tileServerMapStyleURL)
        
        // Create NodeConfiguration
        
        let jitsiConfiguration = NodeJitsiConfiguration(serverDomain: jitsiPreferredDomain,
                                                              serverURL: jitsiServerURL)
                
        return NodeConfiguration(jitsi: jitsiConfiguration,
    ...
    private func getJitsiConfiguration(from vectorWellKnown: VectorWellKnown) -> VectorWellKnownJitsiConfiguration? {
        ...
        if let lastJitsiConfiguration = vectorWellKnown.jitsi {
            ...
            MXLog.debug("[NodeConfigurationBuilder] getJitsiConfiguration - Use deprecated configuration")
            jitsiConfiguration = deprecatedJitsiConfiguration
        } else {
            MXLog.debug("[NodeConfigurationBuilder] getJitsiConfiguration - No configuration found")
    ...
    private func getEncryptionConfiguration(from vectorWellKnown: VectorWellKnown) -> VectorWellKnownEncryptionConfiguration? {
        ...
        if let lastEncryptionConfiguration = vectorWellKnown.encryption {
            ...
            MXLog.debug("[NodeConfigurationBuilder] getEncryptionConfiguration - Use deprecated configuration")
            encryptionConfiguration = deprecatedEncryptionConfiguration
        } else {
            MXLog.debug("[NodeConfigurationBuilder] getEncryptionConfiguration - No configuration found")
    ...
    private func jitsiServerURL(from jitsiServerDomain: String) -> URL? {
        ...
        guard let jitsiServerURL = URL(string: jitsiStringURL) else {
            MXLog.debug("[NodeConfigurationBuilder] Jitsi server URL is not valid")
            return nil
        }
