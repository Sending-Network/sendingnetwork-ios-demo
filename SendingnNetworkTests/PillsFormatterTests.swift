...
private enum Inputs {
    ...
    static let aliceUserId = "@alice:sdn.org"
    static let aliceAvatarUrl = "mxc://sdn.org/VyNYAgahaiAzUoOeZETtQ"
    static let aliceAwayDisplayname = "Alice_away"
    static let aliceNewAvatarUrl = "mxc://sdn.org/VyNYAgaFdlLojoOeZETtQ"
    static let aliceMember = FakeMXRoomMember(displayname: aliceDisplayname, avatarUrl: aliceAvatarUrl, userId: aliceUserId)
    static let aliceMemberAway = FakeMXRoomMember(displayname: aliceAwayDisplayname, avatarUrl: aliceNewAvatarUrl, userId: "@alice:sdn.org")
    static let alicePermalink = "https://sdn.to/#/@alice:sdn.org"
    ...
    static let bobUserId = "@bob:sdn.org"
    static let bobDisplayname = "Bob"
    static let bobAvatarUrl = "mxc://sdn.org/VyNYBgahazAzUuOeZETtQ"
    static let bobMember = FakeMXRoomMember(displayname: bobDisplayname, avatarUrl: bobAvatarUrl, userId: bobUserId)
    static let bobPermalink = "https://sdn.to/#/@bob:sdn.org"
    static let markdownLinkToBob = "[\(bobDisplayname)](\(bobPermalink))"
    
    static let anotherUserId = "@another.user:sdn.org"
    static let anotherUserPermalink = "https://sdn.to/#/@another.user:sdn.org"
    ...
    static let roomId = "!vWieJcXcUdMwavNSvy:sdn.org"
    static let roomAlias = "#fake_room_alias:sdn.org"
    static let roomDisplayName = "Sample Room"
    static let roomPermalink = "https://sdn.to/#/\(roomId)"
    static let roomAliasPermalink = "https://sdn.to/%23/\(roomAlias)"
    static let roomAvatarUrl = "mxc://sdn.org/VzNZAgahaiAzUoOeZETtQ"
    ...
    static let anotherRoomId = "!zWieBcUcUdMwavNSvy:sdn.org"
    static let anotherRoomDisplayName = "Room/Space"
    static let anotherRoomAvatarUrl = "mxc://sdn.org/VzNZBgajauAzUoOeZETtQ"
    
    static let messageEventId = "$JrEsoQO77MCdAubG6z-5oXlOBy1I5QL9FTut_Giztoc"
    static let messagePermalink = "https://sdn.to/#/\(roomId)/\(messageEventId)?via=sdn.org"
    static let messageAnotherRoomPermalink = "https://sdn.to/#/\(anotherRoomId)/\(messageEventId)?via=sdn.org"
...
class PillsFormatterTests: XCTestCase {
    ...
    func testMarkdownLinkToUnknownUserIsNotPillified() {
        let message = "Hello [Unknown user](https://sdn.to/#/@unknown:sdn.org)"
        let messageWithPills = insertPillsInMarkdownString(message)
    ...
    func testBrokenMarkdownLinkIsNotDetected() {
        let brokenMarkdownMessages = [
            NSAttributedString(string: "Hello [Alice](https://sdn.to/#/@alice:sdn.org"),
            NSAttributedString(string: "Hello [Alice]https://sdn.to/#/@alice:sdn.org)"),
            NSAttributedString(string: "Hello [Alice(https://sdn.to/#/@alice:sdn.org)"),
            NSAttributedString(string: "Hello Alice](https://sdn.to/#/@alice:sdn.org)"),
            NSAttributedString(string: "Hello [Alice]](https://sdn.to/#/@alice:sdn.org)"),
            NSAttributedString(string: "Hello (https://sdn.to/#/@alice:sdn.org)"),
        ]
...
private extension PillsFormatterTests {
    func createMessageWithMentionFromBobToAlice() -> NSAttributedString {
        ...
        let messageWithPills = PillsFormatter.insertPills(in: formattedMessage,
                                                          withSession: session,
                                                          eventFormatter: EventFormatter(sdnSession: session),
    ...
    func createMessageWithMentionToRoom(isSpace: Bool = false, knownRoom: Bool = true, usingAlias: Bool = false, withLinkText: String? = nil) -> NSAttributedString {
        ...
        if knownRoom {
            let room = FakeMXRoom(roomId: Inputs.roomId, sdnSession: session, andStore: nil)!
    ...
    func createMessageWithMentionToAnotherRoomMessage(knownRoom: Bool, withLabel string: String) -> NSAttributedString {
        ...
        session.store = FakeMXStore(withEvents: [event])
        if knownRoom {
            let room = FakeMXRoom(roomId: Inputs.anotherRoomId, sdnSession: session, andStore: nil)!
...
private class FakeMXSession: MXSession {
    ...
    init(myUserId: String) {
        ...
        let credentials = MXCredentials(node: "mock_home_server",
                                        userId: "mock_user_id",
                                        accessToken: "mock_access_token")
        let client = MXRestClient(credentials: credentials)
        super.init(sdnRestClient: client)
    }
...
private class FakeMXRoom: MXRoom {
    ...
    override init!(roomId: String!, sdnSession mxSession: MXSession!, andStore store: MXStore!) {
        super.init(roomId: roomId, sdnSession: mxSession, andStore: store)
    }
...
private class FakeMXRoomSummary: MXRoomSummary {
    ...
    init(roomId: String, displayName: String, alias: String?, avatar: String?, sdnSession mxSession: MXSession) {
        super.init(roomId: roomId, andSDNSession: mxSession)
    ...
    override init!(roomId: String!, andSDNSession mxSession: MXSession!) {
        super.init(roomId: roomId, andSDNSession: mxSession)
    }
