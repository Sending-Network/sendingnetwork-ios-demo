...
class MXRoomAliasResolutionDeeplinkTests: XCTestCase {
    ...
    func test_fragmentDoesNotContainServers_ifNoServers() {
        let resolution = MXRoomAliasResolution()
        resolution.roomId = "!abc:sdn.org"
        
        XCTAssertEqual(resolution.deeplinkFragment, "!abc:sdn.org")
    }
    
    func test_fragmentContainsSingleServer() {
        let resolution = MXRoomAliasResolution()
        resolution.roomId = "xyz:sendingnetwork.io"
        resolution.servers = [
            "sdn.org"
        ]
        
        XCTAssertEqual(resolution.deeplinkFragment, "xyz:sendingnetwork.io?via=sdn.org")
    }
    ...
    func test_fragmentContainsMultipleSerivers() {
        ...
        XCTAssertEqual(resolution.deeplinkFragment, "mno:server.com?via=server.com&via=sendingnetwork.io&via=wikipedia.org&via=sdn.org")
    }
}
