...
@MainActor class AuthenticationServiceTests: XCTestCase {
    ...
    func testWizardsWhenStartingLoginFlow() async throws {
        ...
        try await service.startFlow(.login, for: "https://sdn.org")
    ...
    func testWizardsWhenStartingRegistrationFlow() async throws {
        ...
        XCTAssertNil(service.state.node.registrationFlow, "A new service shouldn't provide a registration flow for the node.")
        
        // When starting a new registration flow.
        try await service.startFlow(.register, for: "https://sdn.org")
        ...
        XCTAssertNotNil(service.state.node.registrationFlow, "The supported registration flow should be stored after starting a registration flow.")
    }
    ...
    func testReset() async throws {
        ...
        XCTAssertEqual(service.state.node.address, "https://sdn.example.com", "The actual node address should be discovered.")
        XCTAssertEqual(service.state.node.addressFromUser, "https://example.com", "The address from the startFlow call should be stored.")
        ...
        XCTAssertNil(service.state.node.registrationFlow, "The supported registration flow should be cleared when calling reset.")
        ...
        XCTAssertEqual(service.state.node.address, "https://example.com", "The address should reset to the value entered by the user.")
    }
    ...
    func testResetDefaultServer() async throws {
        ...
        XCTAssertEqual(service.state.node.address, BuildSettings.serverConfigDefaultNodeUrlString,
    ...
    func testProvisioningLink() async throws {
        ...
        let nodeURL = "https://example.com"
        let provisioningLink = URL(string: "app.sendingnetwork.io/register/?hs_url=\(nodeURL)")!
        ...
        XCTAssertEqual(universalLink.nodeUrl, nodeURL)
        XCTAssertNotNil(service.provisioningLink, "The provisioning link should be stored in the service.")
        XCTAssertEqual(service.provisioningLink?.nodeUrl, nodeURL, "The provisioning link's node should not change.")
        XCTAssertEqual(service.state.node.address, "https://sdn.example.com", "The actual node address should be discovered.")
        XCTAssertEqual(service.state.node.addressFromUser, nodeURL, "The address from the provisioning link should be stored.")
        ...
        XCTAssertEqual(service.provisioningLink?.nodeUrl, nodeURL, "The provisioning link's node should not change.")
        XCTAssertEqual(service.state.node.address, nodeURL, "The address from the provisioning link should be stored.")
        XCTAssertNil(service.state.node.addressFromUser, "There shouldn't be an address from the user after resetting the service.")
    ...
    func testNodeState() async throws {
        // Given a service that has begun login for one node.
        try await service.startFlow(.login, for: "https://example.com")
        XCTAssertEqual(service.state.node.addressFromUser, "https://example.com", "The initial address entered by the user should be stored.")
        XCTAssertEqual(service.state.node.address, "https://sdn.example.com", "The initial address discovered from the well-known should be stored.")
        
        // When switching to a different node
        try await service.startFlow(.login, for: "https://sdn.org")
        
        // The the node state should update to represent the new server
        XCTAssertEqual(service.state.node.addressFromUser, "https://sdn.org", "The new address entered by the user should be stored.")
        XCTAssertEqual(service.state.node.address, "https://sdn-client.sdn.org", "The new address discovered from the well-known should be stored.")
    }
    
    func testStartingLoginWithInvalidURL() async throws {
        // Given a service that has started the register flow for one node.
        try await service.startFlow(.login, for: "https://example.com")
        XCTAssertEqual(service.client.node, "https://sdn.example.com", "The client should be set up for the node")
        ...
        do {
            ...
            XCTFail("The registration flow should fail for an incorrect node address.")
        } catch {
    ...
    func testStartingRegistrationForLoginOnlyServer() async throws {
        ...
        // When failing to start registration for another node that only supports login.
        do {
        ...
        XCTAssertEqual(service.client.node, "https://sdn.example.com", "The client should still be set up for the node")
        XCTAssertEqual(service.state.flow, .register, "The flow should still be set as registration.")
        XCTAssertEqual(service.state.node.addressFromUser, "https://example.com", "The initial address entered by the user should still be stored.")
        XCTAssertEqual(service.state.node.address, "https://sdn.example.com", "The initial address discovered from the well-known should still be stored.")
    }
    ...
    func testPasswordLogin() async throws {
        ...
        XCTAssertEqual(session.myUserId, "@alice:sdn.org")
    }
    ...
    func testNodeViewDataForSDNDotOrg() {
        // Given a node such as sdn.org.
        let address = "https://sdn-client.sdn.org"
        let addressFromUser = "https://sdn.org" // https is added when sanitising the input.
        ...
        let node = AuthenticationState.Node(address: address,
        ...
        // When creating view data for that node.
        let viewData = node.viewData
        
        // Then the view data should correctly represent the node.
        XCTAssertEqual(viewData.address, "sdn.org", "The displayed address should match the address supplied by the user, but without the scheme.")
    ...
    func testNodeViewDataForPasswordLoginOnly() {
        // Given a node with password login and registration disabled.
        let address = "https://sdn.example.com"
    ...
    func testNodeViewDataForSSOOnly() {
        // Given a node that only supports authentication via SSO.
        let address = "https://sdn.company.com"
    ...
    func testNodeViewDataForLocalNode() {
        // Given a local node that supports login and registration but only via a password.
        ...
        let node = AuthenticationState.Node(address: addressFromUser,
                                                        addressFromUser: addressFromUser,
                                                        preferredLoginMode: .password,
                                                        registrationFlow: .flowResponse(flowResult))
    ...
    func testNodeAddressSanitization() {
        let basicAddress = "sdn.org"
        let httpAddress = "http://localhost"
        let trailingSlashAddress = "https://sdn.example.com/"
        let whitespaceAddress = " https://sdn.example.com/  "
        let validAddress = "https://sdn.example.com"
        let validAddressWithPort = "https://sdn.example.com:8484"
        
        let sanitizedBasicAddress = NodeAddress.sanitized(basicAddress)
        let sanitizedHTTPAddress = NodeAddress.sanitized(httpAddress)
        let sanitizedTrailingSlashAddress = NodeAddress.sanitized(trailingSlashAddress)
        let sanitizedWhitespaceAddress = NodeAddress.sanitized(whitespaceAddress)
        let sanitizedValidAddress = NodeAddress.sanitized(validAddress)
        let sanitizedValidAddressWithPort = NodeAddress.sanitized(validAddressWithPort)
        
        XCTAssertEqual(sanitizedBasicAddress, "https://sdn.org")
        XCTAssertEqual(sanitizedHTTPAddress, "http://localhost")
        XCTAssertEqual(sanitizedTrailingSlashAddress, "https://sdn.example.com")
        XCTAssertEqual(sanitizedWhitespaceAddress, "https://sdn.example.com")
