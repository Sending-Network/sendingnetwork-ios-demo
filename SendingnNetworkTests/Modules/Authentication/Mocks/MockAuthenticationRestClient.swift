...
class MockAuthenticationRestClient: AuthenticationRestClient {
    ...
    enum MockError: Error {
        ...
        /// The node doesn't allow for registration.
    ...
    var config: Config
    /// The node's URL string.
    var node: String! { nodeURL.absoluteString }
    ...
    /// - Parameters:
    ///   - Node: See `MockAuthenticationRestClient.Config` for various URLs that can be used.
    ///   - handler: Unused.
    required init(node: URL, unrecognizedCertificateHandler handler: MXHTTPClientOnUnrecognizedCertificate?) {
        self.config = Config(url: Node)
    ...
    var loginFallbackURL: URL {
        nodeURL.appendingPathComponent("_api/static/client/login")
    }
    ...
    var registerFallbackURL: URL {
        nodeURL.appendingPathComponent("_api/static/client/register")
    }
    ...
    func supportedSDNVersions() async throws -> MXSDNVersions {
        return MXSDNVersions()
    }
}
