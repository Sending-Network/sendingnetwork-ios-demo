...
class NotificationService: UNNotificationServiceExtension {
    ...
    private var showDecryptedContentInNotifications: Bool {
        return SendingnNetworkSettings.shared.showDecryptedContentInNotifications
    }
    ...
    private static let backgroundServiceInitQueue = DispatchQueue(label: "io.sendingnetwork.NotificationService.backgroundServiceInitQueue")
    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        //  check if this is a SDN notification
        guard let roomId = userInfo["room_id"] as? String, let eventId = userInfo["event_id"] as? String else {
            //  it's not a SDN notification, do not change the content
            MXLog.debug("[NotificationService] didReceiveRequest: This is not a SDN notification.")
            ...
            switch response {
                case .success(let (roomState, eventSenderName)):
                    ...
                    switch event.eventType {
                        ...
                        case .custom:
                            if (event.type == kWidgetSDNEventTypeString || event.type == kWidgetModularEventTypeString),
                               ...
                               (type == kWidgetTypeJitsiV1 || type == kWidgetTypeJitsiV2) {
                                ...
                                if SendingnNetworkSettings.shared.enableRingingForGroupCalls {
                                    self.sendVoipPush(forEvent: event)
                                } else {
        ...
        NotificationService.backgroundSyncService.roomState(forRoomId: roomId) { (response) in
            switch response {
                case .success(let roomState):
                    ...
                    // Else, if the room member is not known, use the user profile to avoid to display a SDN id
    ...
    private static func localizedString(forKey key: String, _ args: CVarArg...) -> String {
        ...
        // SDNKit now sets the app bundle as the MXKLanguageBundle
