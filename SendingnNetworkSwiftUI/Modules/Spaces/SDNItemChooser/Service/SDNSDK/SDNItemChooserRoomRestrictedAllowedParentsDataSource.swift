...
class SDNItemChooserRoomRestrictedAllowedParentsDataSource: SDNItemChooserDataSource {
    ...
    func sections(with session: MXSession, completion: @escaping (Result<[SDNListItemSectionData], Error>) -> Void) {
        ...
        room.state { [weak self] state in
            ...
            if !unknownParents.isEmpty {
                sections.append(SDNListItemSectionData(
                    ...
                    items: unknownParents.compactMap { roomId in
                        SDNListItemData(
                            id: roomId,
                            type: .space,
                            avatar: AvatarInput(mxContentUri: roomId, sdnItemId: roomId, displayName: roomId),
                            displayName: roomId,
                            detailText: nil
                        )
                    }
