...
protocol SDNItemChooserDataSource {
    func sections(with session: MXSession, completion: @escaping (Result<[SDNListItemSectionData], Error>) -> Void)
    var preselectedItemIds: Set<String>? { get }
}

protocol SDNItemChooserProcessorProtocol {
    var loadingText: String? { get }
    var dataSource: SDNItemChooserDataSource { get }
...
class SDNItemChooserService: SDNItemChooserServiceProtocol {
    // MARK: - Properties
    
    // MARK: Private
    
    private let processingQueue = DispatchQueue(label: "io.sendingnetwork.SDNItemChooserService.processingQueue")
    private let completionQueue = DispatchQueue.main

    private let session: MXSession
    private var sections: [SDNListItemSectionData] = []
    private var filteredSections: [SDNListItemSectionData] = [] {
        didSet {
    ...
    private let itemsProcessor: SDNItemChooserProcessorProtocol
    
    // MARK: Public
    
    private(set) var sectionsSubject: CurrentValueSubject<[SDNListItemSectionData], Never>
    ...
    init(session: MXSession, selectedItemIds: [String], itemsProcessor: SDNItemChooserProcessorProtocol) {
    ...
    private func filter(sections: [SDNListItemSectionData]) -> [SDNListItemSectionData] {
        var newSections: [SDNListItemSectionData] = []
        ...
        for section in sections {
            ...
            newSections.append(SDNListItemSectionData(id: section.id, title: section.title, infoText: section.infoText, items: items))
        }
