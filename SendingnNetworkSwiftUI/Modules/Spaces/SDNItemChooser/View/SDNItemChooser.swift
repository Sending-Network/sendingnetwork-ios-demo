...
// $ createScreen.sh Spaces/SpaceCreation/SpaceCreationSDNItemChooser SpaceCreationSDNItemChooser
//
...
struct SDNItemChooser: View {
    // MARK: Properties
    
    @ObservedObject var viewModel: SDNItemChooserViewModel.Context
            ...
            LazyVStack(spacing: 0) {
                ForEach(viewModel.viewState.sections) { section in
                    if section.title != nil || section.infoText != nil {
                        SDNItemChooserSectionHeader(title: section.title, infoText: section.infoText)
                    }
    ...
    private func itemSelectionHeader(with selectionHeader: SDNItemChooserSelectionHeader) -> some View {
...
struct SDNItemChooser_Previews: PreviewProvider {
    static let stateRenderer = MockSDNItemChooserScreenState.stateRenderer
