...
class SpaceCreationPostProcessService: SpaceCreationPostProcessServiceProtocol {
    ...
    private var processingQueue = DispatchQueue(label: "io.sendingnetwork.MXSpace.processingQueue", attributes: .concurrent)

    private lazy var stateEventBuilder = MXRoomInitialStateEventBuilder()

    private lazy var mediaUploader: MXMediaLoader = MXMediaManager.prepareUploader(withSDNSession: session, initialRange: 0, andRange: 1.0)
    ...
    var avatar: AvatarInput {
        ...
        return AvatarInput(mxContentUri: alias, sdnItemId: "", displayName: creationParams.name)
    }
    ...
    private func addToSpace(room: MXRoom) {
        ...
        createdSpace.addChild(roomId: room.sdnItemId, completion: { response in
