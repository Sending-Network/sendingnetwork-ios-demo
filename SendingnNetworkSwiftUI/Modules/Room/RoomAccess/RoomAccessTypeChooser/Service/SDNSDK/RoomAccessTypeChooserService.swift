...
class RoomAccessTypeChooserService: RoomAccessTypeChooserServiceProtocol {
    ...
    init(roomId: String, allowsRoomUpgrade: Bool, session: MXSession) {
        ...
        versionOverride = session.nodeCapabilitiesService.versionOverrideForFeature(.restricted)
    ...
    private func readRoomState() {
        ...
        room.state { [weak self] state in
            ...
            if let roomVersion = state?.stateEvents(with: .roomCreate)?.last?.wireContent["room_version"] as? String, let nodeCapabilitiesService = self.session.nodeCapabilitiesService {
                self.roomUpgradeRequired = self.versionOverride != nil && !nodeCapabilitiesService.isFeatureSupported(.restricted, by: roomVersion)
            }
