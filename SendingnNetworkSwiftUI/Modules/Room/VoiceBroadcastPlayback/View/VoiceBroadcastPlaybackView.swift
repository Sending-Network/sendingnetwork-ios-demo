...
// TODO: To remove
// VoiceBroadcastPlaybackViewModel must be revisited in order to not depend on SDNSDK
#if canImport(SDNSDK)
        ...
        VStack(alignment: .center) {
            ...
            HStack (alignment: .top) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(details.avatarData.displayName ?? details.avatarData.sdnItemId)
                        .font(theme.fonts.bodySB)
                        .foregroundColor(theme.colors.primaryContent)
                    Label {
                        Text(details.senderDisplayName ?? details.avatarData.sdnItemId)
