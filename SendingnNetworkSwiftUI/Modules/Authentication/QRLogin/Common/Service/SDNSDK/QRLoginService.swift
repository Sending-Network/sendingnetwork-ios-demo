...
class QRLoginService: NSObject, QRLoginServiceProtocol {
    ...
    func isServiceAvailable() async throws -> Bool {
        ...
        return try await client.supportedSDNVersions().supportsQRLogin
    }
    ...
    private func processQRLoginCode(_ code: QRLoginCode) async {
        ...
        guard code.rendezvous.transport?.type == "org.sdn.msc3886.http.v1",
        ...
        guard case let .success(data) = await rendezvousService.receive(),
              ...
              let node = responsePayload.node,
              let nodeURL  = URL(string: node) else {
        ...
        // Use a custom rest client linked to the existing device's node
        let authenticationRestClient = MXRestClient(node: nodeURL, unrecognizedCertificateHandler: nil)
