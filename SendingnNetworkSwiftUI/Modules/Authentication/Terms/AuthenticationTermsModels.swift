...
struct AuthenticationTermsViewState: BindableState {
    /// The node asking the user to accept the terms.
    let node: AuthenticationNodeViewData
    /// View state that can be bound to from SwiftUI.
    var bindings: AuthenticationTermsBindings
    
    var headerMessage: String {
        VectorL10n.authenticationTermsMessage(node.address)
    }
...
enum AuthenticationTermsErrorType: Hashable {
    ...
    case mxError(String)
    /// The node supplied an invalid URL for the policy.
