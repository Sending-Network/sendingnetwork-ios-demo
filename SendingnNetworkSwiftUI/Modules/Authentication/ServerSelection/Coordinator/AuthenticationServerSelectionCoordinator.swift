...
final class AuthenticationServerSelectionCoordinator: Coordinator, Presentable {
    ...
    @MainActor init(parameters: AuthenticationServerSelectionCoordinatorParameters) {
        ...
        if BuildSettings.forceNodeSelection, node.addressFromUser == nil {
            nodeAddress = ""
        } else {
            nodeAddress = node.displayableAddress
        }
        let viewModel = AuthenticationServerSelectionViewModel(nodeAddress: nodeAddress,
    ...
    @MainActor private func setupViewModel() {
        authenticationServerSelectionViewModel.callback = { [weak self] result in
            ...
            switch result {
            case .confirm(let nodeAddress):
                self.useNode(nodeAddress)
            case .dismiss:
    ...
    /// Updates the login flow using the supplied node address, or shows an error when this isn't possible.
    @MainActor private func useNode(_ nodeAddress: String) {
        startLoading()
        ...
        Task {
            do {
                try await authenticationService.startFlow(parameters.flow, for: nodeAddress)
                stopLoading()
