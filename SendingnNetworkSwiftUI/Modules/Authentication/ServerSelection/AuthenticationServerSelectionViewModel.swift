...
class AuthenticationServerSelectionViewModel: AuthenticationServerSelectionViewModelType, AuthenticationServerSelectionViewModelProtocol {
    ...
    init(nodeAddress: String, flow: AuthenticationFlow, hasModalPresentation: Bool) {
        let bindings = AuthenticationServerSelectionBindings(nodeAddress: nodeAddress)
    ...
    override func process(viewAction: AuthenticationServerSelectionViewAction) {
        switch viewAction {
        case .confirm:
            Task { await callback?(.confirm(nodeAddress: state.bindings.nodeAddress)) }
        case .dismiss:
