...
@MainActor class AuthenticationRegistrationViewModelTests: XCTestCase {
    let defaultNode = AuthenticationNodeViewData.mockSDNDotOrg
    ...
    @MainActor override func setUp() async throws {
        viewModel = AuthenticationRegistrationViewModel(node: defaultNode)
        context = viewModel.context
    }
    
    func testSDNDotOrg() {
        // Given the initial view model configured for sdn.org with some SSO providers.
        let node = defaultNode
        
        // Then the view state should contain a node that matches sdn.org and shows SSO buttons.
        XCTAssertEqual(context.viewState.node, node, "The node data should match the original.")
    ...
    func testBasicServer() {
        ...
        let node = AuthenticationNodeViewData.mockBasicServer
        
        // When updating the view model with the server.
        viewModel.update(node: node)
        
        // Then the view state should be updated with the node and hide the SSO buttons.
        XCTAssertEqual(context.viewState.node, node, "The node data should should match the new node.")
        XCTAssertFalse(context.viewState.showSSOButtons, "The SSO buttons should not be shown.")
    }

    func testFallbackServer() {
        // Given a basic server example.com that only supports password registration.
        let node = AuthenticationNodeViewData.mockFallback

        // When updating the view model with the server.
        viewModel.update(node: node)

        // Then the view state should be updated with the node and hide the SSO buttons and registration form.
        XCTAssertFalse(context.viewState.node.showRegistrationForm, "The registration form should not be shown.")
        XCTAssertFalse(context.viewState.showSSOButtons, "The SSO buttons should not be shown.")
    }
    ...
    func testUsernameAvailability() async throws {
        ...
        XCTAssertEqual(context.viewState.usernameFooterMessage, VectorL10n.authenticationRegistrationUsernameFooterAvailable("@bob:sdn.org"),
    ...
    @MainActor func testLoadingServer() {
        ...
        // When updating the view model whilst loading a node.
        viewModel.update(isLoading: true)
        
        // Then the view state should reflect that the node is loading.
        ...
        // When updating the view model after loading a node.
        viewModel.update(isLoading: false)
        
        // Then the view state should reflect that the node is now loaded.
