...
enum MockAuthenticationRegistrationScreenState: MockScreenState, CaseIterable {
    ...
    var screenView: ([Any], AnyView) {
        ...
        switch self {
        case .sdnDotOrg:
            viewModel = AuthenticationRegistrationViewModel(node: .mockSDNDotOrg)
        case .passwordOnly:
            viewModel = AuthenticationRegistrationViewModel(node: .mockBasicServer)
        case .passwordWithCredentials:
            viewModel = AuthenticationRegistrationViewModel(node: .mockBasicServer)
        ...
        case .ssoOnly:
            viewModel = AuthenticationRegistrationViewModel(node: .mockEnterpriseSSO)
        case .fallback:
            viewModel = AuthenticationRegistrationViewModel(node: .mockFallback)
        }
