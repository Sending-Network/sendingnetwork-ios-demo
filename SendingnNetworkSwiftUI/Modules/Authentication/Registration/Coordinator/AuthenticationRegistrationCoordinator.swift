...
final class AuthenticationRegistrationCoordinator: Coordinator, Presentable {
    ...
    @MainActor init(parameters: AuthenticationRegistrationCoordinatorParameters) {
        ...
        let node = parameters.authenticationService.state.node
        let viewModel = AuthenticationRegistrationViewModel(node: node.viewData)
    ...
    /// Updates the node if a full MXID is entered, then requests whether the username is valid and available.
    @MainActor private func validateUsername(_ username: String) {
        guard MXTools.isSDNUserIdentifier(username) else {
            // Continue with availability check for a normal username.
            confirmAvailability(of: username)
            return
        }
        
        // Otherwise split out the domain and username and update the node first.
        let components = username.dropFirst().components(separatedBy: ":")
        let domain = components[1]
        let username = components[0]
        let nodeAddress = NodeAddress.sanitized(domain)
        
        startLoading(isInteractionBlocking: false)
        
        currentTask = Task { [weak self] in
            do {
                try await authenticationService.startFlow(.register, for: nodeAddress)
                
                guard !Task.isCancelled else { return }
                
                self?.updateViewModelNode()
    ...
    /// Asks the node to check the supplied username's format and availability.
    ...
    /// Creates an account on the node with the supplied username and password.
    ...
    @MainActor private func handleError(_ error: Error) {
        ...
        if let authenticationError = error as? AuthenticationError {
            ...
            case .invalidNode:
                authenticationRegistrationViewModel.displayError(.invalidNode)
    ...
    @MainActor private func updateViewModelNode() {
        let node = authenticationService.state.node
        authenticationRegistrationViewModel.update(node: node.viewData)
    }
}
