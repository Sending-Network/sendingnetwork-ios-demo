...
struct RegistrationThreePIDTokenResponse {
    ...
    /// Service API's POST /validate/email/submitToken endpoint. The node must send this token to the user (if applicable),
    ...
    /// the node advertises this specification version in the /versions response (ie: r0.5.0).
