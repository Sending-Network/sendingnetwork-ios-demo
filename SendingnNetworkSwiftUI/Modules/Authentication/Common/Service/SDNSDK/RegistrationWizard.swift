...
/// Set of methods to be able to create an account on a node.
///
...
///
/// More documentation can be found in the file https://github.com/Sending-Network/element-android/blob/main/docs/signup.md
/// and https://sdn.org/docs/spec/client_server/latest#account-registration-and-management
class RegistrationWizard {
    ...
    /// True when login and password have been sent with success to the node,
    ...
    /// Call this method to get the possible registration flow of the current node.
    ...
    /// Can be call to check is the desired username is available for registration on the current node.
    /// It may also fails if the desired username is not correctly formatted or does not follow any restriction on
    /// the node. Ex: username with only digits may be rejected.
    ...
    ///
    /// - Parameter threePID: the threePID to add to the account. If this is an email, the node will send an email
    ...
    /// Ask the node to send again the current threePID (email or msisdn).
    ...
    /// Useful to poll the node when waiting for the email to be validated by the user.
