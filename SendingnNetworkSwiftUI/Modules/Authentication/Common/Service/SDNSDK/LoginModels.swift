...
/// The result returned when querying a node's available login flows.
...
/// The supported forms of login that a node allows.
enum LoginMode {
    /// The login mode hasn't been determined yet.
    case unknown
    /// The node supports login with a password.
    case password
    /// The node supports login via one or more SSO providers.
    case sso(ssoIdentityProviders: [SSOIdentityProvider])
    /// The node supports login with either a password or via an SSO provider.
    case ssoAndPassword(ssoIdentityProviders: [SSOIdentityProvider])
    /// The node only allows login with unsupported mechanisms. Use fallback instead.
    case unsupported
    
    var ssoIdentityProviders: [SSOIdentityProvider]? {
        switch self {
        case .sso(let ssoIdentityProviders), .ssoAndPassword(let ssoIdentityProviders):
            // Provide a backup for nodes that support SSO but don't offer any identity providers
            // https://spec.sdn.org/latest/client-server-api/#client-login-via-sso
