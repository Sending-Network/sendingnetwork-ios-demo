...
final class AuthenticationLoginCoordinator: Coordinator, Presentable {
    ...
    @MainActor init(parameters: AuthenticationLoginCoordinatorParameters) {
        ...
        let viewModel = AuthenticationLoginViewModel(node: node.viewData)
    ...
    @MainActor private func loginClick() {
        let myNotificationName = Notification.Name("NotificationLogin")
        NotificationCenter.default.post(name: myNotificationName, object: self, userInfo: ["key": "value"])
        

//        PrintLogger("private - loginClick");
    }
    ...
    @MainActor private func handleError(_ error: Error) {
        ...
        if let authenticationError = error as? AuthenticationError {
            ...
            case .invalidNode:
                authenticationLoginViewModel.displayError(.invalidNode)
    ...
    @MainActor private func parseUsername(_ username: String) {
        guard MXTools.isSDNUserIdentifier(username) else { return }
    ...
    @MainActor private func showForgotPasswordScreen() {
        ...
        let parameters = AuthenticationForgotPasswordCoordinatorParameters(navigationRouter: modalRouter,
                                                                           ...
                                                                           node: parameters.authenticationService.state.node)
    ...
    /// Updates the view model to reflect any changes made to the node.
    @MainActor private func updateViewModel() {
        let node = authenticationService.state.node
        authenticationLoginViewModel.update(node: node.viewData)

        if node.needsLoginFallback {
            callback?(.fallback)
        }
    }
}
