...
enum MockAuthenticationLoginScreenState: MockScreenState, CaseIterable {
    ...
    var screenView: ([Any], AnyView) {
        ...
        switch self {
        case .sdnDotOrg:
            viewModel = AuthenticationLoginViewModel(node: .mockSDNDotOrg)
        case .passwordOnly:
            viewModel = AuthenticationLoginViewModel(node: .mockBasicServer)
        case .passwordWithCredentials:
            viewModel = AuthenticationLoginViewModel(node: .mockBasicServer)
            viewModel.context.username = "alice"
            viewModel.context.password = "password"
        case .ssoOnly:
            viewModel = AuthenticationLoginViewModel(node: .mockEnterpriseSSO)
        case .fallback:
            viewModel = AuthenticationLoginViewModel(node: .mockFallback)
        }
