...
enum AuthenticationLoginViewModelResult: CustomStringConvertible {
    ...
    case selectServer
    /// Parse the username and update the node if included.
...
struct AuthenticationLoginViewState: BindableState {
    /// Data about the selected node.
    var node: AuthenticationNodeViewData
    /// Whether a new node is currently being loaded.
    var isLoading = false
    ...
    var showSSOButtons: Bool {
        !node.ssoIdentityProviders.isEmpty
    }
    ...
    /// `true` if valid credentials have been entered and the node is loaded.
    var canSubmit: Bool {
...
enum AuthenticationLoginViewAction {
    /// The user would like to select another server.
    case selectServer
    /// Parse the username to detect if a node is included.
    case parseUsername
    /// The user would like to reset their password.
    case forgotPassword
...
enum AuthenticationLoginErrorType: Hashable {
    /// An error response from the node.
    case mxError(String)
    /// The current node address isn't valid.
    case invalidNode
    /// The response from the node was unexpected.
    case unknown
}
