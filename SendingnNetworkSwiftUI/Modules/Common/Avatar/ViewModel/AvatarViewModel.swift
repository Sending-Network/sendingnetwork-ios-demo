...
final class AvatarViewModel: ObservableObject {
    ...
    func placeholderAvatar(sdnItemId: String,
                     displayName: String?,
    ...
    /// Load an avatar
    /// - Parameters:
    ///   - mxContentUri: The sdn content URI of the avatar.
    ///   - sdnItemId: The id of the sdn item represented by the avatar.
    ...
    func loadAvatar(mxContentUri: String?,
        ...
        guard let mxContentUri = mxContentUri, mxContentUri.count > 0 else {
            avatarCompletion(placeholderAvatar(sdnItemId: sdnItemId, displayName: displayName, colorCount: colorCount))
            return
        }
