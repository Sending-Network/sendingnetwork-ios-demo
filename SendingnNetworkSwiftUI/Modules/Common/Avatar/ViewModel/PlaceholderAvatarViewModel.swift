...
struct PlaceholderAvatarViewModel {
    ...
    /// The sdn id used as input to create the `stableColorIndex` from.
    let sdnItemId: String
    ...
    /// Provides the same color each time for a specified sdnId
    ///
    /// Same algorithm as in AvatarGenerator.
    /// - Parameters:
    ///   - sdnItemId: the sdn id used as input to create the stable index.
    /// - Returns: The stable index.
    var stableColorIndex: Int {
        // Sum all characters
        let sum = sdnItemId.utf8
            .map { UInt($0) }
            .reduce(0, +)
